...
const KNOWN_EXT_RE = new RegExp(`\\.(?:${[...new Set([...PHOTO_EXTS, ...AUDIO_EXTS, ...VIDEO_EXTS])].join('|')})$`, 'i');
...
export function isAudioKey(key: string): boolean {
  return AUDIO_EXT_RE.test(key) && !isVideoKey(key);
}

/** `.webm`은 소리에도 쓰이므로 영상 이름의 명시적 표식으로 가른다. `.mp4`도 같은 규칙을 쓴다. */
export function isVideoKey(key: string): boolean {
  return /__video__(?:op_[0-9a-f]{32}__)?[0-9a-f]{32}\.(?:mp4|webm)$/i.test(key);
}

/** 영상 표시용 포스터는 본체와 같은 operation/id fence를 쓰되 별도 WebP 객체로 보관한다. */
export function isVideoPosterKey(key: string): boolean {
  return /__video_poster__(?:op_[0-9a-f]{32}__)?[0-9a-f]{32}\.webp$/i.test(key);
}

/** 같은 media id에 본체·포스터·옛 operation 사본이 함께 있어도 하나도 덮어쓰지 않는다. */
export function groupKeysByMediaId(keys: string[]): Map<string, string[]> {
  const grouped = new Map<string, string[]>();
  for (const key of keys) {
    const id = mediaIdOfKey(key);
    if (!id) continue;
...
export const FN_VERSION = 8;
export const FN_OPS = ['probe', 'capabilities', 'list', 'put', 'get', 'delete', 'deleteMany', 'abortMultipart'] as const;
export const FN_SOURCE_SHA256 = '1619d7cfd519e60ddf5e58db9463ebb382838e7bf29f8eeaceaf2466d35ead38';
...
export async function handler(req: Request): Promise<Response> {
  ...
  if (op === 'list') {
    ...
    try {
      for (let page = 0; page < LIST_MAX_PAGES; page++) {
        ...
        for (const k of parsed.keys) {
          ...
          if (isVideoPosterKey(k)) videoPosterIds.push(id);
          else if (isVideoKey(k)) videoIds.push(id);
          else if (isAudioKey(k)) audioIds.push(id);
          else ids.push(id);
        }
    ...
    return json({
      ids: [...new Set(ids)],
      audioIds: [...new Set(audioIds)],
      videoIds: [...new Set(videoIds)],
      videoPosterIds: [...new Set(videoPosterIds)],
      foreign,
      truncated,
      count: ids.length,
  ...
  if (op === 'deleteMany') {
    ...
    const keysOfId = groupKeysByMediaId(before);

    for (const id of idsIn) {
      const keys = keysOfId.get(id) ?? [];
      // 목록에 없으면 이미 없는 것이다(멱등) — 실패로 세지 않는다.
      if (!keys.length) { sent++; continue; }
      ...
      for (const key of keys) {
        try {
          ...
          if (!r.ok && r.status !== 404) { failedStatus = r.status; break; }
        } catch {
          failedStatus = 0;
          break;
        }
      }
      if (failedStatus === null) sent++;
      else errors.push({ id, status: failedStatus });
    }
