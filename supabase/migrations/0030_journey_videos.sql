...
-- 외부 원본은 앱이 수정/삭제하지 않으며, R2에는 **앱이 만든 파생본만** 저장한다.
-- 🔴 파생본은 **둘이다**: 경량 영상과 포스터가 각각 별도 R2 객체로 가고, 두 바이트가 모두
--    되읽혀야 동기화 op이 완료된다. (이 줄은 원래 "경량본 하나만"이라고 적혀 있었는데
--    실제 구현과 어긋났다 — 2026-08-13 정정. 표 comment는 0033에서 함께 고친다.)
