...
export function photoPlaceNotice(hint: PhotoHint): string {
  ...
  if (hint.timedCount > 0) {
    ...
    return `📷 촬영시각은 읽었는데 위치 정보가 없어요 — 앱이 받은 사진에 위치가 안 담겨 있었어요. 💡 [🖼️ 갤러리에서]로 고르셨다면 폰이 위치를 가리고 넘겼을 수 있어요 — 그냥 [📷 사진 추가]로 고르시면 원본 그대로 들어옵니다. 카메라에 「위치 태그」가 꺼져 있었거나 사진을 주고받는 과정(메신저·클라우드)에서 빠졌을 수도 있어요. ${escape}`;
  }
