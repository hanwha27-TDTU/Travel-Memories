...
export function photoPlaceNotice(hint: PhotoHint): string {
  ...
  if (hint.timedCount > 0) {
    ...
    // 🔴 **실측으로 좁혔다**(2026-07-31 · 사용자가 원본 JPEG을 줬다 · M-0058).
    // 그 사진의 GPS 태그는 **껍데기만 있고 값이 비어 있었다**(`0/0, 0/0, 0/0` · Ref는 `""`).
    // 즉 카메라가 **위치를 안 적었다.** 그런데 갤러리는 그 사진을 「청주시 상당구」로 보여준다 —
    // 갤러리는 **파일이 아니라 안드로이드 미디어 DB**를 읽기 때문이고, 웹앱은 그 DB에
    // 접근할 수 없다(파일만 받는다).
    //
    // 그래서 안내가 **한 번 하면 영구히 끝나는 조치**를 알려 준다: 카메라의 위치 태그.
    // 다음에 할 일을 주는 것이 §12이고, 그중 **다시 안 겪게 하는 것**이 가장 좋은 답이다.
    return `📷 촬영시각은 읽었는데 위치 정보가 없어요 — 앱이 받은 사진에 위치가 안 담겨 있었어요. 카메라에 「위치 태그」가 꺼져 있었거나, 사진을 주고받는 과정(메신저·클라우드)에서 빠졌을 수 있어요(갤러리가 지도를 보여줘도 그건 폰이 따로 기억하는 것이라 앱은 못 봐요). 💡 카메라 설정에서 「위치 태그」를 켜 두시면 앞으로 찍는 사진은 자동으로 들어가요. ${escape}`;
  }
