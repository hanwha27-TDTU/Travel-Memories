...
export function photoPlaceNotice(hint: PhotoHint): string {
  ...
  if (hint.timedCount > 0) {
    // 🔴 **원인을 단정하지 않는다**(2026-07-31 정정 · M-0056).
    // 처음엔 *"안드로이드 사진 선택기가 위치를 지웠어요"*라고 **단정**해 적었다. 그런데
    // 사용자가 사후 업로드로 시험하자 **좌표가 그대로 들어왔다** — 그 선택기가 늘 지우는
    // 것이 아니었다. 원인을 하나로 못박으면 ①사용자가 엉뚱한 곳(설정·선택기)을 뒤지고
    // ②실제 원인(찍을 때 위치 기록이 꺼져 있었다)을 영영 못 찾는다.
    // §8: **모르는 것은 「확인 불가」다. 정상으로도 문제로도 반올림하지 않는다.**
    //
    // 🔴 **약속을 걷어냈다**(2026-08-01 10:27 실기기 · v1.42 · M-0068). 이 문장은 한동안
    // *"그냥 [📷 사진 추가]로 고르시면 원본 그대로 들어옵니다"*라고 **약속**했다 — 한 번의
    // 실측(00:19)을 일반화한 것이었고, 같은 날 같은 기기에서 **반증됐다**: 파일 선택기로
    // 골라도 위치 칸이 비워진 채(`0/0` · Ref `""`) 왔고 촬영시각은 읽혔다(🔬 실측). 갤러리
    // 선택기도 같았다. **지키지 못하는 약속은 안내가 아니라 거짓이다** — 실측된 사실과
    // 지금 되는 길만 말한다. 갤러리 앱이 그 사진을 지도로 보여주는 것과 파일에 위치가
    // 없이 오는 것은 **양립한다**(갤러리는 파일이 아니라 폰의 위치 기록을 볼 수 있다 — M-0058).
    return `📷 촬영시각은 읽었는데 위치 정보가 없어요 — 앱이 받은 사진에 위치가 안 담겨 있었어요. 폰이 브라우저에 사진을 넘기며 위치를 가리고 줄 수 있어요 — [📷 사진 추가]·[🖼️ 갤러리에서] 어느 쪽으로 골라도 그렇게 온 사례가 실측됐고, 갤러리 앱이 지도를 보여줘도 파일에는 위치 없이 올 수 있어요. 카메라에 「위치 태그」가 꺼져 있었거나 주고받는 과정(메신저·클라우드)에서 빠졌을 수도 있어요. 카메라로 그 자리에서 바로 찍은 사진에는 위치가 들어와요. ${escape}`;
  }
