...
//
// ─────────────────────────────────────────────────────────────────────────────
// 정직한 한계 — 이 게이트가 못 하는 것
// ─────────────────────────────────────────────────────────────────────────────
// 선언(`@live-covers`)이 **거짓일 수 있다.** 스크립트가 그 화면을 실제로 여는지까지는 이 정적
// 검사가 못 본다 — `check-skill-routing`이 받아들인 것과 같은 한계다. 그래서 이 게이트가 하는
// 일은 *"덮였다"의 보증*이 아니라 **"안 덮인 화면이 조용히 생기는 것의 차단"**이다.
// 실제로 열리는지는 라이브 스크립트 자신이 대상 0에서 공허하게 통과하지 않도록 막는다
// (`verify-diagnostics-live`가 목록 확보를 먼저 판정하는 이유 — 그 판에서 실제로 뚫렸다).
...
console.log(
  `check-live-coverage: OK (셀프테스트 통과 · 화면 ${screens.length}개 중 ${coveredCount}개를 라이브 ${coversByScript.length}종이 덮음 · 이유 있는 제외 ${NO_LIVE_REQUIRED.size}개)`,
);
