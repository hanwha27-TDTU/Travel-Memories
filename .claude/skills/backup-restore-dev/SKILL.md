...
8. **암호 키는 저장하지 않는다**: 사용자만 보유하며 분실 시 복원 불가임을 UI에 명시. 암호 없이 내보낼 때는 평문 PII 경고를 한 번 확인받는다.
...
4. `tests/unit/zip`: CRC 벡터(`0xCBF43926`)·왕복·한글 폴더·오프셋·손상 감지
5. `tests/unit/backupNaming`: 파일명 형식·FS 금지문자·충돌 방지
