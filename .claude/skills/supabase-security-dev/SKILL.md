...
## 🔴 `media-sign`에 속도·용량 한도를 넣지 않기로 했다 (2026-08-12 · ADR-0065)

**이 절은 「아직 안 한 것」이 아니라 「하지 않기로 판단한 것」이다.** 여기 적는 이유는
§7이 요구하는 것이다 — *"적용하지 않는 대상마다 이유를 코드에 남긴다. 이유 없는 제외는
결함이다."* 이유가 없으면 다음 사람(사람이든 AI든)이 **선의로 넣는다.**

### 무엇을 안 하기로 했나

`media-sign`의 `put` 서명 발급에 **사용자별 크기·개수·속도 한도를 넣지 않는다.**
(T-022 후보 20·21 중 `media-sign` 쪽. `geocode` 쪽은 넣었다 — 아래 「왜 갈랐나」.)

### 왜

1. **위협이 성립하려면 「이미 초대받은 계정」이 있어야 한다.** 이 함수는 JWT를 직접 확인하고
   허용목록(`journey.allowed_users`)까지 되묻는다. 혼자 쓰는 앱에서 그 조건을 만족하는 것은
   **사용자 자신**이다. T-019를 닫은 것과 같은 구조다(ADR-0062).
2. 🔴 **고치는 값이 이득보다 크다 — 그리고 그 값은 코드 줄 수가 아니다.**
   `supabase/functions/media-sign/index.ts`는 `schemas/sync-release-contract.json`에
   **논리 소스 SHA-256으로 묶여** 있고, `verify-sync-release-live`가 그 해시를 **운영 함수와
   대조**한다. 즉 이 파일을 한 글자라도 고치면 **§18-F의 배포 창**(제공자 먼저 배포 →
   운영 실물 되읽기 → 그 다음 앱)이 반드시 함께 와야 한다. 안 그러면 저장소가 운영보다
   앞서서 **다음 릴리스의 운영 대조가 실패**한다.
3. **키 경계는 이미 지켜지고 있다.** 요청 본문의 `prefix`·`bucket`은 읽지 않고, 객체 키의
   바깥 폴더는 **검증된 `sub`**에서만 나온다. 즉 남의 자리에 쓸 수는 없다. 남는 것은
   「자기 자리에 얼마나 많이 쓰는가」이고, 그건 저장소 비용 문제이지 침해가 아니다.

### 왜 `geocode`는 갈랐나 (§7 — 비대칭에는 이유가 있어야 한다)

`geocode`는 **남의 회사 쿼터**를 태운다(카카오·VWorld 키). 한 요청이 상류로 **두 번** 나가고,
그 비용은 우리가 통제하지 못한다. 반면 `media-sign`은 **우리 R2**에 우리 사용자의 자료를
쓰는 것이라, 넘치면 청구서로 보이고 진단·저장 용량 화면이 이미 그것을 센다.
그리고 `geocode`는 릴리스 계약에 묶여 있지 않아 배포 창 없이 고칠 수 있었다.

### 다시 열 조건

①이 앱이 여러 사람에게 열리거나 ②`media-sign`을 **다른 이유로** 고쳐 배포 창이 이미 열렸거나
③실제로 R2 비용이 관측되면. 그때는 이 결정을 되살리지 말고 **새로 심사한다.**
