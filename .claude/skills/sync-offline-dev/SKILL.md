...
1-B. 🔴 **동기화는 사용자 작업 도중에 끼어든다 — 그렇게 가정하라**(2026-07-26 M-0033).
   자동 실행 자격·진행 표시·다른 기기 확인은 `sync-runtime-dev`가 정본이다. 로그인 첫 확인과
   실제 보류 작업의 복구 신호만 자동 실행하지만, 여러 단계로 이뤄진 사용자 작업(복원·대량
   사진 인테이크·편집 굽기)은 **그 사이에 동기화가 한 번 도는 것을 기본값으로** 놓고 설계해야
   한다.
...
17. **프로토콜 판과 배포 소스 판은 다르다.** `FN_VERSION`은 응답 형태·연산 호환성만 말하며,
    같은 판 안에서 고친 timeout·CORS·진행 배선이 실제 운영 함수에 올라갔는지는 증명하지 못한다.
    `schemas/sync-release-contract.json`을 정본으로 Edge 소스의 LF 정규화 SHA-256을 생성하고,
    `capabilities.sourceSha256`를 운영에서 되읽어 정확 대조한다. Edge 소스가 바뀐 릴리스는
    **함수 선배포 → 운영 대조 → 앱 배포** 순서이며, 옛 앱이 새 함수에서 계속 동작하도록 하위
    호환을 유지한다.
