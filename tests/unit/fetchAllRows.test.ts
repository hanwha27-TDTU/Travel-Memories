...
import { fetchAllIds } from '../../src/services/canonicalSync';
...
describe('fetchAllRows (M-10 · listAll 페이지네이션)', () => {
  ...
  it('updated_at이 없는 영구삭제 원장도 1000건에서 잘리지 않는다', async () => {
    const r = await fetchAllIds(fakeClientWithRows(2500) as never,'purged_ids');
    expect(r.error).toBeUndefined();
    expect(r.data).toHaveLength(2500);
    expect(r.data.at(-1)).toBe('id-2499');
  });
});
