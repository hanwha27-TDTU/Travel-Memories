...
const sample = (): CollectedRows => ({ trips: [trip('t1')], moments: [moment('m1', 't1')], media: [media('md1')], expenses: [expense('e1')] });
...
describe('실기기 복원 드릴(importMergeRows over Dexie)', () => {
  it('빈 db에 복원 → 실제로 저장되고 되읽힌다(사진 바이트 포함)', async () => {
    ...
    expect(res).toMatchObject({ trips: 1, moments: 1, media: 1, expenses: 1, skippedEmptyGuard: false });
  ...
  it('빈-데이터 가드: 활성 데이터가 있는데 빈 백업을 넣으면 건너뛰고 로컬 보존', async () => {
    ...
    const res = await importMergeRows({ trips: [], moments: [], media: [], expenses: [] });
  ...
  it('LWW: 더 오래된 version은 로컬을 덮어쓰지 않는다', async () => {
    await importMergeRows({ trips: [trip('t1', { version: 3, title: '최신' })], moments: [], media: [], expenses: [] });
    const res = await importMergeRows({ trips: [trip('t1', { version: 2, title: '옛것' })], moments: [], media: [], expenses: [] });
  ...
  it('tombstone 우위: 더 높은 version의 삭제는 활성 로컬을 삭제로 전이', async () => {
    await importMergeRows({ trips: [trip('t1', { version: 1 })], moments: [], media: [], expenses: [] });
    await importMergeRows({
      trips: [trip('t1', { version: 2, deletedAt: '2024-07-01T00:00:00.000Z' })], moments: [], media: [], expenses: [],
    });
