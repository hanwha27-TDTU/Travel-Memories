...
import { pushPendingAudio, pullAudio, backfillAudioOps, type AudioRemote } from '../../src/services/sync';
...
describe('① push — 바이트와 메타가 함께 간다', () => {
  ...
  it('🔴 tombstone을 밀 때 바이트를 올리지도 지우지도 않는다 — 휴지통에서 복원되려면 파일이 남아야 한다', async () => {
    const a = await rec();
    const remote = fakeRemote();
    await pushPendingAudio(remote, USER);
