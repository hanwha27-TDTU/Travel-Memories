...
describe('날짜 처리', () => {
  ...
  it('fxDateFor: 사용일을 쓰되 미래는 오늘로 당긴다', () => {
    // 오프셋 0(UTC)으로 재면 ISO의 날짜가 그대로 사용일이 된다 — 기대값을 시간대에 묶지 않는다.
    expect(fxDateFor('2026-07-10T00:00:00.000Z', '2026-07-16', 0)).toBe('2026-07-10');
    expect(fxDateFor('2027-01-01T00:00:00.000Z', '2026-07-16', 0)).toBe('2026-07-16');
  });

  // ── 새 계약(M-0049 후반): 「사용일」은 **그 돈을 쓴 자리**의 날짜다 ──
  //
  // 사용자가 원래 발견한 결함이 정확히 이 형태였다(*"7월 15일 사용금액인데 왜 7월 16일 환율?"*).
  // 그때 원인은 UTC 절단이었고 이번엔 시차다 — 같은 근본형이라 자를 하나로 맞춘다.
  it('fxDateFor: 여행지 오프셋으로 날짜가 갈린다(같은 순간, 다른 사용일)', () => {
    const instant = '2026-07-15T17:30:00.000Z'; // 베트남 7/16 00:30 · 한국 7/16 02:30 · UTC 7/15
    expect(fxDateFor(instant, '2026-07-20', 0)).toBe('2026-07-15'); // UTC로 보면 15일
    expect(fxDateFor(instant, '2026-07-20', 420)).toBe('2026-07-16'); // 베트남(+7)에서는 16일
    expect(fxDateFor(instant, '2026-07-20', 540)).toBe('2026-07-16'); // 한국(+9)에서도 16일
  });

  it('fxDateFor: 오늘 당김은 **기기 기준 오늘**로 판단한다(여행지가 앞서 있어도)', () => {
    // 여행지에서는 이미 7/21인데 이 기기는 아직 7/20이면, 존재하지 않는 날의 환율을 요청하지 않는다.
    expect(fxDateFor('2026-07-20T17:00:00.000Z', '2026-07-20', 420)).toBe('2026-07-20');
  });
});
