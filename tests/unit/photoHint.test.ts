...
describe('photoPlaceNotice — 없을 때 **왜 없는지**와 **그럼 어떻게 하는지**', () => {
  ...
  // 🔴 **케이스를 뒤집었다**(2026-08-01 10:27 실기기 · v1.42 · §11 ② · M-0068). 전에는
  // *"[📷 사진 추가]로 고르시면 **원본 그대로** 들어옵니다"*를 정상으로 못박아 뒀다 — 한 번의
  // 실측(00:19)을 약속으로 승격한 것이었고, 같은 기기에서 반증됐다: **파일 선택기로 골라도**
  // 위치 칸이 비워진 채(0/0 · ref 없음) 왔고 촬영시각은 읽혔다(🔬 한 줄). 갤러리도 같았다.
  // 지키지 못하는 약속은 안내가 아니라 거짓이다. 통과시키려고 문장을 되돌리지 않는다.
  it('🔴 「사진 추가로 고르면 원본 그대로」를 **약속하지 않는다**(실기기 반증 · M-0068)', () => {
    const s = photoPlaceNotice(photoHintOf([meta({ takenAt: '2026-07-31T05:09:00.000Z' })]));
    expect(s).not.toContain('원본 그대로 들어옵니다'); // 옛 약속
    expect(s).toMatch(/실측/); // 재본 사실로 말한다 — 어느 선택기로도 비워져 온 사례가 있다
    expect(s).toContain('바로 찍은 사진'); // 실측으로 **되는** 길(직접 촬영)은 말한다
  });
