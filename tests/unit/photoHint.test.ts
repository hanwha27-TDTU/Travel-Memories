...
describe('photoPlaceNotice — 없을 때 **왜 없는지**와 **그럼 어떻게 하는지**', () => {
  ...
  it('🔴 원인을 **하나로 단정하지 않는다** — 가능성을 열어 둔다(§8 모르면 확인 불가)', () => {
    const s = photoPlaceNotice(photoHintOf([meta({ takenAt: '2026-07-31T05:09:00.000Z' })]));
    expect(s).toContain('있어요'); // 「~일 수 있어요」 — 단정형이 아니다
    expect(s).not.toMatch(/지웠어요|지웁니다/); // 확인하지 않은 것을 확인한 것처럼 말하지 않는다
    expect(s).toContain('위치 기록이 꺼져'); // 실제로 더 흔한 원인을 먼저 말한다
  });
