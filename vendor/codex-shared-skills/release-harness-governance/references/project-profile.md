...
실행물을 포장하는 프로젝트는 프로필이나 그 프로필이 가리키는 실행 원장에서 **전체 런타임 입력 모집단**과
제외 사유를 선언한다. 산출물의 current 판정·빌드 생략·빌드 뒤 검증은 같은 지문 함수를 써야 한다. 대표 HTML,
manifest 또는 한 번들만의 해시는 전체 실행물 정체성이 아니다. 모집단이 0이거나 입력 원장과 실제 포장 목록이
갈리면 `unknown`이 아니라 실패로 낸다.
...
재생성 원장은 입력과 출력을 구분하고 자기 출력·하류 산출물 메타가 상류 입력 지문으로 되먹임되는 간선을
금지한다. 수렴을 의도했다면 고정점 존재와 반복 재생성 불변을 별도 검사로 증명한다.
