...
|---|---|---|---|---|---|
| T-010 | 🖼️ **파일 실물 도구** — 기록이 가리키는 사진·소리가 저장소에 **있는가(전수)** | 대기 | 새 진단 도구(`group: 'files'`). 지금 그 단계는 **도구가 0개**라 허브가 「아직 이 단계를 보는 도구가 없어요」를 띄운다 | 2026-08-05 | `BLIND_SPOTS`(diagGroups.ts) · 사용자 지시 *"니가 못보는 건 다 넣어야 해"* |
| T-011 | 🖼️ 사진·소리가 **실제로 열리고 재생되는가** | 대기 | T-010과 같은 도구에 붙이거나 형제 도구. 「있다」와 「열린다」는 다른 질문이다(깨진 바이트) | 2026-08-05 | 같은 등록부 항목 |
...
> 🔴 **T-010~T-013은 사용자 지시 하나에서 나온 형제들이다**(2026-08-05: *"니가 못보는 건 당연히
> 모두 넣어야 해..빼면 안 됨"*). 지금 상태는 「뺀 것」이 아니라 **「이유와 함께 등록해 둔 것」**이다 —
