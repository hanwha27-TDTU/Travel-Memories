...
| `file.type`이 비면 EXIF를 안 읽는다 | **진짜 결함이었고 고쳤다**(v1.42 · M-0067). SAF 경로 제공자가 type을 비우면 좌표를 조용히 버렸다 — 이제 바이트(SOI)가 판별. **실기기에서 이게 그 증상이었는지는 미확인** |

### 확인된 사실

- **직접 촬영 · 파일 선택기** 경로 → 좌표가 **들어온다**(00:19 실기기: `36.60916, 127.50248` + 주소)
- **갤러리 선택기** 경로 → **안 들어온다**
- v1.40에서 **기본을 파일 선택기로 뒤집었다**(`ORIGINAL_ACCEPT`). `check-exif-order` 검사 D가 되돌림을 막는다.
- 🔴 **그런데 v1.40 배포 후에도 사용자는 「계속 안 된다」고 했다.** 같은 00:26 사진이었고,
  **그 사진을 새 기본 경로로 다시 골랐는지는 확인되지 않았다** — **이것이 지금 열려 있는 틈이다.**
