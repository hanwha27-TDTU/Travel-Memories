...
> **작성**: 2026-07-27 · **갱신 2026-07-29**(M-0049 시간대 · 지시문서 정본화 §14 · 하네스 빠른 차선)
>
...
| 현재 판 | **v1.23** (M-0049 전반 — 아래 §6) |
| 지금 하던 일 | 🔴 **M-0049 후반(표시)** — 타임라인 시각·Day 그룹을 여행지 오프셋 기준으로 · 여행 시간대 고르기 화면 · 순간별 예외 · 한국시간 환산 · `check-timezone` 갱신. 전반(저장)은 v1.23에 완료 |
...
| 🔴 **§13 내가 볼 수 있는 것을 사람에게 묻지 않는다** | 화면에 나가는 것을 바꿨으면 **브라우저로 열어서 본다.** 못 봤으면 「라이브 렌더 미실행」이라 적는다 | M-0046은 **자료구조가 옳고 화면 문장만** 틀렸는데, 게이트 31종·유닛 686건이 전부 초록인 채 배포됐다가 30분 뒤 사용자 스크린샷에 잡혔다 |
...
## 6. 지금 저장소 상태 (2026-07-28 세션 종료 시점)

### 배포 갭 — ✅ 없다 (v1.22까지 배포 그린 확인)

| 판 | 내용 | 코드(main) | 실기기 |
|---|---|---|---|
| v1.20 | **소리가 서버로 간다**(ADR-0032) — 마지막 「로컬 전용」 예외를 갚음 | ✅ | ✅ (Pages #167) |
| v1.21 | M-0046 — 찾아보지도 않고 「없다」고 말하고 삭제를 권한 것 | ✅ | ✅ (Pages #168) |
| v1.22 | 🔴 M-0048 — **기기마다 정반대 안내** + 바이트 자동 대조 | ✅ | ✅ (`844aae1`) |

> **미배포 커밋 1개**가 브랜치에 있다 — 라이브 검사(D층·이음매 통합검사)뿐이고 **앱 코드
> 무변경**이라 사용자 화면에 차이가 없다. 사용자 방침(*"배포를 좀 모았다가 하는건 어때?"*)에
> 따라 **다음 앱 변경과 묶어 배포**한다.

### 🔴 직전 세션에 새로 생긴 것 (이 문서의 다른 절이 아직 안 말하는 것)

| 무엇 | 왜 |
|---|---|
| `sync.ts` **`reconcileMissingBytes`** | 🔴 헌법의 read-back 규율이 **행에만** 걸려 있고 **바이트(R2 파일)에는 없었다**(§7 형제 비대칭). 이제 동기화가 서버 파일 목록 ↔ 이 기기의 바이트를 맞춰 보고 **로컬에만 있는 것을 자동으로 올린다.** 자동 하루 한 번 · 사용자가 부른 동기화(`deep`)는 항상. **올리기만 하고 지우지 않는다** |
| `diagnostics.ts` **`classifyMissingFiles(..., otherDevices)`** | 「이 기기에 없다」를 「없다」로 반올림하던 것(M-0048). 다른 기기가 있으면 **파괴적 버튼을 만들지 않는다**(비활성이 아니라 **없음**). 기본값을 안 둬 **누락이 컴파일 오류**가 된다 |
| `StoreHeadlineInput.fileBad`가 `Array<{noun,n}>` | 배너가 「**사진** 파일에…」로 못 박혀 있어 소리 문제를 사진이라 했다. 각 감사가 **자기 이름을 들고 다닌다**(`fileAuditMetrics`가 `noun` 반환) |
| `scripts/verify-diagnostics-live.mjs`(22건) | §10 ③(전달 결함)의 기계 층. **A** 실제 앱 · **B** 주입 판정 · **C** 🔴 **버튼을 실제로 누른다**(결과 문장·재판정·실패 사유·잠김) · **D** 요약 문장 |
| `scripts/check-live-coverage.mjs` | **「이 화면은 누가 눈으로 보나」**. `@live-covers` 선언 ↔ 디렉터리 대조. **선언까지만 본다**(라이브가 SKIP될 수 있으므로) |
| `CLAUDE.md` **§13** | 위 전부의 조항. 「완료의 정의」에 **「화면을 바꿨으면 열어서 보고, 버튼을 만들었으면 눌러 봄」** |
| `supabase/migrations/0020` | `journey.audio`에 `block_purged_reinsert` 트리거(M-0045) |
| `media-sign` **v6** | 사진(`webp`)과 소리(`webm/m4a/ogg/mp3/wav`)를 **분리해서** 센다 |
...
### 검증 현황 (2026-07-28 세션 종료 시 실측 — **숫자는 재서 확인하라**, 위 경고 참조)

- harness 게이트 전부 통과, **선택 게이트 포함 건너뜀 0** (측정 시점 33개)
- 유닛 전부 통과 (측정 시점 **722건 / 51개 파일**)
- 라이브 — `verify-editor-live` **165/165** · `verify-diagnostics-live` **22/22**
- 마이그레이션 20개 전부 적용 · 실수 원장은 **M-0048**까지
- 소스 `.ts` 파일 수는 `find src -name '*.ts' | wc -l`로 잰다

> ⚠️ **SKIP은 통과가 아니다.** `npm run harness`만 돌리면 `dist`가 없거나 낡았을 때
> 라이브 2종이 **SKIP**되고, 그 실행은 화면 층을 **한 번도 재지 않은 것**이다.
> 반드시 `npm run build && npm run live`까지 간다.

### 사용자가 확인 대기 중인 것 (실기기 — 배포 후)

1. 🔴 **바이트 자동 대조(v1.22)가 실제 네트워크로 도는가.** 로직과 이음매는 유닛 7건 +
   주입 3종으로 쟀지만 **샌드박스가 `*.supabase.co`를 막아** 실제 왕복은 못 봤다.
   관측 시점은 **다음에 어긋남이 생겼을 때** — 버튼을 누르지 않아도
   「소리 파일 N개 · 기록 N개」가 스스로 맞아야 한다.
2. **영상 기능**(`docs/VIDEO_PROPOSAL.md`) — 사용자 결정 대기(오래됨).
3. **신규 가입이 열려 있다** — Auth 설정이라 MCP로 못 한다(사용자 콘솔 몫).

---

## 7. 이 저장소가 반복해서 낸 실수 (근본형 — 전부 다시 날 수 있다)

전체는 `docs/records/coding-mistakes.md`(M-0001 ~ **M-0046**). 근본형만 추린다.
