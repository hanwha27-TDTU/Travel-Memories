...
> **작성**: 2026-07-27. 🔴 **이 문서에는 현재 버전·인계 번호를 적지 않는다**(§18-I — 손으로 적은 값은 반드시 밀린다). 현재 버전·migration·배포 상태는
...
## 🆕 현재 시작점 — **여기에는 숫자를 적지 않는다**

> 🔴 **이 절이 한 번 낡았다**(2026-08-15에 발견). 원래 제목이 「(2026-08-14 · **v2.37**)」이었고
> 본문이 「HANDOFF-0190을 읽어라」라고 **손으로** 적고 있었다. 그런데 그때 실제 상태는
> **v2.45 · HANDOFF-0221** — **여덟 판이 낡아 있었다.**
>
> 웃기는 것은, 바로 그 문단이 *"손으로 복제하지 않는다"*라고 말하고 있었다는 점이다.
> 이게 헌법 §18-I가 말하는 **「자동 옆의 수동」**의 교과서적 사례다: *자동으로 갱신되는 값
> 바로 옆에 사람이 적는 값이 있으면 그 수동 값은 반드시 밀린다.* 그리고 이 파일은
> **맥락 없는 AI가 가장 먼저 읽는 문서**라, 낡은 포인터는 그 AI를 여덟 판 전 세계로 보낸다.
>
> **그래서 숫자를 올리지 않고 자리를 없앴다.** 아래 어디에도 버전·인계 번호를 적지 않는다.
> 값을 올리는 수정은 다음 판에서 또 밀리지만, **적을 자리를 없애는 수정은 안 밀린다.**

**첫 30초에 읽을 것 — 전부 「맨 위」이거나 「생성물」이다.**

| 알고 싶은 것 | 어디를 보나 | 왜 여기가 정본인가 |
|---|---|---|
| 지금 앱 버전·게이트/도구 개수·적용 migration | `src/app/registry.gen.ts` | **생성물.** 코드에서 자동 집계되고 `check-registry-gen`이 드리프트를 양방향으로 RED |
| 지금 무슨 일이 있었나 / 배포 상태 | `docs/HANDOFF.md` **맨 위 항목** | 최신이 위. 항목마다 브랜치·검증·배포·정직한 한계가 들어 있다 |
| 지금 열린 과제 | `docs/BACKLOG.md` **BKL-1** | 미완료 과제의 **단일 정본**. `npm run brief`가 착수마다 띄운다 |
| 지금 코드 구조 | `docs/모듈별 설계서/README.md` | **생성물**(`npm run gen:module-designs`). 코드와 갈라지면 Required 게이트가 RED |
| 지금 하지 말아야 할 것 | `docs/records/coding-mistakes.md` **맨 위** | 최신 실수가 위. 같은 자리에서 두 번 넘어지지 않게 |
| 릴리스 절차 전부 | `docs/HARNESS_RELEASE_LAW.md` | **생성물** — 아래 절 참조 |
| 진단 시스템 계약 | `docs/DIAGNOSTIC_TOOL_DESIGN_BLUEPRINT.md` | 다른 앱 이식용 정본 |

🔴 **가장 빠른 길은 명령 하나다**: `npm run brief <고칠 파일들>` — 읽을 스킬 문서·형제 목록·
그 영역의 과거 실수·열린 과제·미완성 작업을 **그 자리에서** 뽑아 준다. 이 표를 외울 필요가 없다.

**아래 버전별 절은 과거 사건의 근거이며 최신 상태가 아니다.** 역사로 읽고, 현재 상태는 위 표에서 읽는다.

### 🔴 릴리스를 하려면 이 문서 하나부터 — `docs/HARNESS_RELEASE_LAW.md` (2026-08-09 신설)

릴리스 규율이 헌법 안에 네 곳으로 흩어져 있어(§18 · §15 · 「완료의 정의」 · 「검증 명령」)
릴리스하려는 사람이 스스로 주워 모아야 했다. **이제 통합본 하나가 있다.**

- 🔴 **그 파일은 생성물이다. 거기를 고치면 `check-adapter-parity`가 RED로 막는다.**
  고칠 곳은 언제나 `docs/CONSTITUTION.md`이고, `node scripts/gen-harness-law.mjs`로 심는다.
  (같은 계약을 `CLAUDE.md`·`AGENTS.md`가 이미 지고 있다 — 손편집 중복 자체가 결함이다 · §2)
- **다른 앱에 이식할 때 건네줄 파일도 이것 하나다.** 기계층이 사는 파일 목록이 그 문서 머리에 있다.

**코덱스가 릴리스할 때 바뀐 것 둘**(2026-08-09 · v2.08):

1. **묶음을 얼린다**(§18-H). 검증을 시작하면 `npm run release:arm <후보 SHA>`. 그 뒤로
   `commit`·`push`·`merge`·`rebase`가 막힌다 — **PreToolUse 훅이 막으므로 Claude 쪽에서만
   걸린다**(터미널에서 사람이 직접 치는 것은 못 본다: 정직한 한계). 빨간 게이트를 고치려면
   `npm run release:reopen -- "<이유 10자 이상>"`으로 **이유를 적고** 다시 연다 — 금지가
   아니라 **기록**이고, 재개방 횟수는 완료 보고에 적는다. 배포 그린 확인 뒤 `npm run release:close`.
   > 왜: v2.07에서 검증 중에 커밋을 두 번 얹어 CI가 네 번 돌았다(3~4분짜리 검증이 41분).
   > `cancel-in-progress` 때문에 「커밋 하나 더」의 값은 **검증 한 판 전체**다.
2. **게이트는 자기가 사는 세계를 가정하면 안 된다**(§18-G). 새 게이트를 만들면
   `src/app/gates.ts`의 **`GATE_WORLD`에 한 줄**을 적어야 한다 — 아무것도 가정하지 않으면
   **빈 문자열**(빈칸과 「아직 안 봤다」는 다른 말이라 누락은 RED). `check-env-assumption`이
   얕은 체크아웃과 같은 모양의 빈 세계 사본에서 싼 게이트를 전부 돌려 **선언을 검증**한다.
   > 종료코드 계약: `0`=통과 · `1`=재보고 위반 · `2`=전제 미충족(SKIP).
   > **전제가 없을 때 예외를 스택째 뱉으면 종료코드가 1이 되어 「위반을 찾았다」로 보고된다 —
   > 그건 거짓말이다.** 판정하지 말고 「확인 불가」라고 말하라.
...
## 🆕 v1.63 배포 완료 — 운영 동기화 복구 + 앱 선배포 호환성 (2026-08-03)
...
## 🆕 v1.47 → v1.57에서 무슨 일이 있었나 (코덱스가 오래 비운 사이 · 2026-08-02)

> **이 절이 가장 최신이다.** 아래 다른 절들(§3~§7)은 여전히 유효하지만, 이 절이 말하는
> 것들이 그 위에 얹혀 있다. 코덱스가 마지막으로 본 것은 대략 v1.35~v1.47이므로, 그 사이에
> **셸(APK)·아이콘·자동갱신·설치 플레이북**이 통째로 새로 생겼다. 순서대로 읽어라.

### 한눈에 (무엇이 새로 생겼나)

| 판 | 무엇 | 정본 문서 |
|---|---|---|
| v1.48 | **접속하면 스스로 최신화** — `dist/version.json` 신호 + 안전할 때만 새로고침(입력 중이면 미룸) | `android-apk-dev` §1(7·8) · `appUpdate.ts` · `check-update-signal` |
| v1.49~v1.53 | 시간대 검색 UX · **감사 배치 ①~④**(문서 드리프트 · 동기화 결함 M-1~M-13 · 보안 H-1/M-16 · 디자인 H-6/빈상태/죽은코드) | `docs/HANDOFF.md` 해당 줄 · `coding-mistakes` M-0069~M-0082 |
| v1.53 | **접근성 강조색** — 흰 글자가 원색 채움 위에서 대비 무너진 것 수정(`--accent-strong` 등 `color-mix`) | `ui-responsive-dev` · M-0082 · `check-ui-color-token` |
| v1.54 | **앱 아이콘 5종 + APK 앱 내 전환기**(여권·나침반·지구본·캐리어·산) — activity-alias 방식 | **ADR-0038** · `IconSwitcherPlugin.java` · `apk.ts` `APP_ICONS` |
| v1.55 | **debug APK 고정 키스토어** — 「앱이 설치되지 않음」(서명 불일치) 해결 | **ADR-0039** · `android-shell/.../bugeon-debug.keystore`(커밋됨 — §0 비밀 아님) |
| v1.56 | **셸 업데이트 배너** — 새 APK가 나오면 앱 하단에서 알림. versionCode 자동증가 + shell-version 마커 | **ADR-0040** · `shellUpdate.ts` · `apk.ts` `parseShellBuild`/`APK_RELEASE_API` |
| v1.57 | **아이콘 글자 잘림 수정**(두 층: 흐린 배경+안전지대 카드) + **설치 플레이북 내려받기**(HTML·MD) | **ADR-0042**·M-0083 · **ADR-0041** · `playbook.ts` |

### 🔴 코덱스가 반드시 알아야 할 새 계약 (셸이 생기며 바뀐 세계관)

1. **이 앱은 이제 「웹앱 + 안드로이드 셸」 두 몸이다**(ADR-0036). 셸은 새 앱이 아니라 **WebView
   하나 + 네이티브 문 하나**다. `capacitor.config.json`의 `server.url`이 배포된 웹을 가리키고,
   **웹 자산을 셸에 번들하지 않는다.** → 웹 코드를 고쳐 배포하면 **재설치 없이** 앱이 갱신된다.
   APK 재빌드는 **셸 자체**(아이콘·권한·서명·네이티브)가 바뀔 때만 필요하다.
2. **APK 관련은 전부 `.claude/skills/android-apk-dev/SKILL.md`가 정본이다.** `android-shell/**`,
   `.github/workflows/android-apk.yml`, `src/app/apk.ts`, `src/services/(appUpdate|nativePhotos|capacitorShell|shellUpdate).ts`,
   `src/app/playbook.ts`를 만지기 전에 **반드시 로드**하라(`npm run brief <파일>`이 띄운다).
3. **「항상 최신 APK」는 고정 태그 + `--clobber`**(다운로드 URL 불변). 계약이 여러 자리에 걸쳐
   있고 `check-apk-release-link`가 대조한다 — 셸 업데이트 배너까지 넷을 본다(§1-9).
4. **버전 SSOT는 `src/app/changelog.ts` 맨 앞 하나.** `dist/version.json`(자동갱신 신호)·설치
   플레이북 버전·about 화면이 전부 여기서 파생된다. 다른 곳에 손으로 적지 마라(§7).
5. **셸 감지는 `capacitorShell.ts` 한 곳**에서만(`shellPlugin()`이 크롬에선 null → 웹 경로 무손대).
   셸 전용 분기를 여러 파일에 흩지 마라.

### 🔴 이 사이에 재확인된 근본형(§7) — 새로 만든 것은 형제의 규율을 물려받지 않는다

- **아이콘 잘림(M-0083)**: 「배경=전체이미지/전경=투명」으로 두고 *"글자가 안 잘린다"*고
  **적어 두고 안 쟀다.** 적응형 바깥 ~18dp bleed가 글자를 잘랐다 — 실기기가 반증. →
  **헌법 §13-6 신설**: *마스크·크롭·safe-area가 걸리는 자리는 「보이는 캔버스」가 아니라
  「실제로 남는 영역」을 본다.* 원·스퀘어클 두 마스크로 렌더해 확인하라.
