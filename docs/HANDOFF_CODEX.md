...
> **작성**: 2026-07-27 · **갱신 2026-07-28**(오디오 서버 동기화 완료 · M-0046 · 헌법 §13 신설)
...
| 현재 판 | **코드 = 실기기 = v1.21** (배포 갭 없음 — 아래 §6) |
| 지금 하던 일 | 없음(다음 사람이 고름). 직전: 오디오 서버 동기화 **완료**(v1.20) → M-0046 수정(v1.21) → **게이트 2종·헌법 §13 신설**(미배포·앱 코드 무변경) |
...
### 배포 갭 — ✅ 없다 (2026-07-27의 갭은 해소됐다)

| 판 | 내용 | 코드(main) | 실기기 |
|---|---|---|---|
| v1.17~v1.19 | 순간 실행취소·위치 칩·지도 | ✅ | ✅ |
| v1.20 | **소리가 서버로 간다**(ADR-0032) — 마지막 「로컬 전용」 예외를 갚음 | ✅ | ✅ (Pages #167) |
| v1.21 | M-0046 — 찾아보지도 않고 「없다」고 말하고 삭제를 권한 것 | ✅ | ✅ (Pages #168) |

> **2026-07-27의 「Actions 사용량 한도 초과」는 풀렸다.** 다만 사용자가
> *"앞으로 배포를 좀 모았다가 하는건 어때?"*라고 했으므로 **문서·게이트만 바뀐 커밋은
> 배포하지 않고 다음 앱 변경과 묶는다.** 지금 그 상태의 미배포 커밋이 브랜치에 있다
> (라이브 게이트 2종 + 헌법 §13 — **앱 코드 무변경이라 사용자 화면에 차이가 없다**).

### 🔴 직전 세션에 새로 생긴 것 (이 문서의 다른 절이 아직 안 말하는 것)

| 무엇 | 왜 |
|---|---|
| `scripts/verify-diagnostics-live.mjs` | §10 ③(전달 결함)에 **아예 없던 기계 층.** 실제 Chromium이 진단 도구 7개를 하나씩 열어 판정·글리프·자리·넘침을 잰다(A층). 앱에서 만들 수 없는 상태는 **판정 함수에 먹여 실제 렌더러로** 그린다(B층 — `.diaglive/`를 게이트가 직접 빌드) |
| `scripts/check-live-coverage.mjs` | **「이 화면은 누가 눈으로 보나」**를 묻는다. 라이브 스크립트의 `@live-covers:` 선언 ↔ `src/ui/{screens,panels}/` 대조. 제외는 **이유와 함께만** 등록된다 |
| `CLAUDE.md` **§13** | 위 둘의 조항. 「완료의 정의」에 **「화면을 바꿨으면 열어서 봄」** 단계가 들어갔다 |
| `supabase/migrations/0020` | `journey.audio`에 `block_purged_reinsert` 트리거(M-0045 — 형제 목록을 「그 표의 정의」에서만 뽑아 놓쳤다) |
| `media-sign` **v6** | 사진(`webp`)과 소리(`webm/m4a/ogg/mp3/wav`)를 **분리해서** 센다. 합치면 소리가 「기록 없는 사진 파일」로 뜬다 |
...
### 검증 현황 (2026-07-28 실측 — **숫자는 재서 확인하라**, 위 경고 참조)

- harness 게이트 전부 통과, **선택 게이트 포함 건너뜀 0** (측정 시점 33개)
- 유닛 전부 통과 (측정 시점 702건 / 49개 파일)
- 라이브 렌더 — `verify-editor-live` 165/165 · `verify-diagnostics-live` 12/12
- 마이그레이션 20개 전부 적용 · 실수 원장은 M-0046까지
...
1. 🔴 **v1.21**: 「앱 상태 확인」에서 **[서버에 없는 자료 다시 올리기]**를 눌러야 소리 3건의
   바이트가 R2로 올라간다. 성공하면 「소리」가 **클라우드 4 · 이 기기 4**로 맞는다
   — **그 지표가 곧 read-back이다.**
