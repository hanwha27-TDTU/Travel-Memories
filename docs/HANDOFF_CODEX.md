...
## 🔴🔴 지금 열려 있는 문제 — **여기서 이어받아라** (2026-08-01 · v1.41 기준)

> 이 절이 **문서 전체에서 가장 먼저 읽을 것**이다. 아래 나머지는 참조용이다.

### 증상

**사용자 실기기에서 사진의 위치가 장소 칸에 안 들어온다.** v1.31부터 v1.41까지 **열한 판**을
냈고 **여전히 미해결**이다. 나는 이 건에서 **원인을 네 번 못박고 네 번 틀렸다.**

### 🔴 추측하기 전에 이 두 가지부터

```bash
npm run known 사진 위치 선택기      # 이 저장소가 이미 아는 것 — 원장·ADR·헌장·🔴 코드 주석
```

그리고 **사용자에게 [🔬 앱이 받은 사진 정보 보기] 한 줄을 요청하라**(v1.41에 만든 관측 창).
파일이 아니라 **그 줄**이다. 그 줄이 세 갈래를 **한 번에** 가른다:

| 그 줄 | 뜻 | 다음 |
|---|---|---|
| `위치 칸 자체가 없음` | 카메라가 안 적었다 | **앱 결함 아님.** 카메라 설정 안내로 끝 |
| `위치 칸은 있는데 값이 비어 있음` | 누군가 비우고 넘겼다 | **경로가 변수다** — 어느 버튼으로 골랐는지 물어라 |
| `위치 있음 (N 36/1,…)` | 바이트에 좌표가 **있다** | 🔴 **파서·배선 결함이다.** 그 자리를 고쳐라 |

### 이미 배제된 것 (다시 뒤지지 마라 — 내가 거기서 나흘을 썼다)

| 가설 | 배제 근거 |
|---|---|
| 압축이 EXIF를 먼저 지운다 | EXIF 읽기가 압축보다 앞이다(코드 + `check-exif-order` 게이트) |
| 파서가 파일 앞부분만 읽는다 | `EXIF_HEAD_BYTES = 256KB` — APP1 최대 64KB를 충분히 덮는다 |
| 첫 APP1이 Exif가 아니면 포기 | **진짜 결함이었고 고쳤다**(M-0063). 단 그 증상은 *시각도 없음*이라 이 건과 다르다 |
| 채팅 업로드가 GPS를 0으로 덮는다 | **사실이다**(M-0059 실측 25바이트). 그러나 **나에게 오는 경로**일 뿐 앱 경로가 아니다 |
| 0,0을 좌표로 받는다 | 고쳤다(M-0057 — 파서·도메인·geojson·화면 4층) |
| `file.type`이 비면 EXIF를 안 읽는다 | **진짜 결함이었고 고쳤다**(v1.42 · M-0067). 다만 **이 증상의 원인은 아니었다** — v1.42 실기기에서 증상 재현, 🔬가 「바이트가 0/0으로 도착」을 보여줌(10:27) |
| 파서·배선 어딘가가 좌표를 놓친다 | **종결**(2026-08-01 10:27 실기기). 촬영시각은 읽혔고 🔬(바이트 직접 관측)와 파이프라인이 같은 답 — 좌표는 **브라우저에 닿기 전에 이미** `0/0` |

### 확인된 사실 (2026-08-01 10:27 실기기 · v1.42로 갱신)

- 🔴 **열려 있던 틈이 닫혔다.** 사용자가 v1.42(버전 배지 확인)에서 **파일 선택기·갤러리
  둘 다로 다시 골랐고 둘 다** 위치가 비워진 채 왔다 — 🔬: `20260731_194404.jpg · 정보칸
  exif · 촬영시각 2026-07-31T19:44:04 · (ref없음) 0/0,0/0,0/0`.
- **직접 촬영** → 좌표가 **들어온다**(사용자 확인 — 유일하게 생존하는 사진 경로).
- 원본에 GPS가 있는 것은 확정(사용자 ZIP 대조실험 · M-0059).
- 🔴 **파서·배선 갈래 종결**: 촬영시각이 읽혔고 M-0067 수정이 배포된 상태의 실측이다.
  남은 변수는 **전달 경로 하나** — 바이트가 브라우저에 닿기 전에 이미 `0/0`이다.
- 00:19 「파일 선택기로 들어옴」 실측과 10:27 반증이 **둘 다 실재** — 무엇이 갈랐는지 확인
  불가(후보: 선택기에서 최근/이미지 탭 vs 폴더 트리 진입 — 미검증).
- v1.43이 화면 안내에서 「파일 선택기로 고르면 원본 그대로」 **약속을 걷어냈다**(M-0068).
