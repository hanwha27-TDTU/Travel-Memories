...
## 기술 상세 기록 (최신이 위 · 이 문서는 배포 여부를 판정하지 않는다)

> 🔴 **예전 제목은 `[Unreleased]`였고, 그 라벨은 거짓이었다**(2026-08-12 정정).
> 아래 항목은 **전부 이미 배포됐다** — 가장 최근 항목이 v2.15인데 저장소는 v2.17까지
> 배포를 마쳤다(Deploy to GitHub Pages run 287 · `84d8bb2` · success). Keep a Changelog 서식을
> 들여오면서 절을 **한 번도 닫지 않아** 문서 라벨과 현실이 갈라진 채 굳었다(§17 「문서 ↔ 코드」).
>
> **왜 버전별로 쪼개지 않았나**: 이 문서의 항목은 **35개**인데 실제 릴리스는 **217개**다
> (`src/app/changelog.ts` 실측). 한 항목이 여러 버전에 걸쳐 있어 **1:1 대응이 존재하지 않는다** —
> 예컨대 2026-07-23 하루에만 v0.01~v0.34가 있다. 없는 대응을 지어내는 것은 정리가 아니라
> 날조다(비타협 원칙 #4 「정직한 완료」). 그래서 **라벨만 사실로 고쳤다.**
>
> **이 문서가 못 보는 것**: 무엇이 배포됐는가. 그 답의 정본은 `src/app/changelog.ts`(버전)와
> 배포 워크플로의 성공(배포 사실) 둘이다. 🔴 **여기서 「미배포」를 읽지 마라** — 이 문서는
> 릴리스마다 갱신되지 않는 **선택적 기록**이라, 항목이 없다는 것은 「배포 안 됨」이 아니라
> **「여기 안 적음」**이다.

### 「대조군 없음」은 처음부터 오탐이었다 — 63개 전부 갖고 있었다 (2026-08-12) · v2.25
- 🔴 **M-0152** — 같은 게이트가 네 판에 걸쳐 틀린 숫자를 냈다: **51 → 41 → 12 → 0**. 매번 내가 **아는 한 가지 모양**을 「그 개념 전부」로 착각했다(플래그만 · `function selfTest`만 · `exit 2`만 · 그리고 `throw new Error('SELF-TEST …')`를 몰랐다).
- **§4는 처음부터 지켜지고 있었다.** 안 지켜지고 있던 것은 **내 검출기**였고, 이 작업의 출발점(*"지키는지 아무도 안 봤다"*) 자체가 틀린 관측이었다.
- 🔴 **열어 보다가 진짜 결함을 찾았다**: **19개가 자체검사 실패를 `throw`로 알린다.** 던지면 스택과 함께 `exit 1`이 되고 하네스는 **「위반을 찾았다」**로 적는다. 자체검사 실패는 위반이 아니라 **「이 게이트를 믿을 수 없다」**이므로 `exit 2`가 맞다(§18-G).
- **세 번째 축 `CLEAN_EXIT`를 래칫으로 세웠다**(기준선 40). 3개를 판정 형태로 바꿨다(`check-hooks-wired`·`check-hand-counts`·`check-doc-counts`). **19개를 한 번에 바꾸지 않는다** — 급하면 §11 ②를 밟는다.
- 🔴 그 과정에서 `check-hand-counts`의 **대조군 구멍**도 나왔다: `stripMarkers`를 망가뜨려도 안 잡혔다(사례가 없었다). 양성·음성 2사례를 채웠다. **대조군이 있다고 그 함수가 덮인 것은 아니다.**
- 🔴 **기준선을 세 번 추측했다가 세 번 틀렸다**(24→22 · 40→37 · 44→40). *"래칫 숫자는 세어서 넣는다"*를 적어 놓고 두 번 더 반복했다 — 규칙을 쓰는 것과 지키는 것은 다르다.
- 주입 3방향(대조군 축 · 실행 가능 축 · **판정 방식 축**) 전부 RED → GREEN.

### 검출기가 한 가지 모양만 알아 멀쩡한 형제 39개를 「결함」으로 셌다 (2026-08-12) · v2.24
- 🔴 **M-0151** — 사용자가 *"대조군 없는 게이트 41개 진행하자"*고 해서 붙이려고 첫 파일을 열었더니 **이미 양성·음성 6사례가 들어 있었다**(`check-ui-color-token`). 41개 중 **39개가 이미 대조군을 갖고 있었다.**
- 원인: 검출기가 `function selfTest` **한 모양만** 알았다. 이 저장소에는 세 모양이 있고(이름 붙인 함수 · **이름 없는 블록** · 인자 받는 형태), 39개가 블록 형태였다.
- 🔴 **방향이 나쁜 결함이다 — 오탐이다.** 시키는 대로 했으면 이미 있는 대조군 위에 39개를 덧쓰며 §11 ②(옛 전제를 정상 케이스로 못박음)를 **39번 반복**할 뻔했다.
- 고침: 검출을 두 갈래로 넓히고 판정 근거를 이 저장소의 **종료코드 계약**(자체검사 실패 = exit 2)에 뒀다. 블록 형태 3사례를 음성 포함해 박았다. **대조군 보유 22 → 51로 정정.**
- 진짜로 없던 3개(`check-sw`·`check-lazy-screens`·`check-fn-size`)에 대조군을 붙였다. 셋 다 **판정 함수를 망가뜨려 exit 2로 잡히는지** 확인했다.
- 🔴 그중 `check-fn-size`의 첫 판은 **깨끗하게 판정하지 못하고 죽었다**(`one[0].name`이 TypeError → exit 1 + 스택). 죽음은 위반이 아니다(§18-G) — 옵셔널 체이닝으로 고쳐 exit 2로 나가게 했다.
- 🔴 **오늘 같은 부류로 네 번 틀렸다**: 주석을 코드로 셈(×2) · 플래그만 세면서 대조군을 센다고 함 · 이번 건. 근본형은 **내가 아는 한 가지 모양을 「그 개념 전부」로 착각하는 것**이다.

### 게이트가 자기가 무엇을 재는지 틀리게 말했다 (2026-08-12) · v2.23
- 🔴 **v2.22에서 만든 `check-gate-control`이 스스로 틀렸다.** 「대조군 보유」를 잰다고 하면서 실제로는 **`--selftest` 플래그 노출**만 셌다. 실측하니 **대조군을 갖고도 플래그를 안 단 게이트가 14개** — 진짜 숫자는 12가 아니라 **22**였다. 자기가 재는 것을 틀리게 말하는 것은 §8이 금하는 반올림이다.
- 축을 둘로 갈랐다: **`HAS_CONTROL`**(대조군이 있는가 — §4가 요구하는 것) · **`RUNNABLE`**(밖에서 돌려 볼 수 있는가 — 살아 있는지까지 재려면 필요). 두 기준선 모두 한 방향 래칫이고, 판정문이 *"돌려 본 것은 N개뿐"*이라고 **못 잰 것을 스스로 말한다**.
- 🔴 **주입이 이 게이트의 결함을 잡았다**: 호출을 `// selfTest();`로 **주석 처리해도 살아 있다고 셌다.** 오늘 `check-edge-cors`에서 똑같이 당한 자리를 그대로 반복한 것이다(§7 — 한 곳에서 옳은 것은 형제 전부에서 옳다). `stripComments`를 넣고 그 두 형태를 음성 사례로 박았다.
- `check-secret-leak`·`check-backup-coverage`·`check-blueprint` 셋에 `--selftest`를 노출했다 — **대조군은 이미 있었고**(secret-leak은 양성 8·음성 3) 밖에서 못 돌리고 있었을 뿐이다.
- 🔴 **기준선을 추측했다가 틀렸다**(24로 적었는데 실제 22). 실측으로 바로잡았다 — 래칫 숫자는 세어서 넣는 것이지 어림하는 것이 아니다.
- M-0150 기록: 같은 날 **종료코드 0을 보고 세 번 「됐다」고 믿었고 셋 다 대상이 틀렸다**(감시가 403으로 침묵 · 배포했는데 브라우저가 못 부름 · push가 옛 브랜치). 셋 다 남이 잡았다.
- 주입 2방향(대조군 축 · 실행 가능 축) 전부 RED → GREEN. 게이트 65개 유지.

### 게이트의 대조군과 두 AI의 강제 대칭 (사용자 제안·지시, 2026-08-12) · v2.22
- 🔴 **§4가 조항으로만 있었다.** *"알려진 실패를 주입해 RED로 잡히는지 확인한 뒤에만 게이트를 신뢰한다"* — 지키는지 아무도 안 봤다. 실측: 게이트 62개 중 `--selftest`를 **실제로 처리하는** 것은 **10개**뿐이었다(`grep`으로 세면 12로 보이는데, 둘은 주석·픽스처 이름일 뿐이다 — **말이 아니라 코드를 본다**).
- `check-gate-control` 신설: ①대조군 보유 게이트 수의 **래칫**(한 방향) ②있다고 한 셀프테스트를 **실제로 돌려** 통과를 확인 ③대상 0이면 통과가 아니라 exit 2. 대조군 없는 게이트 51개의 **이름을 판정문에 나열**한다(§12 — 앱이 알면 말한다).
- 🔴 **과소평가함을 스스로 말한다**: `check-schema-parity`처럼 플래그 없이 본문에서 주입하는 게이트는 안 세진다. 「대조군이 *적절한가*」도 못 본다 — 그건 판단이고 §11 ②가 그 사고를 이미 기록했다.
- 🔴 **강제층이 두 AI에게 갈라져 있었다**(M-0149). `.claude/settings.json`의 훅 4개는 **Claude만** 받는데, `AGENTS.md`는 *"실제 강제는 command hook과 CI가 한다"*고 적어 **코덱스에게 없는 보호막을 있다고 말하고 있었다.** 그리고 두 AI가 모두 받는 유일한 층인 git 훅은 `core.hooksPath` 미배선으로 **아무에게도 안 걸리고 있었다.**
- 고침: `package.json`의 `prepare`가 자동 배선(사람이 기억해서 켜는 층은 새어 나간다) · `check-enforcement-parity` 신설(공통 층 실행 권한·자동 배선·Claude 전용 훅의 **코덱스 대체 명시** 강제) · 헌법 S-09에 **「강제 세 층과 각 층이 묶는 대상」** 표와 *"새 강제는 위쪽 층부터 고른다"* 규칙.
- 주입 증명 3방향(대조군 제거 · 자동 배선 제거 · 미등록 훅 추가) 전부 RED → 복원 GREEN. 게이트 63 → 65.
...
- `hook-sql-safe`에 **`WHERE` 없는 `UPDATE`** 검출을 넣었다(`hasWhereLessUpdate` — `;`로 문장을 갈라 판정). 자체검사에 양성 2·음성 2를 함께 넣어 공허하지 않음을 확인했다.
- 진단 보고서 redaction이 **따옴표 붙은 JSON 형태**를 놓쳤다(실측: `{"api_key": "…"}`만 그대로 남았다). 열쇠말과 `:` 사이·값 앞뒤의 따옴표를 허용하고, 값 끝에서 닫는 따옴표·괄호를 제외해 다음 항목을 삼키지 않게 했다. 주입 RED→GREEN.
- **열어 둔 화면에도 잠금이 따라온다.** `openDataManager`가 `onAuthChange`를 구독하고, 첫 확인과 이후 사건이 **같은 함수**(`applySignedIn`)를 탄다(M-0060 — 같은 판정을 두 번 쓰지 않는다). 로그인 상태가 그대로면 조기 반환하므로 토큰 갱신이 작업 중인 사용자를 밀어내지 않는다. 함수 크기 래칫이 걸려 `buildHubShell()`·`buildHubButton()`을 top-level로 분리했다(§7 2층 — 게이트가 설계를 밀어줬다).
- **안 한 것**: 여행 상세 화면의 세션 잔존 재검사는 하지 않았다. 토큰 갱신 사건이 작업 중 화면 이동을 일으킬 위험이 사용자 제약(「사용하는데 불편이 있으면 절대 안 된다」)과 정면으로 부딪히고, 그 화면의 자료는 이 기기의 로컬 기록이라 노출 경계가 다르다 — ADR-0067.

### 큰 JSON 백업의 조용한 디코드 실패와 Android 사진 메모리 경계 (T-021, 2026-08-12) · v2.19
- 실측: `TextDecoder().decode()`는 V8 문자열 상한(`2^29-24`)을 한 바이트만 넘겨도 **던지지 않고 빈 문자열**을 준다. 그 결과 앱이 멀쩡한 백업을 「JSON 형식 아님」이라고 말했다 — 그 말이 시키는 다음 행동이 「버려라」라서 기억 유실 경로였다(M-0147).
- `MAX_BACKUP_JSON_TEXT_BYTES`(실측 경계)와 `decodeBackupJsonBytes()`를 두고, 상한 초과는 **크기 때문임을 말하고 ZIP 백업을 안내**한다. 상한 안이어도 「바이트는 있는데 글자 0」이면 파일 탓을 하지 않는다(read-back).
- ZIP 경로는 미디어가 별도 파일이라 이 절벽에 닿지 않으므로 JSON 경로 한 곳만 지킨다(§7 제외 이유 명시).
- Android `OriginalPhotosPlugin`: 손으로 두 벌 적혀 있던 읽기 경로를 `readBytes()` 하나로 합치고, `Runtime.getRuntime().maxMemory()/4`를 상한으로 둔다 — 한 장이 누적 버퍼·`toByteArray()`·Base64로 **약 3.3배**를 쓰기 때문이다. 초과·OOM은 그 장만 건너뛰고 `reason`을 남긴다.
- 웹 소비 경로는 `data`가 빈 장을 **파이프라인에 넣지 않는다**. 그대로 통과시키면 0바이트 사진이 들어가 「사진은 있는데 열리지 않는」 새 조용한 실패가 된다.

### 로그아웃 상태 동작 잠금 — 내보내기는 열고 복원·영구삭제는 잠금 (T-020, 2026-08-12) · v2.18
- `applySignedOutLock`이 가리는 것은 `viewBar`·`fold`·`form`·`filterNow`·`list` 다섯뿐이고 **헤더는 그 목록에 없어서**, 로그아웃 상태에도 「데이터 관리」가 열렸다. 거기서 백업·복원·영구삭제가 서버 없이 로컬만으로 완결되므로 M-0102가 화면에서 가린 자료가 백업 파일로는 나갔다.
- 사용자 결정(ADR-0063)은 3안 중 2안이다 — 내보내기는 읽기만 하고 로그인이 안 될 때 기억을 꺼낼 마지막 문이라 열어 두고, 덮어쓰는 복원과 되돌릴 수 없는 영구삭제·클라우드 교체는 로그인 뒤에만 연다.
- 🔴 근본형을 함께 고쳤다. `domain/authGate.ts`는 **판정**의 SSOT였지만 그 판정을 어느 표면에 거는지는 손 배선이라 화면 12개 중 2개만 묻고 있었다. `HubCard.needs`를 **필수 필드**로 세워 새 카드가 분류를 빠뜨리면 컴파일이 실패하게 했고(`TS2741` 주입 확인), 동작 분류는 기본값 없는 `Record<LocalDataAction, boolean>`이다.
- `authGateMismatches()`가 두 표를 함께 돌아 기존 진단(`sessionState.ts`)이 손대지 않고도 새 규칙을 잰다. `currentUser()`가 답하기 전에는 **잠김이 기본값**이다(모름을 열림으로 반올림하면 그 사이가 우회로다).
- 새 게이트 `verify-authgate-live` — 잠금은 빌드 시 박히는 값에 달려 있어 `dist`와 기존 라이브 픽스처가 **원리적으로 닿지 못하는 갈래**였다. 픽스처를 가짜 Supabase 환경변수로 빌드해 로그아웃+클라우드 설정 상태를 만들고 실제 렌더러로 그려 12/12. 잠긴 카드는 눌러도 상세가 안 열리고 열린 카드는 열린다. 전제(클라우드 설정·카드 존재)가 무너지면 재지 않고 실패한다.

### 사진·영상 뷰어에서 현재 앱 보관본 개별 저장 (2026-08-11)
- 사진 전체보기와 영상 재생기에 공용 「저장」 동작을 추가했다. 클릭 순간의 Blob·이름·MIME을 먼저 고정해 사진을 넘기거나 뷰어 URL이 폐기돼도 다른 항목을 저장하지 않는다.
- 사진은 EXIF가 제거된 표시용 WebP, 영상은 앱의 감상용 파생본이다. 외부 원본과 구별하도록 접근성 이름과 완료·요청·실패 문장에 「앱 보관본」을 명시한다.
- 백업 저장 경계를 일반 앱 파일 저장 코어로 확장하되 기존 백업 래퍼·형식·신선도 계약은 유지했다. 0바이트, MIME·확장자 불일치, 영상 행 MIME·Blob MIME 불일치, native/browser read-back 변조를 fail-closed로 거절한다.
- Android `BackupFiles` wire 이름은 구형 APK 호환을 위해 유지하지만 사진·영상의 실제 요청 파일명과 MIME을 사용한다. 네이티브 표시명 조회 실패도 고정 ZIP명이 아니라 요청명을 돌려주며, 그래서 Pages와 `apk-latest`를 함께 배포한다.

### 라이브 게이트의 고정 대기를 실측해 38곳을 내용 대기로 (T-016, 2026-08-09)
- BGT-2의 재는 순서대로, 게이트를 고치지 않고 playwright를 감싸 성분별로 쟀다: `verify-editor-live` 전체 128.6s 중 고정 대기 137.3s/351회 — 사실상 자고 있는 검사였다. 원문 이식 지시서의 답(폰트 격리)은 우리 병목이 아니었다.
- 근본형은 §7이다 — M-0119가 "시간이 아니라 내용을 기다림"을 이미 적었는데 한 곳만 고쳐졌고 형제 166곳이 그대로 잤다.
- 클릭 직후 300ms 이하 38곳을 `settle()`(rAF×2 — 값을 시계가 아니라 브라우저의 렌더 파이프라인이 정한다)로 바꿔 128.6s→121.3s, 390/390 PASS 그대로. 400ms 이상은 진짜 비동기일 수 있어 손대지 않았다(T-018).
- 재발 방지는 문서가 아니라 게이트다: `check-live-sleep`이 라이브 게이트의 고정 대기 수를 harness 원장에서 파생한 모집단으로 래칫(한 방향)한다.
- 조사에 쓴 계측기(`scripts/measure-live-time.mjs`)를 T-018이 다시 쓸 수 있게 저장소에 남겼다.

### CI 문제의 구조적 차단 — 헌법 §18-G·§18-H + 릴리스법 통합본 (2026-08-09)
- **§18-G 빈 세계 시연**: 게이트가 자기가 사는 세계(`origin/main`·`dist`)를 가정하다 CI에서만 죽는 결함군을 막는다. `GATE_WORLD` 전수 선언(빈 문자열도 선언이라 누락은 RED) + `check-env-assumption`이 얕은 체크아웃과 같은 모양의 임시 사본에서 싼 게이트를 전부 병렬로 돌린다. 매 실행 카나리아(`rev-parse origin/main`)로 「세계가 실제로 벗겨졌는지」를 먼저 증명한다. 종료코드 계약을 못박았다 — **전제 미충족은 위반이 아니다**(exit 2 또는 「확인 불가」).
- 그 시연이 곧바로 `check-gate-integrity`의 순서 결함을 잡았다: 「없는 파일」을 말하는 정직한 절이 있었는데 그 **앞**에서 `readFileSync`가 스택째 터져 거기까지 못 갔다.
- **§18-H 릴리스 묶음 동결**: `release-lock.mjs`가 묶음 경계를 파일로 만들고 `hook-release-freeze.mjs`(PreToolUse)가 얼어 있는 동안 `commit`·`push`·`merge`·`rebase`를 막는다. 재개방은 **금지가 아니라 기록**(이유 10자 이상 → 열리고 세어진다). 낡은 잠금은 3시간 뒤 스스로 풀린다. 실사격으로 런타임 발화까지 확인.
- **릴리스법 통합본**(`docs/HARNESS_RELEASE_LAW.md`): 헌법에 네 곳으로 흩어진 릴리스 규율(§18·§15·완료의 정의·검증 명령)을 한 문서로 **생성**한다. 손으로 옮겨 적지 않는다 — `check-adapter-parity`가 「커밋본 == 재생성본」을 강제하고, 생성물 손편집 주입으로 RED를 확인했다.
- 부수: `recordValues` 파서가 주석 달린 항목의 값을 다음 항목까지 삼키던 것 · `check-hooks-wired`의 정상 픽스처를 손목록에서 원장 파생으로(§11 ②) · 빈 세계 게이트 실행을 직렬→병렬로(§18-C를 심는 커밋에서 어기고 있었다).
