...
## ASM-1 가정 원장 — 확정 전의 보수적 기본값

> **축: 가정 하나 = 행 하나**(ID 오름차순). 확정되면 `docs/DECISIONS.md`로 승격하고 여기서는 상태만 갱신한다.
> **근거: 손목록** — 가정은 사람이 판단해 적는다. 파생할 원장이 없다.
...
## ASM-2 열린 질문 — 사용자 확인이 필요한 것

> **축: 질문 하나 = 항목 하나**(Q번호). 닫히면 취소선과 함께 **닫은 방법**을 같은 자리에 적는다.
> **근거: 손목록** — 질문은 대화에서 나온다. 2026-08-03 교차검증으로 현행화했다.
...
> **축: 원리적 한계** — 적히지 않은 가정 · 상태의 신선도 · 결번의 뜻.
> **근거: 손목록** — 사각지대는 정의상 파생할 수 없다.

### ASM-9.1 적히지 않은 가정은 이 문서가 모른다

이 원장은 **적은 것만** 안다. 말 없이 코드에 들어간 기본값은 여기 없고, 그래서 나중에 「왜 이렇게 했지」의 답이 없다.

### ASM-9.2 상태는 적힌 시점의 것이다

`잠정`이 아직 잠정인지, 그 사이 사실상 확정됐는지는 이 표가 스스로 모른다. 상태를 신뢰하려면 그 항목의 근거를 다시 읽어야 한다.

### ASM-9.3 결번이 없다는 것이 완전하다는 뜻은 아니다

트리로 세운 뒤 `A-001~A-015`·`Q1~Q7` 모두 결번이 없었다. 이 문서는 **원래 표였으므로 형제가 이미 나란히 서 있었다** — 트리 전환이 새로 드러낸 빈칸은 없다. 다만 `Q7`이 `Q6`보다 앞에 적혀 있어 읽는 순서와 번호가 어긋난다(내용에는 영향 없음).
