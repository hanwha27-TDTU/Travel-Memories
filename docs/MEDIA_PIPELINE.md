...
### 앱용 영상 기준 (ADR-0055 · v2.11)

영상도 외부 원본 파일을 수정·삭제하지 않고 앱용 파생본만 저장한다. 입력은 250MiB·60초 이하,
출력은 긴 변 1280px·30fps·25MiB 이하이며 MP4(AVC/AAC)를 우선하고 브라우저 지원에 따라
WebM(VP9/Opus)으로 대체한다. 변환은 Mediabunny+WebCodecs로 기기 안에서 수행한다.

`localVideos`에는 재생본·WebP 포스터·메타·동기화 op이 원자 커밋된다. R2 업로드 뒤 서버 행의
operation/version/path와 R2 GET 바이트가 모두 일치해야 임시 `sourceBlob`과 queue를 정리한다.
지원하지 않는 브라우저, 손상 파일, 상한 초과는 원본 업로드로 우회하지 않고 명시적으로 거부한다.
