...
🔴 **없다. 「있다」고 읽지 마라**(2026-08-15 전수조사). 이 절은 `scripts/build-single-html.ts`로
휴대용 단일 HTML판을 만든다고 적고 있었으나 **그 스크립트도, 대응하는 npm 스크립트도 저장소에 없다.**
배포 계약이 없는 산출물을 설명하면 다음 사람은 그것을 만들 수 있다고 믿고 계획에 넣는다.

〔목표〕 휴대용 단일 HTML판(로컬 아카이브 열람 중심의 제한된 보조판 — `file://`은 SW·설치형 PWA와 동등하지 않음). 배포 우선순위: ① 운영 PWA(여러 파일, GitHub Pages 주) → ② 헤더 호스트 미러(옵션) → ③ 단일 HTML 보조.
