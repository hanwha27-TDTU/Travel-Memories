...
## HANDOFF-0234 · 🔴 **세션 종료 — v2.47 배포 완료 · 다음 작업자가 여기부터 읽는다** (2026-08-16)

> **이 항목은 맥락 없는 다음 작업자(Claude·Codex·그 밖 무관)를 위한 것이다.** 이전 대화를 전혀
> 안 읽어도 이 항목 + `docs/BACKLOG.md` + `docs/HANDOFF_CODEX.md` + `docs/CONSTITUTION.md`만으로
> 이어받을 수 있게 쓴다.

### 1. 지금 상태 (실제 Git에서 확인한 값 — 문서를 믿지 말고 다시 재도 된다)

| 무엇 | 값 |
|---|---|
| 기본·배포 브랜치 | **`main`** (브랜치 보호 켜짐 · Required 2개가 실제로 병합을 막는다) |
| 배포된 버전 | **v2.47** (`66c142c` · PR #293 squash 머지) |
| 작업 브랜치 | `claude/constitution-rules-compliance-4m7gp0` — **`origin/main`에서 다시 세웠다**(머지 전 트리 동일 확인) |
| 커밋 안 된 것 · 숨김 · 미푸시 | **전부 0** |
| 잔재 원격 브랜치 | **없음**(머지 시 자동 삭제 · 낡은 로컬 참조는 prune) |
| merge/rebase 중단 | **없음** |
| 재개방 횟수 | **0회** (§18-H) |
| DB·migration | **이번 세션 변경 0건** — `supabase/migrations/`를 건드리지 않았다 |

### 2. 이번 세션이 한 일 — 한 줄 요약

🔴 **사용자 화면은 안 바뀌었다.** 바꾼 것은 **앱을 점검하는 층**이고, CHANGELOG도 §20에 따라
*"이번엔 화면이 바뀌지 않았어요"*로 적었다. 커밋 11개(v2.47 한 묶음).

| # | 무엇 | 기록 |
|---|---|---|
| ① | **§18-J 신설**(사용자 지시) — 검사가 답을 알면 **고쳐 놓고 보고**한다. 경계: **재계산은 고쳐도 되고 설계 결정은 보고만** | ADR-0063 · §18-J |
| ② | **§18-J ③층** — 재생성 드리프트 게이트 넷에 자동 고침. 🔴 **CI에서는 끈다** | M-0183 · HANDOFF-0233 |
| ③ | **「안 잰 것」을 「위반」으로 보고하던 지도 5건**을 SKIP으로 가름 | M-0180 · HANDOFF-0229 |
| ④ | **겹친 오버레이**에서 자가 밑에 깔린 화면을 재고 있었다 | M-0179 · HANDOFF-0228 |
| ⑤ | **하네스 실패 판정문**이 성공보다 말이 적었다 | M-0181 · HANDOFF-0230 |
| ⑥ | **로그인 헤더**를 처음 쟀다(픽스처가 CSS를 안 부르고 있었다) | M-0182 · HANDOFF-0231 |
| ⑦ | **`check-reduced-motion-scope` 신설** — `no-preference` 블록은 덫이다 | HANDOFF-0227 |
| ⑧ | **브리핑 ⑧**이 「릴리스 대기」를 🔴 미완성이라 부르던 오탐 | M-0178 · HANDOFF-0226 |
| ⑨ | **T-051·T-052 종결**(글꼴 배율 축 · 움직임 줄이기 축) | M-0176·M-0177 |

### 3. 검증 결과 (전부 실측 · 통과/스킵/미실행을 가른다)

| 층 | 로컬 | CI |
|---|---|---|
| `npm run build` | exit 0 · `dist/version.json = 2.47` | success |
| `npm run harness` | **exit 0 — 모든 게이트 통과(건너뛴 것 없음)** | **success** |
| `verify-editor-live` | **517/517 PASS** + 🔴 **SKIP 5**(지도 키 없음 · 이름과 함께 보고) | success |
| `verify-diagnostics-live` | **144/144 PASS** | success |
| `verify-authgate-live` | **22/22 PASS** (18 → 22) | success |
| CodeQL 5종 | — | success |
| Pages `build`·`deploy` | — | **success** |

**§4 주입증명**: 이번 묶음에서 **12종**을 주입해 RED 확인 후 복원했다.

### 4. 🔴 다음 작업자가 **먼저 알아야 할 것 넷** (모르면 시간을 버린다)

1. 🔴 **로컬에서 지도 검사가 빨가면 그건 진짜다.** 예전 인계서들이 *"로컬 빨간불 5건은 정상"*
   이라고 적어 뒀는데 **더 이상 아니다** — 이제 전제(키)가 없으면 **SKIP(확인 불가)**으로 나온다.
   키는 **저장소에 있다**(Repository Variable · `ci.yml` 2곳·`deploy-pages.yml` 1곳이 주입).
   없는 것은 **샌드박스**다. 정부지도(`VITE_JUSO_MAP_KEY`)만 아직 승인 대기다(T-038).
2. 🔴 **`npm run gates`가 이제 파일을 고칠 수 있다**(§18-J). 재생성 드리프트 게이트 넷이
   빨개지면 `npm run gen`을 돌리고 **다시 쓰인 파일을 이름으로** 보고한다. **커밋은 안 한다** —
   보고에 딸려 나오는 `git checkout -- …`가 되돌리기다. **CI와 `--no-fix`에서는 꺼진다.**
3. 🔴 **픽스처는 「진짜 렌더러를 쓴다」고 해서 「진짜 화면」이 아니다.** 진입점이 `main.ts`가
   아니면 **CSS·폰트·전역 초기화가 통째로 빠진다** — M-0182에서 실제로 데였고, 그 무스타일
   화면이 「44px 미달」 허상 3건을 냈다.
4. 🔴 **「배포 그린 확인」과 「사이트에서 확인함」은 다른 말이다.** 이 샌드박스는 `*.github.io`를
   막는다(실측 `curl` exit 56). 사이트가 실제로 v2.47을 내주는지는 **사용자 실기기 몫**이다.

### 5. 열린 과제 — **전부 막혔거나 「안 해도 결함이 아님」이다**

| ID | 상태 | 무엇을 기다리나 |
|---|---|---|
| T-036 | 막힘-실기기 | 우즈베키스탄 Android 실기기에서 TomTom 귀속·타일·팬·핀치 |
| T-037 | 막힘-상류 | Tauri가 `glib >= 0.20`을 쓸 수 있는 판(2026-08-15 확인: 여전히 2.11.5) |
| T-038 | 막힘-사용자 | 행정안전부 「지도제공 검색 API」 승인 + `VITE_JUSO_MAP_KEY` 등록 |
| T-028 | 대기(안 해도 결함 아님) | 릴리스 시각의 **지역 시간대 정본**을 어디 둘지 — 설계 결정 |
| **T-053** | 대기(신설) | §18-J 자동 고침을 넷 **밖으로** 넓힐지 — 먼저 「재계산인가」로 전 게이트 분류 |
| **T-054** | 대기(신설) | `verify-editor-live`가 여전히 **로그인 안 된 앱**만 잰다 |
| **T-055** | 대기(신설) | 「조항 옆의 계측기」 일반형 — 대응표 **정본을 어디 둘지**가 실제 내용 |

🔴 **그러므로 새 일을 만들지 말고 사용자에게 물어라.** 억지로 과제를 만들면 §15가 말하는
축적의 반대가 된다. **T-053·T-054·T-055는 셋 다 「기준을 먼저 정하는」 과제**이고, 기준 없이
기계부터 만들면 초록의 뜻이 「지켰다」가 아니라 **「안 봤다」**가 된다.

### 6. 사용자 실기기 확인이 밀려 있는 것 (내가 원리적으로 못 재는 것)

1. **v2.47이 실제로 사이트에 떴는가**(위 4-④).
2. **`.chip-person` 34px 오터치** — 넓혔더니 이웃 터치를 훔쳐 되돌리고 예외 등록(M-0173).
3. **글꼴 배율 시뮬레이션이 실기기와 같은가** — rem 파생으로 흉내 냈다(M-0176).
4. **헤드리스 `emulateMedia`가 실기기 OS 「움직임 줄이기」와 같은가**(M-0177).

### 7. 다음 세션의 첫 번째 작업

**아무것도 정해져 있지 않다.** 열린 과제가 전부 막혔거나 유보이므로, **`npm run brief`를 돌려
상태를 스스로 확인한 뒤 사용자에게 무엇을 할지 물어라.** 그것이 §16 ④이 요구하는 것이다.

---

## HANDOFF-0233 · **§18-J ③층 신설 — 재생성 드리프트 게이트 넷이 스스로 고치고 보고한다** (2026-08-16)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **사용자 지시**: *"재생성 드리프트 게이트 넷에 자동 고침 붙여줘"* → 붙였다. §18-J가 어제(같은 날) 정한 **첫 적용 자리** 그대로다.
- **대상 넷**: `check-adapter-parity` · `check-registry-gen` · `check-module-design-docs` · `check-doc-counts`. 넷 다 보는 것이 「커밋본 != 재생성본」 하나뿐이라 고침이 **재계산**이다(§18-J 경계표의 「고쳐도 되는」 칸).
- **구현은 `scripts/harness.mjs` 한 곳**(§7 2층) — 넷이 각자 고치면 규율이 네 벌이 되고 그중 하나가 조용히 갈라진다. `npm run gen`은 **한 번만** 돌고(넷이 같은 생성기를 공유한다), 고친 뒤 **다시 판정한다**(§8 — 고쳤다고 말하지 말고 다시 읽는다).
- 🔴 **CI에서는 끈다**(`process.env.CI`). CI는 **그 커밋**을 판정하는 자리인데(§15) 거기서 작업트리를 고쳐 초록을 만들면 **초록이 가리키는 트리와 머지되는 커밋이 달라진다.** 덤으로 §18-J의 *"§4 주입증명은 자동 고침을 끈 상태에서"*를 **자동으로 만족**시킨다. 로컬 재현은 `--no-fix`.
- **보고 계약 셋을 코드로 옮겼다**: 고쳐진 게이트 이름 · **다시 쓰인 파일 이름** · **그대로 붙는 되돌리기 명령** · 🔴 *"작업트리만 고쳐졌고 커밋되지 않았습니다"*.
- 🔴 **첫 구현이 보고 계약을 두 군데에서 어겼고, 내가 심은 경고가 둘 다 잡았다**(M-0183):
  - ①한글 경로가 이스케이프돼 **되돌리기 명령이 안 돌았다** → `core.quotepath=false`.
  - ②「다시 쓰인 파일」을 「**새로** 수정 상태가 된 파일」로 근사해, **이미 수정 상태인 생성물이 다시 쓰이는 가장 흔한 경우를 놓쳤다** → **내용 지문**(`git hash-object`) 비교로 교체.
- **§4 주입 4종 전부 RED**: `CI=true`/`--no-fix`에서 완전히 꺼짐(§18-J 블록 0건 · 드리프트는 빨간불로 남음) · 유령 게이트 이름 → *"등록부가 유령을 가리킵니다"* · 이유 빈 등록 → *"이유가 비었습니다(§7)"*. 뒤 둘은 **게이트를 돌리기 전에** 멈춘다.
- **실측 판정문**(드리프트를 실제로 만들어 확인):
  ```
  · §18-J 자동 고침: 1개 게이트가 재생성으로 초록이 됐습니다(원래 초록과 다른 칸입니다).
     · 고쳐진 게이트: check-module-design-docs
     · 다시 쓰인 파일 2개: docs/모듈별 설계서/18-…md, docs/모듈별 설계서/README.md
     · 미동의면 되돌리세요: git checkout -- "docs/모듈별 설계서/18-…md" "docs/모듈별 설계서/README.md"
     🔴 작업트리만 고쳐졌고 커밋되지 않았습니다 — 커밋해야 이 초록이 그 커밋의 것이 됩니다(§15).
  ```
- **검증**: `npm run gates` exit 0 · 주입 4종 전부 RED 후 복원 · 조항 현황이 `CLAUDE.md`·`AGENTS.md`·`HARNESS_RELEASE_LAW.md` 세 곳에 다시 심김.
- 🔴 **정직한 한계 넷**(조항에 적었다): ①**커밋은 안 한다**(기계가 커밋하면 되돌리기가 더 비싸진다) ②되돌리기 명령의 정확성은 이제 경로 인용에 달려 있다 ③「다시 쓰인 파일」은 **git이 보는 것**까지다 ④넷 밖으로 넓힐 때는 *"고침이 재계산인가 설계 결정인가"*로 가른다 — 느낌으로 넓히지 않는다.

---

## HANDOFF-0232 · **사용자가 검사의 기본값을 바꿨다 — 「고쳐 놓고 보고」로** (2026-08-16 · §18-J 신설)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **사용자 제안**: *"자체 리뷰어가 직접 결함을 찾으면 … 먼저 리뷰어가 직접 고치면 되는거 아냐? … 동의하면 다음 단계 넘어가고 미동의하면 되돌리면 되잖아??"* → **동의했고 §18-J로 박았다.**
- **근거(실측)**: 2026-08-16 한 세션에서만 **두 번**, 게이트가 빨개진 이유가 오직 「생성물이 낡음」이었고 고침은 언제나 `npm run gen` 한 줄이었다. **기계가 아는 답을 사람에게 받아쓰게 한 것**이고, §12를 게이트에게 묻지 않았을 뿐이다.
- 🔴 **경계가 조항의 본론이다**: 고쳐도 되는 것은 **재계산**(정본에서 다시 생성·형식·카운트), 보고만 하는 것은 **설계 결정**(CSS·문구·자료구조). 근거는 **같은 세션의 M-0179·M-0182** — 두 번 다 게이트가 **확신에 차서 틀렸고**, 자동 고침이었다면 실제 회귀를 만들었을 것이다.
- **절대 대상 밖**: 사용자 자료·migration·RLS·삭제 경로·비밀키(§0) · 얼린 묶음(§18-H).
- **보고 계약 셋**: 이름으로 나열(「N건」은 정보가 없다) · **따로 커밋**(한 덩어리로 되돌리게) · 다음 단계 전에 사람이 읽음.
- 🔴 **새 위험**: 자동 고침 게이트는 **빨간불을 낼 수 없어** 검출력이 관측 불가가 된다. **고친 내역이 곧 빨간불**이고, §4 주입증명은 **자동 고침을 끈 상태**에서 한다 — 안 그러면 주입한 결함을 게이트가 지워 초록이 나오고, 그 뜻은 「잡았다」가 아니라 **「지웠다」**이다.
- **어디에 심겼나**: 정본 `docs/CONSTITUTION.md` §18-J → `npm run gen`이 **`CLAUDE.md`·`AGENTS.md`·`docs/HARNESS_RELEASE_LAW.md`** 세 곳에 글자 단위로 같게 심었다(확인: 세 파일 모두 `§18-J` 1건). 사용자 요청 *"모든 인공지능이 알게"*의 실행 형태다.
- **검증**: `npm run gen` exit 0 · `npm run gates` exit 0(재본 71개 통과) · `check-adapter-parity` PASS(커밋본 == 재생성본).
- 🔴 **정직한 한계 — 아직 조항뿐이다.** 저장소에 **자동 고침 코드는 0건**이다(실측: `--fix`·autofix 0건). ②층은 이미 절반 있다(`npm run gen`이 파생물을 원본에서 다시 만든다 — 자동 고침은 그것을 **게이트 실패 지점에서 부르는 것**이지 새 능력이 아니다). ③층은 **없다.**
- **다음 작업자에게 — 첫 적용 자리는 정해져 있다**: 재생성 드리프트 게이트 넷(`check-adapter-parity`·`check-registry-gen`·`check-module-design-docs`·`check-doc-counts`)이 빨개지면 `npm run gen`을 부르고 **무엇이 다시 쓰였는지 이름으로** 보고하는 것. 그 밖으로 넓히기 전에 §18-J의 경계표를 다시 읽어라.
- **기록**: ADR-0063 · §18-J.

---

## HANDOFF-0231 · **화면을 눈으로 보다가, 사용자가 실제로 보는 헤더를 아무도 안 재고 있었다는 걸 알았다** (2026-08-16)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **어떻게 시작했나**: 자동 축은 넘침·잘림만 본다. §10이 말한 *"이 화면이 사람에게 이해되는가"*는 기계가 못 보므로 **홈을 344px·글꼴 200%로 그려서 눈으로 봤다**(§13 — 내가 볼 수 있는 것을 사람에게 묻지 않는다).
- **먼저 본 작은 것**: `📴 로컬 저장 모드 · 대기 0건` 배지 **바로 아래** `📴 로컬 모드`가 또 있었다(같은 아이콘·같은 말·항상 함께). 🔴 **다만 운영에는 Supabase가 설정돼 있어 사용자는 못 본다**(워크플로 실측) — dev 전용이라 결함으로 올리지 않았다.
- 🔴 **그 확인이 진짜 공백을 가리켰다.** `verify-editor-live`(517건)는 `dist`를 재는데 그 빌드에 Supabase 값이 없어 `isConfigured()`가 **거짓**이다. 즉 큰 라이브가 내내 그린 헤더는 **`📴 로컬 모드` 한 줄**이었고, 사용자가 보는 **이메일 + [로그아웃]** 헤더는 **폭·배율·터치 표적 어느 축에서도 한 번도 안 쟀다.** 🔴 그 자리는 **사용자가 직접 지적한 적도 있다**(*"이메일 배치를 이쁘게 안 될까?"*).
- **수정**: `isConfigured()===true`로 빌드하는 **유일한** 게이트(`verify-authgate-live`)에 헤더 측정을 넣었다. 서버가 있어야 만들 수 있는 상태이므로 §13 1항의 길 — **값을 먹여 실제 렌더러로 그린다**(`renderHomeAuth`를 export · 검사 전용 우회로가 아니라 **그 함수 자체**를 연다). 긴 이메일로 잰다(짧은 값이면 축이 저절로 쉬워진다).
- 🔴 **첫 실행이 결함 3건을 냈는데 전부 허상이었다** — M-0179가 **하루도 안 돼 재발**했다. `24px`는 **스타일 없는 브라우저 기본 버튼** 높이였다: 앱 CSS는 `main.ts`가 부르는데 **픽스처는 자기 진입점을 쓰므로 아무 스타일도 안 불렀다.**
  - **내 검사만의 문제가 아니었다**: 이 픽스처의 **기존 넘침 검사(320·375px)도 내내 무스타일 페이지를 재고 있었다.** 픽스처가 이제 `tokens.css`·`app.css`를 먼저 부른다.
- **§4 — 무스타일 실행이 곧 주입이었다**: 스타일 없이 **3건 RED**(44px 미달 2 · 200% 넘침 1) → 스타일 부른 뒤 **8건 전부 통과.** 축에 검출력이 있고, **실제 헤더는 세 배율·터치 표적 전부 정상**이다.
- **검증**: `verify-authgate-live` **22/22 PASS**(18 → 22) · `tsc --noEmit` exit 0 · `npm run gates` exit 0(재본 71개 통과).
- **교훈**: 🔴 **「그 상태를 그릴 수 있는 곳」과 「그 상태를 재는 곳」이 다르면 그 상태는 안 재진다.** 설정 상태를 만들 수 있는 곳은 처음부터 authgate뿐이었는데 측정 축(`coverSweep`)은 **다른 스크립트에만** 있었다 — 두 조각이 만난 적이 없다. 그리고 🔴 **픽스처는 「진짜 렌더러를 쓴다」고 해서 「진짜 화면」이 아니다.** 진입점이 다르면 CSS·폰트·전역 초기화가 통째로 빠진다. 픽스처를 만들면 *"실제 앱이 부르는 것 중 내가 안 부른 것은 무엇인가"*를 먼저 물어라.

---

## HANDOFF-0230 · **하네스가 실패할 때 성공할 때보다 말이 적었다 — §21 판단이 그 자리에서 내려지는데** (2026-08-16)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **어떻게 나왔나**: M-0180의 교훈(*"늘 같은 상태인 것을 의심하라"*)으로 형제를 쓸었다. 전체 하네스에 상시 SKIP은 **0건**이었는데, 대신 **판정문 자체**에 구멍 둘이 있었다.
- **구멍 ①**: 실패하면 `harness: 1 gate(s) FAILED` 한 줄을 찍고 **곧바로 `exit`**했다. 영어인 것도 §19 위반이지만 곁가지다 — **진짜 문제는 그 뒤 블록을 통째로 건너뛴 것**이다. 빨간 실행에서는 **무엇을 건너뛰었는지·무엇을 반쪽만 쟀는지를 아예 못 들었다.** 성공하면 그 셋을 다 말하면서.
- **구멍 ②**: 🔴 **빠른 차선에서는 「건너뜀」 보고가 도달 불가**였다(`if (FAST) … else if (skipped.length)` 사슬). FAST에서 전제 미충족으로 건너뛴 게이트는 판정문에서 **통째로 사라졌다.**
- 🔴 **왜 무거운가**: §21은 *"머지 단계에 들어갔으면 빨간 검사가 있어도 배포한다 — 단 데이터 유실은 예외"*라고 말하고, 그 판단은 **빨간 실행의 판정문을 읽고** 내린다. 판정문이 「무엇을 안 쟀는지」를 안 알려주면 **라이브 층이 통째로 건너뛴 줄도 모르고 강행**하게 된다.
- **수정**: ①실패를 **이름**으로 모으고 **조기 종료를 없애** 판정문을 끝까지 찍은 뒤 종료코드를 낸다. ②「건너뜀」을 사슬에서 빼 **차선과 무관한 독립 블록**으로. 「안 잰 것」(차선이 빼놓음)과 「건너뛴 것」(전제 없음)은 **다른 칸**이고 둘 다 통과가 아니다. ③실패 시 「통과」라고 쓰던 문구를 갈래로 조였다.
- **§4 주입 2종**: Ⓐ 실패 1 + 건너뜀 1 → 판정문이 **빨간 이름 6 · 안 잰 것 5 · 반쪽만 잰 것 17**을 전부 말했다(예전엔 한 줄). 🔴 **덤 — 등록 안 된 가짜 게이트가 등록부 게이트 5개를 빨갛게 만들었다**(그 다섯의 비공허 증명). Ⓑ 건너뜀만 주입 → **빠른 차선에서도** 이름·이유가 나왔다.
- 🔴 **부수 효과 — 층이 저절로 맞물렸다.** M-0180이 만든 라이브의 SKIP 요약 문장이 하네스의 한계 감지(`CAVEAT_RE`)에 걸려 `verify-editor-live`가 **「일부만 쟀다」 목록에 스스로 들어갔다**(19 → 21). 배선한 적이 없다 — **두 층이 같은 어휘를 쓰기 때문**이다(§7 2층의 좋은 형태).
- **검증**: `npm run gates` exit 0(재본 71개 통과) · 🔴 **`npm run harness` exit 0 — 전체 하네스가 로컬에서 처음으로 통과했다**(M-0180 전에는 지도 5건 때문에 늘 빨갰다).
- **교훈**: 🔴 **판정문은 실패할 때 더 많이 말해야 한다.** 성공은 다음 할 일이 없고 실패는 있다 — 이 저장소는 정확히 반대였다. 그리고 🔴 **보고를 `else if` 사슬에 넣으면 어떤 차선에서 도달 불가가 된다** — 보고는 **독립 블록**으로 두고 「그 값이 있느냐」로만 켠다.

---

## HANDOFF-0229 · **사용자가 한 줄을 물어서, 여러 세션 동안 복사돼 온 거짓 두 개가 나왔다** (2026-08-16)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **어떻게 나왔나**: 사용자가 물었다 — *"지도키 5개는 저장소에 있지 않아? 정부지도만 없을거고"*. 맞는 지적이었고, **내가 반복해 쓰던 요약 한 줄**(*"남은 5는 지도 키 결손 — T-036·T-038"*)에 **거짓이 둘** 들어 있었다.
- **틀린 것 ①**: 그 5건은 **Kakao 3 + TomTom 2**이고 **정부지도(T-038)는 아예 없다.**
- **틀린 것 ②**: 키는 **저장소에 있다** — `ci.yml` 2곳·`deploy-pages.yml` 1곳이 Repository Variable로 주입한다(실측). 없는 것은 **이 샌드박스**이지 저장소가 아니다.
- 🔴 **그 확인이 더 큰 것을 드러냈다 — 그 5건은 결함이 아닌데 `FAIL`로 보고되고 있었다.** §18-G 1항(*"전제가 없으면 판정하지 말고 **말한다**"*)과 §8(*"모르는 것은 정상으로도 **문제로도** 반올림하지 않는다"*)을 정면으로 어기고 있었다.
- 🔴 **값은 이미 치르고 있었다.** 늘 빨간 5건은 사람이 곧 무시하고(§11 ③), 그러면 **진짜** 지도 결함이 나도 「늘 그 5개」로 넘어간다. 실제로 나는 여러 세션 동안 그 문장을 **읽지도 않고** 적었다. **M-0178과 같은 근본형이고, 그걸 고친 바로 다음 세션에 같은 형태를 지나치고 있었다.**
- **수정**: `skip(name, why)` — 세 번째 상태. 전제가 없으면 판정하지 않고 **「확인 불가」와 이유**를 적는다. 총계는 `517/517 PASS` 뒤에 **🔴 5개는 전제가 없어 아예 재지 않았습니다**를 **이름과 함께** 붙인다(§2-G · `npm run gates`가 하는 그대로).
- **§4 주입**: 가짜 키를 환경에 넣자 5건이 **SKIP이 아니라 실행**됐다(가짜라 FAIL) — **전제에 달린 건너뜀**이지 상시 면제가 아니다.
- 🔴 **정직한 한계 — 전제 판정은 대리 지표다**(§18-A). `import.meta.env`는 빌드 시 치환돼 사라지므로 「번들이 키를 품었는가」를 실행 중에 되물을 수 없고, `process.env`를 읽는다(빌드와 같은 셸이라는 전제). **오차는 안전한 방향으로 눕는다**: 환경엔 있고 번들엔 없으면 조용히 건너뛰지 않고 **시끄럽게 FAIL**한다(위 주입이 실증). 위험한 방향은 좁지만 **0은 아니다.** `version.json`에 심는 길도 있으나 그건 `appUpdate`가 읽는 **배포 계약**이라 오염시키지 않았다.
- **형제 확인(§7)**: `verify-diagnostics-live`·`verify-authgate-live`에는 지도 전제가 **0건**(`grep` 전수) — 걸릴 다른 자리는 없다.
- **검증**: `verify-editor-live` **517/517 PASS + SKIP 5**(exit 0) · 가짜 키 주입 시 **517/522**(exit 1).
- 🔴 **다음 작업자에게 — 기억을 바꿔라.** 예전 인계서들이 *"로컬 빨간불 5건은 정상"*이라고 적어 뒀는데 **더 이상 아니다.** 이제 로컬에서 지도 검사가 **빨가면 그건 진짜**다. HANDOFF-0225에 정정을 심어 뒀다.
- **교훈**: 🔴 **「늘 빨간 것」을 발견하면 그것부터 조사하라** — 그건 대개 검사의 결함이고, 그 자리는 **이미 죽어 있다.** 그리고 🔴 **내가 반복해서 쓰는 요약 문구를 의심하라.** 검증 없이 복사된 한 줄 안에 틀린 귀속과 틀린 판정 상태가 함께 숨어 있었다.

---

## HANDOFF-0228 · **안 재던 화면 셋에 자를 댔더니, 그 자가 밑에 깔린 화면을 재고 있었다** (2026-08-16)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **어떻게 나왔나**: 폭·배율 축(`coverSweep`)이 **어느 화면을 덮는지** 셌다. 라이브가 **열기는 하는데 안 재는** 화면이 셋이었다 — 설계 개요도 · 기계화 검증 흐름도 · 🔴 **위치관리대장**(마지막은 **사용자 화면**이다: 데이터 관리 → 등록한 장소 관리).
  - 🔴 **곁가지로 내 의심 하나를 스스로 기각했다**: 처음엔 `@live-covers` 선언 넷이 **거짓**인 줄 알았다(`grep`에 안 잡혀서). 실제로는 스크립트가 **한글 라벨**로 열고 있었다(`설계 개요도 열기`·`기계화 검증 흐름도 열기`·`위치관리대장`). **선언은 정직했다** — 성급한 결론을 값으로 뒤집었다(§8).
- **세 곳에 자를 댔고 12건이 전부 초록이었다.** 🔴 **그런데 두 개발자 화면의 `controls`가 똑같이 5였다.** 서로 다른 화면이 같은 숫자를 내는 것은 우연이라기엔 이상하다 — **그 한 줄이 유일한 단서였다.**
- **원인**: `coverSweep`이 `document.querySelector(root)` — **첫 번째** 일치를 쓴다. 이 앱의 오버레이는 **겹친다.** 실측(임시 프로브): `개발자 정보만 → .guide-overlay 1개(버튼 5)` · `설계 개요도 연 뒤 → 2개(버튼 5 / 버튼 1)`. **보고된 `controls: 5`는 정확히 개발자 정보의 버튼 수였다** — 검사가 **가려진 화면**을 재고 있었다.
- 🔴 **그 초록의 뜻은 「괜찮다」가 아니라 「엉뚱한 걸 쟀다」였다.** 모집단 가드(`controls > 0`)는 **개수만** 보지 **무엇을** 셌는지는 안 본다.
- **수정(구조)**: `querySelectorAll`의 **마지막**(= DOM 순서상 맨 위)을 쓰고, **겹친 개수(`roots`)를 판정 증거에 싣는다** — 안 보이면 다음 사람이 같은 자리에서 또 속는다.
- **§4 — 전후 실측이 곧 주입증명이고, 전수로 오염 범위를 확정했다**:
  - 같은 화면이 **틀린 값 `controls: 5` → 옳은 값 `controls: 1`, `roots: 2`**로 바뀌었다.
  - 🔴 **기존 호출부를 전수로 다시 쟀다 — 편집기 11곳·진단 17곳 전부 `roots: 1`.** 즉 **기존 초록은 공허하지 않았다.** 결함은 내가 방금 추가한 두 줄에만 있었다. (전수로 안 쟀으면 「어디까지 썩었는지」를 몰랐다.)
- **결과**: `verify-editor-live` **517/522**(505 → 517 · 새 화면 3 × 4축 · 남은 5는 지도 키 결손) · `verify-diagnostics-live` **144/144**. 세 화면 모두 네 축·세 배율 전부 통과 — **결함 0건이지만 이제 그 말이 참이다.**
- **개발자 화면에도 같은 자를 줬다**(§7 — 대칭이 기본값, 차별이 예외). *"사용자 화면이 아니다"*는 안 잴 이유가 아니다: 둘 다 폴드 커버에서 열리고, 표와 71장짜리 카드 목록이라 **가로 넘침이 가장 나기 쉬운 모양**이다.
- 🔴 **정직한 한계**: 「마지막이 맨 위」는 **DOM 순서 = 쌓임 순서**를 전제한다. 명시적 `z-index`로 순서를 뒤집으면 이 전제는 깨진다 — 지금 이 앱에서는 참이지만(실측) **계약으로 강제되지는 않는다.** 그리고 `roots`는 **보고할 뿐 판정하지 않는다**: 셀렉터가 무관한 여러 요소에 맞아도 게이트는 조용하다.
- **교훈**: 🔴 **「모집단 0이 아니다」는 「올바른 모집단이다」가 아니다.** §4의 모집단 가드는 *안 본 것*을 막지 *엉뚱한 것을 본 것*은 못 막는다. 겹치는 표면에서는 **몇 개에 맞았는지를 증거에 남겨라.**

---

## HANDOFF-0227 · **T-052가 「안 봤다」고 적어 둔 자리를 재러 갔더니, 결함은 없고 덫이 있었다** (2026-08-16)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **어떻게 나왔나**: T-052의 「정직한 한계」가 *"`app.css`의 다른 `prefers-reduced-motion` 선언은 안 봤다"*였다. **적어 둔 한계를 실제로 재러 갔다** — 선언은 셋이고 라이브가 덮는 것은 둘이었다.
- **남은 하나는 결함이 아니었다**: `no-preference` 안의 `.trip-card` hover(떠오름+그림자). `cursor: pointer`와 `:focus-visible` 외곽선이 블록 **밖**에 있어 reduce에서도 살아 있다(실측). 🔴 **그러나 「안 쟀다」와 「괜찮다」는 다른 말이므로 재고 나서 그렇게 적는다.**
- 🔴 **재면서 진짜 위험이 보였다 — `@media (prefers-reduced-motion: no-preference)`는 덫이다.** 거기 넣은 선언은 reduce 사용자에게 **통째로 사라지는데**, 자료구조는 옳으므로 **유닛은 영원히 초록**이다(§10 ③의 CSS판). 지금은 장식만 들어 있지만 누가 정보(색·글자·표시 여부)를 넣으면 **아무도 못 잡는다.**
- **그래서 정적 층을 놓았다** — `check-reduced-motion-scope`(게이트 70 → 71):
  - 두 방향의 `prefers-reduced-motion` 블록 안에는 **움직임 속성만** 온다. 그 밖은 **이유와 함께 등록**해야 통과한다(`NON_MOTION_ALLOWED` — 이유 없는 제외는 결함 · §7).
  - **반대 방향도 같은 규칙**: `reduce`에서 끄는 것도 움직임이어야 한다. `display: none`은 배려가 아니라 **기능을 뺏는 것**이다(M-0177).
  - 🔴 **모집단이 0이면 실패로 닫는다** — 초록이 「위반 없음」이 아니라 **「안 봤음」**이 되기 때문이다(§4 · §2-J ①).
- **§4 주입 양방향(둘 다 실제 `app.css`에 넣고 복원)**: `reduce`에 `display:none` → *"기능·정보를 끄면 배려가 아니라 결함입니다(M-0177)"*로 RED · `no-preference`에 `border-color` → *"reduce 사용자에게 통째로 사라집니다"*로 RED. 자체검사 8건(양성 3·음성 4·예외 등록 1) 내장.
- **현재 등록된 예외 1건**: `.trip-card:hover { box-shadow }` — 이유는 위 실측 그대로 코드에 적혀 있다.
- **검증**: `gen` exit 0 · `gates` exit 0(**71개 통과** · 5개는 안 잼) · `check-env-assumption` exit 0(게이트 76개 세계 선언 완비 · 빈 세계에서 67개 실연 — 새 게이트 포함).
- 🔴 **정직한 한계**: **속성 이름**만 본다 — 「이 선언이 장식인가」는 못 본다. `transform`으로 정보를 나르면 이 게이트는 **조용하다.** 하는 일은 「장식만 들었다」의 보증이 아니라 **「움직임이 아닌 것이 조용히 섞이는 것의 차단」**이고, 판단은 예외 등록의 *이유*가 지며 그건 사람이 읽는다.
- **곁가지 — 개수 드리프트 전수 조사(§6 결함군 승격)**: M-0178의 근본형(「자동 옆의 수동」)이 **다른 곳에도 있는지** 문서·스킬의 손편집 개수 주장 30건을 뽑아 대조했다. **새 사례는 없었다** — 실제로 낡는 두 종류(게이트 개수 · 에이전트/역할 개수)는 `check-hand-counts`+`check-doc-counts`와 `<!--reg:-->` 마커+`check-registry-gen`이 **이미 고리를 닫고** 있고, 나머지는 전부 `<!--hist-->`로 얼린 **과거 사건 서술**(그때 초록이었다)이라 얼려 두는 것이 옳다. **음성 결과도 결과이므로 적어 둔다** — 다음 사람이 같은 조사를 다시 하지 않게.

---

## HANDOFF-0226 · **착수 절차를 돌렸더니, 내가 규율을 지킨 그 상태를 계측기가 🔴라고 불렀다** (2026-08-16)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **어떻게 나왔나**: v2.46 배포를 끝내고 §9 착수 절차(`npm run brief`)를 돌렸다. ⑧이 **🔴 미완성 1건**을 띄웠는데, 그건 §18-H 3항을 지켜 **배포 확인 뒤 따로 올린 문서 커밋**(HANDOFF-0225)이었다. 처방은 *"`chore(wip):`로 보존 → 재고 → 묻는다"* — **끝난 커밋을 미완성으로 되돌리라는 말이다.**
- 🔴 **원인은 헌법 안의 충돌이었다.** §16이 미완성으로 정의한 *"`origin/main`에 없는 브랜치 커밋"*은, §15가 2026-08-04에 축적을 기본값으로 만든 뒤로 **「구현됨·릴리스 대기」와 같은 바이트**다. 헌법은 그 상태에 **이미 이름을 갖고 있었는데** §16이 그것을 몰랐다 — 두 이름 · 두 처방 · 한 값(M-0060의 문서판).
- 🔴 **위험한 이유는 오탐이라서가 아니라 「평상시에 뜨는 빨간불」이라서다.** 축적이 기본값이므로 릴리스 대기는 **정상**이다. 정상에서 매번 뜨는 🔴는 사람이 무시하기 시작하고, 그때 그 검사는 죽는다(§11 ③) — **§16이 막으려던 사고가 §16의 계측기 때문에 난다.**
- **수정(세 층 · §7)**: ①헌법 §16에 두 상태를 가르는 표를 넣었다(정본만 고치고 `gen:adapters` — §14). ②가르는 표식은 **이미 있었다**: `chore(wip):`을 `.githooks/commit-msg`가 「미완성 · 머지 금지」와 함께 강제한다. ③브리핑 ⑧이 두 칸으로 갈라 **다른 처방**을 준다.
  - 🔴 **총계에서 릴리스 대기를 뺐다** — 안 빼면 위에서 가른 것이 총계에서 다시 합쳐져 🔴가 그대로 뜬다.
  - 🔴 **그래도 침묵하지 않는다**: 「사라질 위험」은 둘이 똑같으므로(squash 머지 뒤 브랜치 재생성) 릴리스 대기도 개수와 함께 *"함께 옮긴다"*를 적는다. **갈라진 것은 처방이지 경고가 아니다.**
- **§4 주입(양성·음성)**: Ⓐ 결함을 **먼저 재현** — 깨끗한 트리에서 옛 브리핑이 `머지 안 됨 1` → 🔴 + wip 처방. Ⓑ 진짜 `chore(wip):` 커밋을 주입하니 같은 트리에서 **`미완성 1` + `릴리스 대기 1`이 다른 칸에 다른 처방**으로 나왔다(옛 판이면 「머지 안 됨 2」 한 칸). 주입 커밋·파일 흔적 되돌림 확인.
- **검증**: `npm run gen` exit 0 · `npm run gates` exit 0(70개 통과 · 5개는 안 잼).
- 🔴 **정직한 한계**: 가르는 근거는 **커밋 제목의 접두사**뿐이다. 미완성인데 `chore(wip):`을 안 붙이고 커밋하면 여전히 「릴리스 대기」로 센다 — 기계가 보는 것은 **표식이지 상태가 아니다.** 그 자리를 메우는 것은 §16 ③(훅)과 사람의 정직함이고, 이 계측기가 하는 일은 *"미완성이 없다"의 보증*이 아니라 **「끝난 것을 미완성이라 부르는 오탐의 차단」**이다.
- 🔴 **곁가지 교훈 — 조항을 고치면 그 조항을 재는 계측기도 함께 낡는다.** §15의 2026-08-04 개정은 큰 변화였는데 그때 §16의 정의와 브리핑 ⑧은 **아무도 다시 안 봤다.** §18-I의 「자동 옆의 수동」의 사촌이다: **조항 옆의 계측기.** 상세는 M-0178.

---

## HANDOFF-0225 · 🔴 **세션 종료 — v2.46 배포 완료 · 다음 작업자가 여기부터 읽는다** (2026-08-16)

> **이 항목은 맥락 없는 다음 작업자(Claude·Codex 무관)를 위한 것이다.** 이전 대화를 전혀 안 읽어도
> 이 항목 + `docs/BACKLOG.md` + `docs/HANDOFF_CODEX.md`만으로 이어받을 수 있게 쓴다.

### 지금 상태 (실제 Git에서 확인한 값)

| 무엇 | 값 |
|---|---|
| 배포된 버전 | **v2.46** (`16001e7` · PR #292 squash 머지) |
| 작업 브랜치 | `claude/constitution-rules-compliance-4m7gp0` — **`origin/main`에서 다시 세웠다**(내용은 `main`에 전부 있다 · 트리 동일 확인) |
| 커밋 안 된 것 | **없음** (`git status --porcelain` 빈 출력) |
| 숨겨 둔 것 | **없음** (`git stash list` 빈 출력) |
| 푸시 안 된 커밋 | **없음** (`origin/main..HEAD` 빈 출력) |
| 남은 원격 브랜치 | **없음** (머지 시 자동 삭제 · 낡은 로컬 참조는 `git remote prune`으로 정리) |
| 재개방 횟수 | **0회** (§18-H — 얼린 뒤 커밋을 얹지 않았다) |

### 이번 릴리스가 한 일

**접근성 두 축을 신설했다.** 이 저장소는 화면 **폭**과 **방향**은 촘촘히 쟀는데, 사용자가 자기
기기에서 바꿔 두는 것 — **글꼴 배율**과 **움직임 줄이기** — 는 규칙도 검사도 0건이었다.

- **T-051(글꼴 배율)**: 기준을 근거에서 세웠다(`130%` 안드로이드 One UI 최대 · `200%` WCAG 1.4.4).
  **결함 5건** — `white-space: nowrap`이 접히지 못해 200%에서 화면 밖으로 밀려났다. `coverSweep`
  호출부 20곳이 자동으로 세 배율을 잰다. 상세는 HANDOFF-0223 · M-0176.
- **T-052(움직임 줄이기)**: **앱 결함은 0건**이었고, 잡은 것은 **내가 쓰려던 자**였다. 상세는
  HANDOFF-0224 · M-0177.

### CI·배포 (숫자로)

- PR #292: `harness` **success** · `live-render` **success** · CodeQL 5종 전부 success · `fast-gates` success(병합 근거 아님).
- Pages: `build` success · `deploy` success · 시크릿 검사 통과.
- 🔴 **로컬 하네스는 505/510이었고 CI는 통과했다.** 차이는 결함이 아니라 **환경**이다 — 이 샌드박스에는
  지도 키(`VITE_TOMTOM_API_KEY`·Kakao)가 없고 CI에는 Repository Variable로 있다. 로컬에서 그 5건이
  빨간 것은 **정상**이니 고치려 들지 마라(T-036·T-038).
  - 🔴 **2026-08-16 정정 두 가지**(사용자 지적): ①그 5건은 **Kakao 3 + TomTom 2**이고 **정부지도(T-038)는 아예 없다** — 위 `(T-036·T-038)` 귀속은 틀렸다. ②**더 이상 빨갛지 않다**: 전제(키)가 없으면 이제 **SKIP(확인 불가)**으로 나오고 총계가 이름과 함께 「안 쟀다」고 말한다(M-0180). 그러므로 **로컬 빨간불 5건을 「정상」으로 기억하지 마라** — 이제 로컬에서 지도 검사가 빨가면 그건 **진짜**다.
- 🔴 **「배포 그린 확인」과 「사이트에서 확인함」은 다른 말이다.** 이 샌드박스는 `*.github.io`를
  막아 운영 `version.json` 되읽기가 **불가능하다**(실측: `curl` exit 56). **사이트가 실제로 v2.46을
  내주는지는 사용자 실기기 몫이다.**

### 🔴 다음 작업자에게 — 지금 할 수 있는 일이 없다

`docs/BACKLOG.md`의 열린 과제 **4개가 전부 막혔거나 「안 해도 결함이 아님」**이다:

| ID | 상태 | 무엇을 기다리나 |
|---|---|---|
| T-036 | 막힘-실기기 | 우즈베키스탄 Android 실기기에서 TomTom 귀속·타일·팬·핀치 확인 |
| T-037 | 막힘-상류 | Tauri가 `glib >= 0.20`을 쓸 수 있는 판이 나오기까지(2026-08-15 확인: 여전히 2.11.5) |
| T-038 | 막힘-사용자 | 행정안전부 「지도제공 검색 API」 승인과 `VITE_JUSO_MAP_KEY` 등록 |
| T-028 | 대기(**안 해도 결함이 아님**) | 실제 내용은 「릴리스 시각의 지역 시간대 정본을 어디 둘 것인가」라는 **설계 결정**이다 |

**그러므로 새 일을 만들지 말고 사용자에게 물어라.** 억지로 과제를 만들면 §15가 말하는 축적의
반대가 된다.

### 사용자 실기기 확인이 밀려 있는 것들 (내가 못 재는 것)

1. **v2.46이 실제로 사이트에 떴는가** (위 참조 — 샌드박스가 막는다).
2. **`.chip-person` 34px 오터치** — 넓혔더니 이웃 터치를 훔쳐 되돌리고 예외 등록했다(M-0173). 실제로 잘못 눌리는지는 손가락이 판정한다.
3. **글꼴 배율 시뮬레이션이 실기기와 같은가** — rem 파생으로 흉내 냈다. 안드로이드 textZoom과 같은지 **확인하지 않았다**(M-0176).
4. **헤드리스 `emulateMedia`가 실기기 OS 「움직임 줄이기」와 같은가** — **확인하지 않았다**(M-0177).

---

## HANDOFF-0224 · **「움직임 줄이기」 축을 만들다가, 내가 쓰려던 자가 거꾸로인 것을 먼저 잡았다** (2026-08-16 · T-052 종결)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **결론부터 — 앱에는 결함이 없었다.** `prefersReducedMotion()`은 `liftScale`만 1로 만들고 **끌어서 재배열은 그대로 된다**(실측). 이 과제가 의심하던 「접근성 설정이 기능을 뺏는다」는 **일어나지 않고 있었다.**
- 🔴 **그런데 잡은 것이 있다 — 내가 쓰려던 검사였다.** 처음 자는 *「켜면 `animation`이 `none`이 된다」*였는데, **그 자는 이 축이 막으려는 결함을 정확히 통과시킨다**: 기능이 사라진 화면도 애니메이션은 `none`이다. **정상과 결함이 같은 값을 낸다** — §4가 말하는 공허함보다 나쁜, **거꾸로 선 자**다.
- **그래서 기준을 두 갈래로 갈랐다**: **장식**(확대·전환)은 사라지는 것이 옳고, 🔴 **정보**(「지금 들려 있다」·「지금 재생 중이다」)는 **다른 수단으로 여전히 보여야** 한다. 검사는 전제(`none`)와 결과(**여전히 보이는가·여전히 되는가**)를 **한 검사에서 함께** 읽는다.
- **잰 것(5건)**: `R⑨-0` 전제(`matchMedia` 일치) · `R⑨-1` 배율이 `none`이어도 **테두리 색**으로 들림이 보인다(두 값을 나란히 찍는다) · `R⑨-2` **끌어서 재배열이 실제로 된다**(순서 배열 전후 대조) · `R⑨-3` 되돌렸다 · 오디오 2건(전제 `none` + 🔊가 이퀄라이저로 바뀌고 `aria-pressed=true`).
- **구조(§7 2층)**: `underReducedMotion(page, fn)`을 `live-browser-lib.mjs`에 두어 켜고·되돌리는 일을 **한 곳**이 맡는다. 🔴 `browser.newContext()`가 아니라 `page.emulateMedia()`인 이유 — 새 컨텍스트는 **IndexedDB가 빈 채로 시작**해 픽스처가 통째로 날아간다.
- **§4 주입 2종**: Ⓐ 헬퍼를 `reducedMotion: null`로 → 전제 2건 RED(`애니메이션: "eqp"` vs 기대 `"none"`). Ⓑ 드롭에서 `onReorder` 건너뛰기(**이 축이 막으려는 진짜 결함**) → `R⑨-2`만 RED + 증거(`ro-wide → ro-square → ro-tall` = 원래 순서 그대로)이고 `R⑨-1`·`R⑨-3`은 초록 유지 = **죽지 않고 판정**(§18-G).
- 🔴 **주입 지점을 처음엔 잘못 골랐다.** 같은 결함을 `onDown`에 걸었더니 **검사가 `.drag-lift` 대기에서 죽어** R⑨-1/2/3 줄이 아예 안 나왔다 — 그러면 「게이트가 잡았다」와 「스크립트가 죽었다」를 구별할 수 없다. **§18-G가 게이트에게 요구하는 것을 주입에도 요구해야 한다.**
- **검증**: `verify-editor-live` **505/510**(남은 5는 지도 키 결손 — T-036·T-038).
- 🔴 **정직한 한계**: 잰 것은 **두 자리**(끌어서 재배열 · 소리 재생 표시)뿐이다. `app.css`의 `prefers-reduced-motion` 선언이 걸리는 다른 전환들은 **안 봤다.** 그리고 헤드리스 `emulateMedia`가 실기기의 OS 설정과 같은지는 **확인하지 않았다**(글꼴 배율과 같은 부류의 한계).
- **곁가지**: `ui-responsive-dev`의 「아직 안 재는 축」 표가 **비었다.** 지우지 않고 비워 뒀다 — 지우면 다음 사람은 그 규율이 있었다는 것도 모른다. 그리고 **빈 표는 「다 잰다」는 뜻이 아니다**(그때 대조한 것은 외부 규칙집 한 권이고, 그 목록 밖은 애초에 묻지 않았다).

---

## HANDOFF-0223 · **폭만 재고 있었다 — 글꼴 배율 축을 만들자 결함 5건이 나왔다** (2026-08-16 · T-051 종결)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- 🔴 **완료 조건 ①이 「기준을 먼저 정하라」였고, 숫자를 고르지 않았다.** `130%`(안드로이드 One UI 「글자 크기」 최대 단계) · `200%`(**WCAG 1.4.4 Resize Text**) — 둘 다 외부 근거다.
- 🔴 **기준을 정하기 전에 「이 축이 이 앱에 걸리는가」를 먼저 쟀다**: `font-size` 선언 **238개가 전부 `rem`, `px` 0개**이고 `html { font-size }` 선언이 없다 — 브라우저·OS 기본 글꼴을 **그대로 따라간다.** 반면 `--touch-target-min`은 `44px`이라 **글자만 커지고 상자는 안 커진다.** 그러므로 루트 `font-size` 조정이 이 앱에서는 **충실한 시뮬레이션**이다.
- **구조**: `coverSweep`에 배율 축을 넣어 **호출부 20곳이 자동으로** 세 배율을 잰다(§7 2층). 판정 **개수는 그대로** 두고 **깨진 배율을 이름으로** 적는다 — 검사 줄이 세 배가 되면 사람이 안 읽고, 안 읽는 판정문은 없는 것과 같다(§8).
- 🔴 **44px 표적만은 100%에서만 잰다.** 글자가 커지면 상자도 커져 그 축은 **쉬워진다** — 쉬워진 검사로 통과를 사지 않는다(§4).
- **찾은 결함 5건 — 전부 200% · 130%는 전부 통과**: `.chip{white-space:nowrap}`이 접히지 못해 `span.chip(+39 w312)` · `.guide-body`의 긴 영문 토큰이 `+107` · `.place-chip-copy`가 269~284px. 처방은 결함군 등록부 **1.32·1.33**과 같다 — **자르지 않고 접는다.**
- 🔴 **고치다 회귀를 만들었고 T-049가 어제 남긴 검사 셋이 잡았다.** `.place-chip`을 `flex-wrap: wrap`으로 바꿨더니 **`wrap`은 줄마다 교차축 크기를 따로** 계산해서 자식의 `align-self: stretch`가 무력해졌다 — 히트 높이가 **30px → 17px로 되돌아가며** 이웃 터치까지 훔쳤다. 부모는 되돌리고 **자식이 자기 안에서 접게** 했다. **예외를 「면제」가 아니라 다른 축으로 보낸 결정이 하루 만에 값을 했다.**
- **곁가지(§18-I)**: 순서 토글 픽스처가 `'2026-08-16T04:00:00.000Z'`를 손으로 못박아 뒀는데 **KST로 날이 넘어가자** 다른 묶음과 같은 날이 되어 빨개졌다 — 「오늘+1일」 파생으로 고쳤다.
- **§4 주입 2종**: 칩 수정 되돌림 → 200%에서 **원인 이름과 함께 RED** · 같은 결함인데 `SCALE_STEPS=[100]`이면 **0건 검출**(축의 검출력이 **배율에서 나온다**는 결정적 증명).
- **계측기도 두 번 고쳤다**: 페이지 넘침에 **원인을 안 적고 있었고**(`page+39`만), 원인을 `scope` 안에서만 찾아 「순간 폼」처럼 원인이 밖에 있는 화면에서는 이름이 비었다.
- **검증**: `gates` exit 0 · `verify-editor-live` **499/504**(남은 5는 지도 키 결손) · `verify-diagnostics-live` **144/144**.
- 🔴 **정직한 한계**: 잰 것은 **가로 넘침·자르는 상자·글자 갈라짐**뿐이다. **세로로 잘려 못 읽는 것**은 안 잰다. 그리고 배율은 **rem 파생**으로만 흉내 냈다 — 실기기의 안드로이드 textZoom과 같은지는 **확인하지 않았다.**

---

## HANDOFF-0222 · 🔴 **세션 종료 — v2.45 배포 완료 · 다음 작업자가 여기부터 읽는다** (2026-08-15)
...
| 항목 | 값 |
|---|---|
| 기본/배포 브랜치 | **`main`** (브랜치 보호 켜짐 · Required 2개가 병합을 실제로 막는다) |
| 배포된 버전 | **v2.45** · squash 커밋 **`b320327`** (PR #290) |
| 작업 브랜치 | `claude/constitution-rules-compliance-4m7gp0` — **`origin/main`에서 다시 세움**(기여분 0) |
| 미커밋·미푸시·stash | **전부 0** |
| 병합 안 된 브랜치 | **없음** (원격 브랜치는 `main`과 위 작업 브랜치 둘뿐) |
| DB·마이그레이션·Edge | **이번 판 변경 0건** — 적용할 migration 없음 |
| 릴리스 잠금 | 닫힘 · **재개방 0회** |
...
| 판 | 내용 | 인계 |
|---|---|---|
| **v2.44** | 타임라인 최신순 토글 · 사람 단위 동행인 + 「함께한 기록」 화면 · 계약 전수조사 · `sync.ts` 판정 3개 추출 | 0216·0217·0218·0219 |
| **v2.45** | 투명 모달 수리(M-0172) · 커버 폭 훑개 공용화와 T-049 종결(M-0173) | 0220·0221 |

**종결한 과제 넷**: T-047(완료 조건을 셀 수 있게 개정 후 종결) · T-048 · T-049 · T-050.
**새로 연 과제 둘**: T-051(글꼴 배율) · T-052(reduced-motion).

### 🔴 이번 세션의 관통 주제 — **손으로 복사된 규율은 반드시 한 곳에서 빠진다**

세 번 같은 형태가 나왔고, 셋 다 §2·§7이 이미 적어 둔 것이었다:

1. **모달 표면 네 줄**이 형제 다섯에 손으로 복사돼 있었다 → 여섯 번째가 **투명하게** 태어났다(M-0172).
2. **커버 폭 훑개**가 한 라이브 파일 안에만 있었다 → 진단 도구 17개는 344px를 **한 번도 안 봤다**(M-0173).
3. **완전 인계서의 「현재 시작점」**이 버전·인계 번호를 손으로 적고 있었다 → **여덟 판** 낡았다(M-0174).

**처방도 같았다**: 값을 고치지 않고 **적을 자리 자체를 없앴다.** 셋 다 이제 정의가 한 곳이고, 앞의 둘은 게이트가 되돌림을 RED로 잡는다.

### 다음 작업자를 위한 것

**첫 번째로 할 일**: `npm run brief <고칠 파일들>` — 읽을 스킬 문서·형제 목록·과거 실수·열린 과제·미완성 작업을 그 자리에서 뽑아 준다. 아무것도 외울 필요가 없다.

**열린 과제 6건**(정본은 `docs/BACKLOG.md` BKL-1):

| 우선순위 | 과제 | 상태 | 선행조건 |
|---|---|---|---|
| **1** | **T-051** 글꼴 배율 최대에서 아무것도 안 잰다 | 대기 | 「어디까지 버티는가」 기준을 **먼저** 정할 것 |
| **2** | **T-052** `prefers-reduced-motion`이 기능을 뺏는지 안 본다 | 대기 | 없음 — `reducedMotion: 'reduce'` 컨텍스트로 라이브 1회 |
| 3 | **T-028** 그럴듯하게 틀린 날짜 | 대기 | 「릴리스 시각의 지역 시간대 정본」 설계 결정 |
| — | **T-036** TomTom 실기기 확인 | 막힘-실기기 | 사용자 우즈베키스탄 기기 |
| — | **T-037** Tauri glib 보안 업데이트 | 막힘-상류 | 상류 판 출시 |
| — | **T-038** 정부지도 승인키 | 막힘-사용자 | 지도제공 검색 API 승인 |

🔴 **T-051·T-052는 「기준을 먼저 정하라」가 완료 조건에 들어 있다.** 기준 없이 게이트를 만들면 초록의 뜻이 「지켰다」가 아니라 **「안 봤다」**가 된다 — 이 저장소가 반복해서 지키는 자세다.

### 🔴 사용자 실기기가 확인해야 하는 것 (내가 잴 수 없는 것)

1. **v2.45가 실제로 떴는지** — 이 샌드박스는 `*.github.io`를 막는다. **「배포 그린 확인」과 「사이트에서 확인」은 다른 말**이고, 후자는 아직 안 됐다.
2. **사람 이름표(34px) 오조작** — 크게 만들어 봤더니 이웃 칩의 터치를 훔쳐서(실측) 되돌렸다. 실기기에서 실제로 잘못 눌리면 재심사(칩 줄 밀도 = 배치 결정 · T-044와 같은 자리).
3. **지도 5건** — CI는 초록이지만 실기기의 Kakao/TomTom 타일은 사용자 몫이다(T-036).
4. **`map-modal`** — 표면 이식 뒤 불투명한지 **재지 않았다**(라이브 참조 0회). 값이 같으므로 같아야 하지만 그건 추론이다.

### 바꾸면 안 되는 것 (이번 세션에서 값을 치르고 세운 것)

- **모달 표면 네 줄은 `.modal-base`에만.** 형제가 같은 값으로 다시 쓰면 `check-verdict-symmetry`가 RED. `padding`은 **일부러 안 올렸다**(헤더/바디 구조를 가진 모달이 깨진다).
- **`coverSweep`·`TOUCH_TARGET_EXCEPTIONS`는 `live-browser-lib.mjs`에만.** 호출부로 되돌리면 진단 도구가 다시 344px를 안 본다.
- **`.chip-person`·`.place-chip-map`·`.place-chip-copy`에 세로 `::after`를 넣지 마라.** 넣어 봤고 이웃 칩의 터치를 훔쳤다(실측). 되돌림을 시도하면 라이브가 `stolen`으로 RED.
- **`sync/merge.ts`의 판정 3종은 유니온이다.** `boolean`으로 뭉치지 마라 — M-0170이 그 사고다.
- **`HANDOFF_CODEX.md`에 버전·인계 번호를 적지 마라**(M-0174).

---

## HANDOFF-0221 · **못 잰 화면을 재러 갔더니, 못 잰 것은 화면이 아니라 「자」였다** (2026-08-15 · T-049 종결)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **잰 것**: T-043이 「못 잰 것」으로 남겨 둔 전부 — 순간 폼 전체 · R2 설정 가이드 본문 · 휴지통 · 동행인 칩 3개(창 안 + 카드 위) · 🔴 **진단 도구 17개**(× 4축 = **68건**).
- 🔴 **가장 큰 발견은 화면이 아니라 도구였다.** `coverSweep`이 `verify-editor-live.mjs` **안에만** 있어서 `verify-diagnostics-live`는 **375px 하나로만** 재고 있었다. 이 앱의 최난이도 폭은 **344px**이므로, 그동안의 「375px 통과」는 「좁은 화면에서 괜찮다」가 아니라 **「344px는 안 봤다」**였다. §7 2층은 *"규칙을 한 곳에만"*이라고 말하는데 그 「한 곳」이 **호출부 파일**이면 다른 파일은 애초에 못 통과한다.
- **수정**: `coverSweep`·`TOUCH_TARGET_EXCEPTIONS`를 `live-browser-lib.mjs`로 올리고 호출부는 얇은 껍데기만 남겼다. 진단 쪽은 **등록부를 도는 반복 안**에서 부르므로 **새 도구가 생기면 자동으로 따라온다.** 옮긴 뒤 **473/478 동일**로 거동 불변을 확인했다.
- **훑개가 스스로 결함 3건을 잡았다**(주입 없이 §4를 증명):
  1. **동행인 칩 34px** — v2.44에서 **내가 만든** 버튼이 44px 규율을 안 물려받고 태어났다.
  2. **`place-chip-map`·`place-chip-copy` 17px** — 형제가 받은 「칩 높이를 채운다」 처방을 이 둘만 못 받았다(칩 가족에서 **세 번째** 같은 형태) → 30px로 수정.
  3. 🔴 **사진 띠 「10px 잘림」은 내 오탐이었다** — `overflow-x: auto`는 의도된 스크롤이고 저장소엔 `XSCROLL_OPT_OUT`이 이미 있었는데 **`scanOverflowContainers`만 그걸 보고 `coverSweep`은 안 봤다.** 같은 질문에 스캐너 둘이 다른 답을 내던 상태(§17)라 통일했다.
- 🔴 **넓히려다 실측에 막혔고, 그게 이 판의 교훈이다.** 셋 다 형제 처방(투명 `::after`로 세로 44px)을 실제로 넣었더니 `closeButtonTarget()`이 **`stolen: ["chip companions chip-person"]`**을 냈다 — 칩 줄이 `flex-wrap`으로 접히고 `gap`이 확장분보다 작아 **이웃 칩의 터치를 훔쳤다.** 「러원이」를 누르려던 손에 **다른 사람의 기록**이 열리는 것은 미달보다 나쁘다(M-0162와 같은 판정). **되돌리고 실측값과 함께 예외로 등록**했다.
- **예외는 면제가 아니다**: 셋을 **다른 축으로 보냈다** — 라이브가 「덮음 0」과 「칩 높이 ≥30px」를 계속 잰다. 다음 사람이 선의로 `::after`를 다시 넣으면 `stolen`이 차서 **RED**가 된다(그 상태를 실제로 만들어 확인했다).
- **검증**: `gates` exit 0 · `verify-editor-live` **499/504**(남은 5는 지도 키 결손) · `verify-diagnostics-live` **144/144**.
- 🔴 **정직한 한계 둘**: ①초록은 「없다」가 아니라 **「그 화면은 봤다」**까지다 ②잰 축은 **넷**(페이지 넘침·자르는 상자·글자 갈라짐·터치 표적)뿐이고 **글꼴 배율과 reduced-motion은 여전히 안 잰다**(T-051·T-052).

---

## HANDOFF-0220 · **외부 규칙집에서 둘만 흡수하고, 그 첫 사례로 투명 모달을 잡았다** (2026-08-15 · 사용자 지시)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **사용자 지시**: *"우리에게 도움이될만한 것들만 흡수하자. 그리고 난후 첫 사레로 이거 수정하는데 사용해보자"*(「여행 편집」 모달 스크린샷 첨부).
- 🔴 **순서를 한 번 뒤집어 제안했고 그대로 갔다**: 사전은 *"좋은 게 뭔지"*를 알려 주지 *"지금 뭐가 깨졌는지"*는 못 말한다. 그래서 **①먼저 재서 결함을 확정 → ②사전은 「고친 상태가 옳은가」의 체크리스트**로 썼다. 반대로 했으면 사전이 말하는 것만 고치고 진짜 결함을 놓쳤을 것이다.
- **흡수 결과 — 딱 둘**: 외부 규칙집(`ui-ux-pro-max`)이 말하는 것 대부분은 우리가 **이미 더 촘촘하게** 갖고 있었다(터치 44px·safe-area·대비·다크 토큰·4/8 간격·경계 폭 — 게다가 우리는 344px까지 잰다). **없던 것은 ①글꼴 배율 최대에서의 확인 ②`prefers-reduced-motion` 실측**뿐이고, 둘 다 **규칙 0·검사 0**이었다. 🔴 **당장 게이트를 만들지 않았다** — 「어디까지가 정상인가」의 기준이 없어서, 만들면 초록의 뜻이 「지켰다」가 아니라 **「안 봤다」**가 된다. `T-051`·`T-052`로 올리고 헌장에 「아직 안 재는 두 축」으로 적었다.
- **첫 사례(M-0172)**: `.trip-editor-modal`의 배경이 `rgba(0,0,0,0)`이라 **뒤 화면 글자가 모달 글자를 뚫고 나왔다.** 원인은 「빠뜨렸다」가 아니라 **형제 다섯이 같은 네 줄을 손으로 들고 있었던 것**이다 — 다섯 벌 복사되는 규칙은 여섯 번째에서 반드시 빠진다(§2).
- **고친 방식(§7 3층)**: 표면 네 줄을 `.modal-base` **한 곳**으로 올리고 형제 다섯에서 걷어냈다 → 새 모달은 `modal-base`만 붙이면 **물려받는다.** `padding`은 **일부러 안 올렸고 이유를 코드에 적었다**(헤더/바디 구조를 가진 모달은 자식이 여백을 갖는다).
- **곁가지**: 취소 버튼 폭이 **「취소」 두 글자 길이(52px)**로 정해져 저장과 **8.08:1**이었다. 3-E ②가 폭에도 걸리는 자리다 → 하한을 줘 2.6:1.
- 🔴 **게이트를 만들며 내가 두 번 틀렸다**: 주석을 안 벗겨 **정본이 지켜지는 상태를 위반으로** 보고했고, 이름 패턴만 보고 `.timeline-drop-overlay`를 **오탐**했다. 둘 다 §11이 적어 둔 형태이고, 둘 다 고쳤다.
- **검증**: `gates` exit 0 · `check-verdict-symmetry` 셀프테스트 **55건** · **§4 주입 3종 전부 RED 후 복원**(정본 삭제 → 정적 3건 · 손복사 부활 → 정적 · 정본 삭제 → **라이브가 `alpha:0`을 값으로** RED) · `verify-editor-live` **473/478**(신규 3건 PASS · 남은 5는 지도 키 결손) · 344px·800px 두 폭에서 **캡처로 눈 확인**(§13).
- 🔴 **정직한 한계 — 세어 보고 적는다**: 형제 다섯에서 표면 선언을 걷어냈는데, 라이브가 실제로 여는 것은 **넷**이다(`guide` 4회 · `pr-records` 6회 · `pe-sheet` 1회 · `single-photo-moment` 1회 참조). 🔴 **`map-modal`은 라이브 참조가 0회**라, 그 모달이 여전히 불투명한지는 **재지 않았다.** 값이 `.modal-base`와 글자 단위로 같았으므로 계산 결과는 같아야 하지만 **그건 추론이지 실측이 아니다**(§8). 지도 화면은 이 환경에서 키 결손으로 5건이 이미 빨간 자리이기도 하다 — 사용자 실기기가 첫 검출자다.

---

## HANDOFF-0219 · **닫힐 수 없는 완료 조건을 셀 수 있는 것으로 바꾸고 닫았다** (2026-08-15 · T-047 종결)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- 🔴 **먼저 인정할 것**: T-047의 원래 완료 조건(*"줄 수 목표를 정하지 않는다 … 하나씩 꺼낸다"*)은 **닫힐 수 없다.** 그건 일감이 아니라 **자세**다 — T-034를 닫을 때 쓴 판정이 이 행에도 그대로 걸린다. 사용자가 *"T-047 050 048까지만 완벽히 해결하고 배포하자"*라고 했을 때, 이 조건으로는 「완벽히」가 정의되지 않는다.
- **그래서 조건을 개정했다(셀 수 있게)**: 🔴 **「파괴적 쓰기를 가르는 판정」이 전부 이름을 갖고 유닛으로 덮이는가.** `sync.ts`가 「최고 위험 표면」인 이유는 길이가 아니라 **서버 행·바이트·로컬 자료를 지울 수 있다**는 것이므로, 이게 이 과제가 실제로 재려던 위험이다.
- **전수는 `grep`으로 셌다**(손으로 세지 않는다). 아홉 자리 중 **셋이 무방비**였고 이번에 꺼냈다:
  - `requeueDeleteAction` — **일곱 pull 루프 전부**가 부르는데 **유닛 0건**. 참이면 만들어지는 것은 `delete` op = 서버 행을 지우는 길의 입구. `server-read-only`면 **로컬 큐도 안 건드린다**는 M-0095 계약이 **산문으로만** 있었다.
  - `unreferencedByteAction` — DB가 **지금 가리키는** 키를 지우면 「행은 있고 바이트가 없는」 최악 상태가 된다(비타협 원칙 #1). 그걸 막던 것이 **두 줄**이었고 유닛이 없었다.
  - (앞 판) `serverOrphanAction` — HANDOFF-0216.
- 🔴 **셋 다 `boolean`이 아니라 유니온으로 꺼냈다** — M-0170이 나흘 전 같은 파일에서 가르쳐 준 것이다. 특히 **「안 한다」의 뜻이 둘 이상**이면 반드시 갈라 부른다: `skip-read-only`(계약이라 안 한다)와 `skip-not-stranded`(해당 없음), `no-candidate`(없다)와 `still-referenced`(**현역이다**). 뭉치면 다음 사람이 「어차피 안 지우는데」로 가드를 단순화하고, 그 순간 현역이 지워진다.
- **§4 주입 5종 전부 RED 확인 후 복원**: read-only 순서 뒤집기 · read-only를 「할 일 없음」으로 뭉개기 · 사용자가 안 지운 것도 삭제 대상으로 · 현역 키 가드 제거 · 두 침묵 뭉개기.
- **검증**: 유닛 **1744/1744**(신규 11) · `npx tsc --noEmit` exit 0 · `gates` exit 0.
- 🔴 **정직한 한계 셋**: ⓐ`sync.ts`는 **여전히 2,700줄대**다 — 이 과제는 줄이는 것이 아니라 꺼내는 것이라 그게 정상이다 ⓑ목록은 **`grep`이 잡은 파괴적 쓰기 자리**에서 나왔고, `grep`이 못 본 경로가 있으면 목록도 그만큼이다 ⓒ`discardVerifiedLocalOriginal`은 **일부러 안 꺼냈다** — 본질이 바이트 비교 + 트랜잭션 read-back이라 꺼내면 알맹이가 안 나온다(유닛은 양쪽 갈래로 있다).
- **자세는 계속 간다 — 과제 행 없이.** `check-fn-size` 래칫(§7 3층)이 매 작업마다 강제한다. 실제로 이번에도 그 래칫이 `pullMedia`에서 실행부를 덜어내게 밀었다.

---

## HANDOFF-0218 · **기다릴 것을 하나만 골랐다 — 결과는 기다리고 전제는 안 기다렸다** (2026-08-15 · T-048 종결)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적) · **앱 코드 변경 없음**(검사만).
- **무엇이 흔들렸나**: `verify-editor-live`의 `R⑧ 재배열 직후 사진 뷰어`. 같은 코드에서 1회차 빨강·재실행 통과.
- 🔴 **원인은 읽어서 확인했다.** `onReorder`는 `void (async () => { await reorderMomentPhotos(); await refresh(); })()`이고 `refresh()`가 **격자를 통째로 다시 그린다.** 검사는 `settle`(프레임 두 장 ≈32ms) 뒤에 바로 다음 칸을 눌렀다 — 재그리기 중이면 `mousedown`과 `mouseup`의 대상이 갈려 **`click` 자체가 안 난다.**
- 🔴 **이번 교훈의 핵심(M-0171)**: **결과 기다림은 전제 기다림을 대신하지 못한다.** M-0164에서 이 자리를 고칠 때 「뷰어가 열렸다」는 결과를 `waitUntil`로 기다리게 했는데, **일어나지 않은 클릭의 결과는 영원히 안 온다.** 「무엇을 기다리나」에 답이 둘이었고 하나만 골랐다.
- **고친 것**: ①누르기 **전에** 「격자가 새 순서로 다시 그려졌다」를 **내용으로** 기다린다(`R⑦-1`) ②실패하면 뷰어 수·누른 자리·놓은 자리·**거리**·지금 순서·그 점에 있는 요소를 들고 나온다.
- 🔴 **증거가 유력 가설 하나를 죽였다.** `postDragClickBlock`은 놓은 자리 **10px·180ms** 안쪽의 click 한 번을 삼킨다 — 가장 그럴듯한 용의자였다. 주입해 재보니 **거리 204px**였다. 삼킬 수 없다는 것이 **값으로** 증명됐다(§8 — 그럴듯한 원인은 확인된 원인이 아니다).
- **§4 주입 3종**: 기다림 조건 틀림 → `R⑦-1` RED이면서 **본 순서를 적는다**(앱이 틀렸나 내 기대가 틀렸나가 갈린다) · 그때도 `R⑧`은 **정상 통과**(기다림과 판정이 분리돼 있다는 증거 · T-045와 같은 규율) · 클릭 삼킴 → `R⑧` RED + 증거 JSON.
- **검증**: `build` exit 0 · `verify-editor-live` **연속 2회 470/475**(남은 5는 지도 키 결손 — 이 환경의 알려진 상태).
- 🔴 **정직한 한계**: **「흔들림이 사라졌다」고 말하지 않는다.** 흔들림은 정의상 가끔 나므로 2회 통과가 부재의 증거는 아니다. 말할 수 있는 것은 「확인된 경합 하나를 없앴고, 다음에 나면 **값이 남는다**」까지다.

---

## HANDOFF-0217 · **비대칭을 의심해 열었더니, 재보니 정당한 비대칭이었다** (2026-08-15 · T-050 종결)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적) · **거동 변경 없음**(주석과 문서만).
- **무엇을 물었나**: 서버 고아 스윕이 `pullMedia`에만 있는 것이 **§7 위반(형제 비대칭)인가?** 소리·영상도 바이트를 갖는 형제인데 부모 여행의 영구삭제를 안 본다.
- 🔴 **§9 4단계(「세계를 본다」)를 먼저 했고, 그것이 처방을 뒤집었다.** 운영 DB를 **읽기 전용**으로 재니 구조가 답이었다:
  `trips ──CASCADE──> moments ──CASCADE──> media · audio · videos · expenses`. 네 자식 모두 `moment_id`가 **NOT NULL**(실측 null 행 0)이고 `trips`를 직접 참조하는 것은 `moments`뿐이다. **여행 행이 지워지면 서버가 자식을 전부 데려간다** — 빠져나갈 구멍이 없다. §7-A(M-0107)가 요구하는 「영구삭제의 가족 범위 = 서버 FK가 데려가는 범위」가 **이미 지켜지고 있었다.**
- **실측 수치**: 활성 고아 **다섯 도메인 전부 0건** · 모집단은 실재(`purged_ids` **102** · trips 10 · media 141).
- 🔴 **그래서 확장하지 않는다.** 확장은 FK가 이미 막는 경우를 위해 **파괴적 서버 쓰기 경로를 넷 더** 만드는 일이다 — 안전을 사는 게 아니라 위험을 파는 것이다. 이 스윕의 정체는 FK 연쇄가 닿지 않는 **역사적 잔재**를 위한 안전망이다.
- **이유를 코드에 적었다**(완료 조건 ②): `serverOrphanAction`·`sweepServerOrphanMedia` 두 머리말. 🔴 **M-0060의 교훈대로 「이 이유가 참인 사정권」을 함께 못박았다** — *"자식의 `moment_id`를 nullable로 바꾸거나 `trips` 직접 FK를 추가하면 이 판단부터 다시 하라."* 산문은 스키마가 바뀔 때 같이 안 바뀌기 때문이다.
- 🔴 **정직한 한계**: 소리·영상은 **서버 행이 0개**라 그쪽 「고아 0건」은 **공허하다**(§4). 이 판단이 기대는 것은 행 수가 아니라 **스키마 구조**(FK + NOT NULL)이고, 그건 행이 0이어도 참이다 — 그 점을 코드에도 적었다.
- **배운 형태**: 「형제가 안 하는 일이 있다」는 관측이지 결함이 아니다(§8 · M-0056). 세지 않고 확장했으면 **없는 위험을 막으려고 진짜 위험을 넷 만들 뻔했다.**

---

## HANDOFF-0216 · **최고 위험 표면의 첫 판정을 꺼냈고, 꺼내다 내가 거동을 바꿨다** (2026-08-15 · T-047 착수)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **무엇을 꺼냈나**: `pullMedia` 안에 인라인이던 **서버 고아 스윕 판정** → `sync/merge.ts`의 `serverOrphanAction`. 이 판정이 지키는 것은 **파괴적 서버 쓰기**(tombstone push + R2 바이트 삭제)인데 **유닛이 하나도 없었다** — IO 클로저 안이라 붙일 수가 없었다. 실행부도 `sweepServerOrphanMedia`로 갈라 「지울까?」와 「어떻게 지우나」를 분리했다.
- 🔴 **꺼내다 내가 거동을 바꿨다(M-0170).** 처음엔 `boolean`으로 꺼냈는데 원래 흐름은 **세 갈래**였다(`not-orphan` 흘려보냄 / `skip-row` = `continue` / `sweep`). `false`를 받은 호출부가 아래로 흘러가 **`server-read-only`에서 R2 다운로드가 돌았다.** `ServerOrphanAction` 유니온으로 고쳤다.
- 🔴 **그걸 잡은 것은 새 검사가 아니라 있던 검사다** — `canonicalSync.test.ts`가 이미 `server-read-only`에서 R2 호출 0을 세고 있었다. **T-047의 값을, T-047이 시작하자마자 예전 사람이 붙여 둔 유닛이 증명했다.**
- **검증**: 유닛 **1733/1733**(신규 6 — 네 입력 **16조합 전수**로 `sweep 1 · skip-row 1 · not-orphan 14`를 못박음) · §4 **주입 3종 전부 RED 확인 후 복원**(read-only를 not-orphan으로 뭉갬 · parentPurged 제거 · localMissing 제거) · `gates` exit 0 · `build` exit 0 · `verify-editor-live` **469/474**(남은 5는 지도 키 결손 — 거동 불변).
- **래칫이 또 밀어 줬다**: `pullMedia`가 상한을 넘어 실행부를 덜어냈고, 결과적으로 판정·실행이 갈렸다(§11 마지막 문단).
- 🔴 **찾았지만 안 고친 것 → T-050**: 이 스윕은 **`pullMedia`에만** 있다. 나머지 여섯 pull은 자기 id만 보고 **부모 여행 영구삭제는 안 본다.** 소리·영상도 바이트를 갖는 형제라 같은 사각지대가 있을 수 있다 — **그러나 「있을 수 있다」는 확인이 아니다**(§8). 서버 FK cascade가 데려가는지 재보지 않았고, 거동을 바꾸는 판단이라 분리했다.
- **정직한 한계**: 꺼낸 것은 **판정 하나**다. `sync.ts`는 여전히 2,700줄대이고 줄 수는 오히려 늘었다 — 🔴 **이 과제는 줄이는 것이 아니라 꺼내는 것**이라 그게 정상이다(T-034 재판단에서 정리한 그대로).
- **다음**: T-047 계속(다음 판정) · T-050 · T-048 · T-049.

---

## HANDOFF-0215 · **모든 과제를 현 기준에서 재판단 — 「자세」가 과제 목록에 앉아 있었다** (2026-08-15 · 사용자 지시)

- **사용자 지시**: T-034를 왜 해야 하냐는 물음 → 재보니 **해야 할 일이 아니었다** → *"모든 과제를 현 기준에서 재판단해보는건 어때?"*
- 🔴 **T-034: 수치 넷 중 셋이 낡았고 하나는 10배 넘게 틀렸다.** `diagnostics.ts` **2,869 → 282**(이미 쪼개졌다 — 판정이 `domain/*Verdict.ts` 12개 이상으로 나갔고 그 전부에 유닛이 붙었다) · `tripDetail.ts` 3,426 → **3,846으로 늘었다** · `app.css` 2,293 → 2,498 · `sync.ts`만 2,673 그대로.
- 🔴 **그리고 완료 조건이 잘못된 것을 재고 있었다**: *"래칫 값이 의미 있게 내려가면 닫는다"*인데 **줄 수는 이 과제가 재려던 것이 아니다.** 진짜 이유는 *"순수 로직이 DOM 클로저에 갇혀 유닛을 못 붙인다"*(M-0022)이고, 그건 **꺼내면 해결되지 줄여서 해결되지 않는다.** 실측: `tripDetail.ts`의 최상위 함수 97개 중 **33개는 이미 DOM을 안 만진다** — 한 덩어리 blob이 아니다.
- **T-034 → 「상시 자세로 전환」으로 닫았다.** 행 스스로 *"한 번에 쪼개지 않는다 · 작업하다 걸릴 때마다 덜어내는 것이 정상 진행"*이라고 적었다. 그건 일감이 아니라 자세이고 `check-fn-size` 래칫이 이미 강제한다 — **오늘 하루에만 세 번 걸려** `momentCard`·`orderBar`·`convertedTotalStat`를 덜어냈다. 🔴 **자세를 과제 목록에 두면 영원히 안 닫히고, 안 닫히는 행은 읽히지 않는다.**
- **남은 진짜 위험만 T-047로 분리**: `sync.ts` 2,673줄 — 형제들은 줄었는데 **유일하게 안 줄었고**, 헌법이 「최고 위험 표면」이라 부르는 자리다. 완료 조건에 **줄 수 목표를 안 적었다**(적으면 그 숫자를 맞추려고 우회한다).
- 🔴 **상태 어휘에 자리가 없어 분류가 거짓말을 하고 있었다.** T-037(Tauri glib)은 상류가 판을 내야 움직이는데 갈 상태가 없어 **「대기」에 앉아 할 수 있는 일처럼 보였다.** `막힘-상류`를 추가하고 옮겼다 — 「대기」는 *내가 안 한 것*, 「막힘」은 *내가 못 하는 것*이다.
- 🔴 **기록만 하고 추적은 안 하던 것 둘을 찾아 올렸다**(§21 ③의 경계에 있던 상태): **T-048**(`R⑧` 흔들림 — M-0164가 「고쳤다」고 적은 세 자리 중 하나가 오늘 다시 흔들렸고 HANDOFF에만 적혀 있었다) · **T-049**(T-043이 「못 잰 것」으로 남긴 화면들 + 동행인 칩 여러 개일 때의 344px 줄바꿈).
- **T-028 재판단**: 전제가 **아직 그대로**다(§18-I가 *"일반형에는 아직 ③이 없다"*고 적고 있고 `check-date-freshness`도 미래·이틀 낡음만 본다). 여전히 「안 해도 결함이 아니다」이고, 실제 내용은 **「릴리스 시각의 지역 시간대 정본을 어디 둘 것인가」라는 설계 결정**이다.
- **T-036·T-038**: 상태 맞음(각각 실기기·정부 승인). T-038의 정부지도 어댑터는 코드에 살아 있고 키만 없다 — 확인함.
- **결과**: 열린 과제 5 → **7**(닫음 1 · 신설 3). 🔴 **재판단이 과제를 늘렸다** — 그게 정상이다. 안 보이던 빚이 보이게 된 것이지 빚이 늘어난 게 아니다.
- 🔴 **낡은 숫자를 다시 적지 않았다**(§18-I 「자동 옆의 수동」): 코드가 움직이는데 옆에 손으로 적은 숫자가 붙으면 반드시 밀린다. 필요하면 `wc -l`로 그때 잰다.
- **검사**: `gates` exit 0(70종).
- **다음**: T-047(`sync.ts`) · T-048(흔들림) · T-049(못 잰 화면) · 문서↔코드 모순 조사.
...
**하네스 게이트**: SSOT=`scripts/harness.mjs`, 현재 **<!--reg:gateCount-->76<!--/reg-->개**. **개수도 목록도 여기 손으로 적지 않는다** — `src/app/registry.gen.ts`(자동 생성, `check-registry-gen`이 드리프트 차단)에서 파생하고 개발자 정보→설계 개요도/가이드가 그 목록과 한 줄 설명을 그대로 표시한다. 목록을 눈으로 보려면 `npm run harness` 또는 `src/app/registry.gen.ts`를 열면 된다.
...
**검증**: harness <!--reg:gateCount-->76<!--/reg-->/27 · 유닛 410 · verify-editor-live 127/127 · build 그린. **미실행**: 실기기 PWA 설치·실제 로밍 — 사용자 확인 몫.
...
**검증**: harness <!--reg:gateCount-->76<!--/reg-->/26 · 유닛 410 · verify-editor-live 123/123(뷰어 13종·폰트 조각 4종 포함, 실제 Chromium이 `dist` 서빙) · build 그린. **미실행(정직)**: 실기기 터치·네트워크 동기화(샌드박스가 `*.supabase.co` 차단)·PWA 설치 — 여전히 사용자 실기기 확인 몫.
...
npm run harness                        # Required 게이트 전체 (현재 <!--reg:gateCount-->76<!--/reg-->개 — 목록은 scripts/harness.mjs, 손편집 나열 금지 M-0001; 이 숫자는 gen-registry가 자동 갱신·check-doc-counts가 대조)
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(건너뜀 0) · 유닛 1016 · build OK · 문장은 순수 함수라 라이브가 SSOT로 따라옴(`verify-editor-live` 문장 대조 PASS).
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 1016(photoMetaSniff 5 포함) · build OK
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **1011** · live 275/275 + 22/22
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **998** · live 270/270 + 22/22
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **989** · live 270/270 + 22/22
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **981** · live 270/270 + 22/22 · `check-adapter-parity` PASS(정본↔어댑터 2개 글자 단위 일치)
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **978** · live 270/270 + 22/22 · build OK
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 969 · `verify-editor-live` **270/270**(+4) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **969**(+18) · `verify-editor-live` **266/266**(+9) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 951 · `verify-editor-live` **257/257**(+3) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 951(+8) · `verify-editor-live` **254/254**(+7) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 943 · `verify-editor-live` **247/247**(+4) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **943**(+4) · `verify-editor-live` **243/243**(+4) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **939**(+21) · `verify-editor-live` **239/239**(+23) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **918**(신규 `tripClock.test.ts` 29건 + 시간대 계약 케이스 확장) · `verify-editor-live` **216/216**(신규 18건) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(건너뜀 0) · 유닛 **882** · `verify-editor-live` **198/198** · build OK.
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(건너뜀 0) · 유닛 **745**(+23) · live 165+22 · build OK.
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(건너뜀 0) · 유닛 **715**(+13) · live 165 + **14** · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(라이브 2종 포함 — 건너뜀 0) · 유닛 702 · live 165+12 · build OK. **주입 RED**: 미등록 화면 · `scripts/`·`supabase/` 라우트 제거 · 셀프테스트 강제 실패(Required이므로 FAIL, SKIP 아님).
...
하네스 <!--reg:gateCount-->76<!--/reg-->개 전부 PASS(라이브 2종 포함 — 건너뛴 것 없음) · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS · 유닛 542건 · **주입 RED 8건**(백업 호출 제거 · `occurred_at`·`taken_at` 되돌리기 · `CHECK_COUNT` 드리프트 · `localeCompare` 복귀 · `withCanonicalStamps` 축소 · `compareInstants` 무력화 · 게이트 자체검사) · build OK · live 136/136(1회 흔들림, 아래 참조).
...
**검증**: 하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS · 유닛 526건 · 옛 로직 주입 **3건 RED** · live 136/136 · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS · 유닛 513건(instantFormat 28건 신설) · 주입 RED 3건(rowmap 타입 · merge · integrity) · live 136/136 · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(로컬 + Node 20) · 유닛 PASS(원자성 3건 신설 + 되살리기 3건) · 주입 RED 확인 · live 130/130 · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->76<!--/reg-->개 PASS(로컬 Node + **CI가 쓰는 Node 20**) · 유닛 PASS(`restoreUnpurge` 12건 신설 + 문장 검사 6건) · **주입 검증**으로 가드 제거·되읽기 신뢰·보호 해제 각각 RED 확인 · `verify-editor-live` 130/130 · `npm run build` OK.
