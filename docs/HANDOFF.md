...
**한 줄**: 소리를 서버로 보냈고(v1.20), **배포 30분 만에 사용자 스크린샷이 거짓 안내를 잡았고**(v1.21 · M-0046), 그 결함군에 **없던 기계 층 둘**을 놓았다. 마지막에 사용자가 규율 하나를 더 세웠다 — **"내가 볼 수 있는 것을 사람에게 묻지 않는다"**(헌법 §13).

| 순서 | 무엇 | 결과 |
|---|---|---|
| 1 | **오디오 서버 동기화**(HANDOFF-0013) — 마지막 「로컬 전용」 예외를 갚았다 | 마이그레이션 `0019`+`0020` 적용 · `media-sign` **v6** 배포·되읽기 · PR #126 merged · Pages #167. 실서버 `journey.audio` 4행, R2에 `.webm`이 사진 9장 옆에 착지(50.54KB = `bytes:50542`) |
| 2 | 🔴 **M-0046**(HANDOFF-0014) — 진단이 **찾아보지도 않고** 「소리 자체는 없습니다」라 하고 **삭제를 권했다.** 그 3건은 로컬 휴지통에 멀쩡히 있었다 | 원인 둘(①tombstone이면 착지 키를 모르는데도 안 올림 ②진단이 「로컬 사본이 있나」를 안 물음) 셋 다 고침 + `[서버에 없는 자료 다시 올리기]` 복구 경로. PR #127 merged · Pages #168 |
| 3 | **`verify-diagnostics-live` 신설**(12건) — §10 ③(전달 결함)에 **아예 없던 기계 층** | A층: 실제 Chromium이 진단 도구 7개를 하나씩 열어 판정·글리프·자리·넘침을 잰다. B층: 앱에서 만들 수 없는 상태를 **판정 함수에 먹여 실제 렌더러로** 그린다. 주입 3종 RED |
| 5 | **집중점검**(사용자 지시) — 교훈 반영·상충 점검·인계서 최신화 후 **이번에 추가한 것을 적대적으로 다시 봤다** | 🔴 **내가 만든 것에서 결함 3건**(M-0047): 커버리지 게이트가 SKIP된 층을 「덮음」이라 말함 · `check-skill-routing`이 자기 주장보다 좁게 훑음 · Codex 어댑터가 §13을 **안 하는 쪽으로** 읽히게 적혀 있었음. 셋 다 수정 + 주입 RED |
| 4 | **`check-live-coverage` 신설 + 헌법 §13** — 사용자 지적: *"니가 의심되는 부분에서 스크린샷 찍어서 확인하면 되는거 아냐?"* | 조항(§13) + 구조(`@live-covers` 선언) + 기계(이 게이트). 화면 13개 중 10개가 라이브에 덮이고 3개는 **이유와 함께** 제외. 주입 3종 RED |

**M-0046이 왜 이 저장소에서 가장 비싼 종류였나**: 게이트 **31종·유닛 686건이 전부 초록**인 채로 배포됐다. **자료구조는 옳았다** — 틀린 것은 사용자에게 가는 문장과 버튼뿐이었고, 그 층을 아무도 재고 있지 않았다. 그리고 그 문장은 **사용자에게 파괴적 행동을 권하고 있었다.** 이 부류(§10 ③)는 유닛으로 원리적으로 안 잡힌다 — 유닛은 판정 함수를 재지 *그 판정이 화면에 그려지는지*를 재지 않는다.

**§13이 세워진 이유(가장 크다)**: 나는 §12(*"앱은 사람에게 무엇을 대신 시키고 있는가"*)를 **앱에만 묻고 나 자신에게는 안 물었다.** Playwright·Chromium이 이 환경에 있는데 화면 확인을 사용자에게 시키고 있었다. **실기기가 유일한 층인 것과, 내가 재볼 수 있는 것을 안 재는 것은 다른 이야기다.**

**🔴 아직 확인 안 된 것 (다음 세션이 사용자에게 물어볼 것)**

1. 🔴 **v1.21 실기기 확인이 아직 안 됐다.** 「앱 상태 확인」→ **[서버에 없는 자료 다시 올리기]**를 눌러야 소리 3건의 바이트가 R2로 올라간다. 성공하면 「소리」가 **클라우드 4 · 이 기기 4**로 맞는다(그 지표가 곧 read-back이다).
2. **신규 가입이 열려 있다.** allowlist 밖 계정(`uzdoctorkim@gmail.com`, 2026-07-23 로그인)이 하나 있고 journey 자료 접근은 **0건임을 정책 정의로 확인**했지만, 가입 자체는 막혀 있지 않다. **Auth 설정이라 MCP로 못 한다 — 사용자 콘솔 몫.**
3. **영상 기능**(`docs/VIDEO_PROPOSAL.md`) — 사용자 결정 대기 중(오래됨).
4. **v1.09의 「지금 동기화 중」 문구.** 유닛으로만 확인됐다 — 결함이 아니라 **미관측**이다. 그렇게만 말할 것.

**이 영역을 만지기 전에 반드시 읽을 것**: `diagnostics-dev` **§7-C**(「없다」는 찾아보고 나서 하는 말이다) · `ui-responsive-dev` **§4·§5**(시각 확인 의무 + 새 화면의 라이브 등록) · `gates-mechanization-dev` **§2-G**(선택 게이트 계약). `npm run brief <파일>`이 자동으로 띄운다.

**이 세션에서 가장 크게 배운 것 넷** (다음 사람이 같은 자리에서 넘어지지 않게):

- **「없다」는 찾아보고 나서 하는 말이다.** 진단이 서버만 보고 「자료가 없습니다」라 단정했다. **한 곳을 보고 전체를 말하면, 아직 안 본 곳이 곧 사용자의 기억이다.** 파괴적 행동을 권하기 전에 **가능한 소재지를 전부** 물어라.
- **조건을 베낄 때는 그 조건이 *왜* 참이었는지도 베껴라.** 사진에서 `deletedAt === null`이 안전했던 이유는 사진은 tombstone 전에 반드시 업로드를 거치기 때문이다. 소리는 그렇지 않았는데 조건만 따라왔다.
- **§4 「비공허」는 게이트 자신에게도 걸린다.** 새 라이브 게이트가 첫 판에서 **대상 0으로 공허하게 PASS**였다(빈 배열의 `every`는 true다). 목록 확보를 **먼저 판정**하게 고쳤다.
- **내가 열어볼 수 있는 화면을 사람에게 묻지 마라.** 캡처는 수십 초, 질문은 왕복 수 분이다. 못 열었으면 **「라이브 렌더 미실행」이라고 적는다**(원칙 #4).
- 🔴 **「안 잰 것」을 「문제 없음」이라 말하지 마라**(M-0047 · 근본형). 검사가 *무엇을 못 했는지*를 **판정 항목으로 갖지 않으면** 못 한 것이 통과로 반올림된다. §8이 진단 도구에 걸어 둔 규율인데 **게이트와 문서에는 안 걸고 있었다.** 그리고 셋 다 **전날·당일 내가 만든 것**에서 나왔다 — M-0033의 *"수정 직후가 가장 위험하다"*가 또 맞았다.

**이전 세션(2026-07-27) 요약**: *"건강진단 부탁해요"* → 앱 코드 결함 0, 나온 것은 **검사 체계의 구멍 하나**(유일한 런타임 검출층이 harness·CI 어디에도 없어 사람이 기억해서 돌려야만 돌았다 → 선택 게이트 개념 §2-G + CI `live-render` job)와 **서버 설정 셋**(정책 18개 행별 재평가 → InitPlan 재작성으로 advisor 18→0 · 트리거 함수 EXECUTE 회수 · `SECURITY DEFINER` `search_path` 드리프트 복구). 그때 얻은 넷 — ①「게이트가 다 초록」과 「검사가 다 돌았다」는 다른 말이다 ②**무해한 이탈이 가장 오래 산다**(실害 0은 "고칠 필요 없다"가 아니라 "탐지되지 않는다"는 뜻) ③**advisor 권고문을 그대로 따르지 마라** — 같은 경고에 정반대 처방이 붙는다(`is_allowed()`는 회수하면 앱이 죽는다. 표는 `docs/SECURITY.md` 상단이 정본) ④역사와 현재를 가르지 않는 검사는 오탐으로 죽는다.
...
**하네스 게이트**: SSOT=`scripts/harness.mjs`, 현재 **<!--reg:gateCount-->33<!--/reg-->개**. **개수도 목록도 여기 손으로 적지 않는다** — `src/app/registry.gen.ts`(자동 생성, `check-registry-gen`이 드리프트 차단)에서 파생하고 개발자 정보→설계 개요도/가이드가 그 목록과 한 줄 설명을 그대로 표시한다. 목록을 눈으로 보려면 `npm run harness` 또는 `src/app/registry.gen.ts`를 열면 된다.
...
**검증**: harness <!--reg:gateCount-->33<!--/reg-->/27 · 유닛 410 · verify-editor-live 127/127 · build 그린. **미실행**: 실기기 PWA 설치·실제 로밍 — 사용자 확인 몫.
...
**검증**: harness <!--reg:gateCount-->33<!--/reg-->/26 · 유닛 410 · verify-editor-live 123/123(뷰어 13종·폰트 조각 4종 포함, 실제 Chromium이 `dist` 서빙) · build 그린. **미실행(정직)**: 실기기 터치·네트워크 동기화(샌드박스가 `*.supabase.co` 차단)·PWA 설치 — 여전히 사용자 실기기 확인 몫.
...
npm run harness                        # Required 게이트 전체 (현재 <!--reg:gateCount-->33<!--/reg-->개 — 목록은 scripts/harness.mjs, 손편집 나열 금지 M-0001; 이 숫자는 gen-registry가 자동 갱신·check-doc-counts가 대조)
...
**검증**: 하네스 <!--reg:gateCount-->33<!--/reg-->개 PASS(라이브 2종 포함 — 건너뜀 0) · 유닛 702 · live 165+12 · build OK. **주입 RED**: 미등록 화면 · `scripts/`·`supabase/` 라우트 제거 · 셀프테스트 강제 실패(Required이므로 FAIL, SKIP 아님).
...
하네스 <!--reg:gateCount-->33<!--/reg-->개 전부 PASS(라이브 2종 포함 — 건너뛴 것 없음) · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->33<!--/reg-->개 PASS · 유닛 542건 · **주입 RED 8건**(백업 호출 제거 · `occurred_at`·`taken_at` 되돌리기 · `CHECK_COUNT` 드리프트 · `localeCompare` 복귀 · `withCanonicalStamps` 축소 · `compareInstants` 무력화 · 게이트 자체검사) · build OK · live 136/136(1회 흔들림, 아래 참조).
...
**검증**: 하네스 <!--reg:gateCount-->33<!--/reg-->개 PASS · 유닛 526건 · 옛 로직 주입 **3건 RED** · live 136/136 · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->33<!--/reg-->개 PASS · 유닛 513건(instantFormat 28건 신설) · 주입 RED 3건(rowmap 타입 · merge · integrity) · live 136/136 · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->33<!--/reg-->개 PASS(로컬 + Node 20) · 유닛 PASS(원자성 3건 신설 + 되살리기 3건) · 주입 RED 확인 · live 130/130 · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->33<!--/reg-->개 PASS(로컬 Node + **CI가 쓰는 Node 20**) · 유닛 PASS(`restoreUnpurge` 12건 신설 + 문장 검사 6건) · **주입 검증**으로 가드 제거·되읽기 신뢰·보호 해제 각각 RED 확인 · `verify-editor-live` 130/130 · `npm run build` OK.
