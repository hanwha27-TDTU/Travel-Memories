...
**현재 단계**: **실사용 가능한 개인 여행기록 PWA — v0.47 배포 라이브**(https://hanwha27-tdtu.github.io/Travel-Memories/, GitHub Pages, base=/Travel-Memories/). 아래 "현재 기능 지도"의 기능이 모두 구현·게이트·배포됨. 브랜치 `claude/travel-log-app-r2xd5f`, origin 동기화됨. 버전 SSOT는 `src/app/changelog.ts`(현재 v0.47), 연구노트(사람/AI/결정 해시체인)는 `src/app/researchLog.ts`(seq 1–33).
