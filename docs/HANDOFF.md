...
## HANDOFF-0183 · **한국 좌표 Kakao Maps 배선과 초보자 설정 가이드** (2026-08-13 · 구현·로컬 검증 완료)
...
- 작업 브랜치 `codex/kakao-map-korea`에서 프로덕션 키 배선 build, 제공자 유닛 5건, 빠른 게이트 68개, 편집 라이브 407건과 전체 라이브를 통과했다. Kakao SDK 허용 주소를 일부러 뺀 CSP 대조군은 예상대로 RED였고 복원 뒤 GREEN이다. 운영 Pages에는 아직 배포되지 않았으므로 T-026은 `대기`이고, 실제 Kakao 응답·한글 검색·해외 MapLibre는 다음 릴리스의 운영 확인으로 닫는다.
