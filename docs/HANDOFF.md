...
## HANDOFF-0214 · **빚을 갚으려다 원인 진단이 틀렸던 걸 알았고, 통로를 넓히니 결함 둘이 더 나왔다** (2026-08-15 · T-046 닫음)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- 🔴 **T-046의 원인 진단이 처음부터 틀렸다.** 「생성기가 자체검사 픽스처를 흘린다」가 아니라 `check-current-doc-facts.mjs`의 **JSDoc 산문**(과거 결함을 설명하며 없는 이름을 일부러 인용)을 생성기가 표의 설명 칸으로 옮긴 것이었다. 원인을 잘못 짚어 처방이 **「통로를 닫는 것」**이 됐고, 그 통로는 전날 코드 주석 화석을 잡아 준 자리였다(§8 · M-0056).
- **해결**: 「일부러 없는 것」을 **경로로 등록**(`INTENTIONALLY_ABSENT` 3건 · 이유 필수)해 원본에서 한 번 정하면 생성물이 따라오게 했다 → **생성물 제외를 걷어냈다.**
- 🔴 **그리고 통로를 넓혔다: 문서 99 → 문서·소스 356.** `src/`·`scripts/` 주석까지 훑는다. 코드 주석이 원본이라 **고칠 자리를 정확히 가리킨다.**
- 🔴 **넓히자마자 결함 둘이 나왔다(M-0169)**: `time.ts`가 없는 게이트를 「강제한다」고 적었고(규율은 `check-timezone`이 실제로 강제 중 — 이름표만 틀림), `tripDetail.ts`가 없는 게이트를 「RED로 잡는다」고 적었다. **후자는 내가 v2.41에서 쓴 문장**이고 실제 보호층은 라이브 검사였다. **M-0051을 고치려고 만든 게이트가 내가 저지른 M-0051을 잡았다.**
- 🔴 **어제의 「전수조사」는 모집단이 반쪽이었다** — 문서만 보고 소스 주석을 안 봤다. 조사 자체가 §4의 모집단 문제에 걸린 형태다.
- **검사**: `gates` exit 0(70종) · 자체검사 통과 · **새 통로에 주입해 RED 확인 후 복원**(§4) · `tsc` exit 0.
- 🔴 **남는 대가**: `INTENTIONALLY_ABSENT`는 **경로 단위**라 등록된 이름은 **어디에 적혀 있어도 통과한다.** 목록을 짧게 유지하고 이유를 적는 것 말고 방어가 없다.
- **다음**: 사용자가 고른 **문서↔코드 모순 조사**(§17 ④축 — 「가리키는 것이 있는가」가 아니라 「적힌 규칙이 코드에서 참인가」) · T-034 · T-028.

---

## HANDOFF-0213 · **계약 전수조사 — 60건 중 진짜는 4종이었고, 그 4종이 컸다** (2026-08-15 · 사용자 지시)

- **사용자 지시**: *"현재 우리앱 계약현황을 전수조사해서 잘못되어있는건 바로잡고 불필요한건 제거하고..하면 어떨까?"*
- 🔴 **「제거」에 대해 먼저 의견을 냈다.** 이 저장소는 *"중복 같은데?"* 한 줄로 사라질 뻔한 것을 겪었고(M-0029), 반대로 없는 게이트를 있다고 적은 문서가 반년치 코드를 통과시킨 적도 있다(M-0051). 그래서 **①명백한 사실 오류(내가 고침) ②없애도 되나(사용자 결정)**로 나눴고, 사용자가 「정직하게 갈라 적기 + 게이트로 기계화 + 문서↔코드 모순」을 골랐다.
- **모집단**(손으로 세지 않음): 문서 **99** · 게이트 **69** · migration **32** · RLS 공격검사 **8** · 스킬 **13** · 에이전트 **28**.
- 🔴 **가장 무거운 것(M-0168)**: `docs/SECURITY.md` 절 제목은 「게이트 **후보**」인데 첫 줄이 **「…에 구현:」**이었고, 나열된 아홉 중 **여섯이 저장소 어디에도 없었다.** 형제 문서 셋(`SYNC_PROTOCOL`·`MEDIA_PIPELINE`·`DATA_MODEL`)은 **이미 정직한 문형**을 쓰고 있었다 — 이 문서만 달랐다(§7).
- **함께 나온 셋**: `PRIVACY.md`의 EXIF whitelist 현재형(형제는 「미구현」으로 알고 있었다) · `naming.ts` 주석이 없는 테스트 파일 지목(대조는 실제로 돎) · `DEPLOYMENT.md`의 없는 빌드 스크립트.
- **고친 방식**: **없앤 것은 없다.** 넷 다 「지금 도는 것」과 「후보/목표」로 **갈라 적었다** — 이름을 지우면 *무엇을 안 만들었는지*도 함께 사라진다.
- **기계 층**: `check-doc-references` 신설(게이트 69→70). 경로·`npm run`·게이트 이름이 실재하는지 매번 재고, 후보는 **이유와 함께 등록**해야 통과한다. 🔴 부산물 — **「있다고 말만 하고 안 만든 것」 10건이 한 곳에 모였다**(예전엔 문서 넷에 흩어져 아무도 세지 못했다).
- 🔴 **§4 주입에서 내 게이트가 한 번 공허했다**: 첫 주입 이름이 한글이라 정규식이 **아예 안 읽고 초록**이 나왔다. ASCII로 다시 주입해 **세 축 전부 RED** 확인 후 복원했고, **그 한계를 게이트 머리말에 적었다.**
- 🔴 **정직하게 — 첫 판 60건 중 실제 결함은 4종.** 나머지는 확장자 생략 19건·v0.2 보관본·역사 문서·`check-known-index`(정상 실행 중), 그리고 *"`config.toml`이 **없다**"*고 **정확히 말하던** 문서까지 내가 결함으로 셌다. 오탐을 안 걷어냈으면 **멀쩡한 것을 「정리」할 뻔했다.**
- **검사**: `gates` exit 0(70종) · 자체검사 통과 · 유닛 **1727/1727**.
- 🔴 **남긴 빚**: `check-doc-references`가 지금 **생성물 설계서를 안 본다** — 생성기가 자체검사 픽스처 문자열을 흘리기 때문이다(T-046). 그 통로로 코드 주석 화석을 한 번 잡았으므로 **값이 있는 통로를 닫아 둔 상태**다.
- **다음**: 사용자가 고른 **문서↔코드 모순 조사**(§17 ④축 — 「가리키는 것이 있는가」가 아니라 「적힌 규칙이 코드에서 참인가」. 자동화가 안 되고 하나씩 읽어야 한다) · T-046 · T-034 · T-028.

---

## HANDOFF-0212 · **타임라인 순서 토글과 사람 단위 동행인 — 그리고 T-044 결정** (2026-08-15 · 사용자 지시)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0` · **상태**: 구현됨·릴리스 대기(§15 축적).
- **사용자 지시 셋을 받았고 하나는 되물었다.** *"day 1부터인데 최신 순으로 하는데 어떨까?"* — 🔴 **그냥 뒤집는 것에 반대 의견을 냈다**: 기록 중과 회고 중이 정반대 순서를 원하고, 앱은 이미 여행 상태를 안다. 사용자가 **「상태로 기본값 + 토글」**을 골랐다.
- **새 파일**: `domain/moment/timelineOrder.ts`(순수 판정) · `domain/moment/companions.ts`(사람 쪼개기·집계) · `ui/screens/companionRecords.ts`(「이 사람과 함께한 기록」) · 유닛 2종 27건.
- **바꾼 것**: 타임라인 순서 토글(날짜와 **그 안의 순간까지** 함께 뒤집는다 — 한 층만 뒤집으면 최신 순간이 맨 위 묶음의 맨 아래에 앉아 목적이 무너진다) · 동행인 입력을 **토큰(한 명씩)**으로 · 순간 카드에 **사람마다 칩 하나** · 칩을 누르면 그 사람의 순간·사진 창.
- 🔴 **결정 둘을 ADR로 남겼다**: **ADR-0074**(인물 원장을 만들지 않는다 — 새 형제의 상속 비용이 크고 화면에서 얻는 것은 같다. 대신 「이름이 비슷하면 같은 사람일 수 있다」를 **단정하지 않고** 화면에 적는다) · **ADR-0075**(T-044 — 사진 칸 위 ✕는 현 크기 유지. 넓히면 **사진을 보려던 손이 사진을 지운다**).
- 🔴 **게이트가 다섯 번 막았고 전부 진짜였다**: `check-instant-normalization`(새 화면만 `localeCompare` — M-0034를 안 물려받음) · `check-fn-size` 둘(우회 대신 `momentCard`·`orderBar`·`convertedTotalStat`를 덜어냈다 — **게이트가 설계를 밀어줬다**) · `check-live-coverage`(새 화면에 눈이 없다) · `check-module-design-docs`. **§7 최빈형이 오늘만 다섯 번째다.**
- 🔴 **라이브가 진짜 결함을 잡았다**: 동행인 입력을 `blur`에서 확정했더니 **[저장]을 누르는 순간 칩이 생겨 버튼이 손가락 밑에서 아래로 밀렸고, 클릭이 아예 안 일어났다**(M-0162와 같은 식구). 확정을 Enter·쉼표·`getValue()`로만 두어 고쳤다 — 적다 만 이름은 `getValue()`가 집어 가므로 **유실은 없다**.
- 🔴 **내 검사가 공허했고 스스로 잡았다**: 순서 토글 검사가 처음엔 날짜 묶음 **1개**에서 돌아 **뒤집기가 고장 나도 초록**이었다(§4). 다른 날 순간을 심어 모집단을 만들고 **모집단 판정을 먼저** 넣었다. 그 과정에서 `reload()`가 URL 목표를 다시 읽어 사진 뷰어를 열고 클릭을 가로채는 것도 실측으로 알았다.
- **검사**: `gates` exit 0 · 신규 유닛 27건 · `build` exit 0 · `verify-editor-live` **469/474**(남은 5는 지도 키 결손). 🔴 1회차에 `R⑧`이 한 번 빨갰다가 재실행에서 통과 — **M-0164가 지목했던 세 자리 중 하나가 아직 흔들린다**(내 변경 탓이 아님을 재실행으로 갈랐다).
- 🔴 **정직한 한계**: 화면을 **눈으로는 보지 않았다**(라이브 렌더로 재기만 했다). 좁은 폭(344px)에서 사람 칩이 여러 개일 때의 줄바꿈도 **안 쟀다** — T-043에서 만든 `coverSweep`이 이 화면을 아직 안 지난다.
- **다음**: 위 두 가지 실측 · T-034 · T-028 · T-036/T-038(사용자 몫).

---

## HANDOFF-0211 · **흔들린 검사를 고쳤다 — 원인은 못 찾았고, 대신 다음 실패가 말을 하게 만들었다** (2026-08-15 · T-045 닫음)

- **브랜치**: `claude/constitution-rules-compliance-4m7gp0`(v2.43 머지 뒤 `origin/main`에서 다시 세움) · **상태**: 구현됨·릴리스 대기(§15 축적).
- **바꾼 파일**: `scripts/verify-editor-live.mjs`(read-back 한 자리) · `docs/BACKLOG.md`·`records/coding-mistakes.md`(M-0167) · 생성문서.
- **확인된 결함 둘을 고쳤다**(코드에서 읽어 확정한 것만):
  - ① 🔴 **폴링 안에서 IndexedDB를 여는 유일한 자리였고 연결을 닫지도 않았다.** 형제 20여 곳은 전부 `evaluate` 한 번 안에서 연다 — 여기만 rAF 폴링마다 새 연결을 열어 그대로 뒀다(§7 최빈형).
  - ② 🔴 **실패하면 `null` 한 글자만 남았다.** 「값이 달랐다」와 「읽지 못했다」를 같은 말로 적은 것이라(§8) CI가 빨개져도 **다음에 할 일이 정해지지 않았다.**
- **수정**: `waitForFunction` → `waitUntil`(T-042가 세운 자리 · **던지지 않고 마지막 값을 돌려준다**) · `db.close()` · 반환을 `{read,why}`와 `{read,place,moment}`로 갈라 「읽지못함: 사유」를 별도 문장으로. 🔴 **기다림의 조건과 판정의 조건을 분리**해, 조건이 틀려도 판정은 현실을 다시 읽는다.
- **§4 주입 3종 전부 실행**: Ⓐ「읽지 못함」→ RED(사유 표시) · Ⓒ「대기 조건이 틀림」→ **정상 통과**(분리가 작동한다는 증명) · Ⓓ「옛 이름」→ 값과 함께 RED. 그 뒤 **연속 2회 458/463 동일**(남은 5는 지도 키 결손).
- 🔴 **조사 중 실증한 것**: `waitForFunction` timeout은 **스크립트를 통째로 죽인다**(주입 Ⓑ가 다른 자리에서 `TimeoutError`로 프로세스를 끝냈다). 그러므로 CI의 `FAIL … null`은 **timeout이 아니었다.**
- 🔴 **정직한 한계 — 근본 원인은 확정하지 못했다.** 로컬 5회 모두 통과라 재현하지 못했고, ①이 원인이라는 것도 **그럴듯할 뿐 확인된 것이 아니다**(§8 · M-0056). 한 일은 「원인을 고쳤다」가 아니라 **「원인이 무엇이든 `null` 한 글자로 끝나지 않게 만들었다」**이다. **재발 여부는 다음 CI가 판정한다.**
- **다음**: T-044(**사용자 결정 필요** — 형제 셋) · T-034 · T-028 · T-036/T-038(사용자 몫).

---

## HANDOFF-0210 · **v2.43 배포 — 재개방 0회, 세 표면 그린. 그리고 CI가 흔들림을 하나 드러냈다** (2026-08-15)

- **릴리스 묶음: 검증 1회 + 재개방 0회**(§18-H). v2.41·2.42에 이어 **세 판 연속 0회**다. 순서를 지켰다: 버전·CHANGELOG 확정 커밋 → `release:arm` → build → harness → PR → 그린 → 머지 → 배포 → `release:close`.
- **담긴 것**: T-043(폴드 커버 5화면 측정 · 결함 5건 수리 · M-0165)과 T-033(초대제 게이트 3갈래 · ADR-0073 · M-0166). T-037은 crates.io 조회로 **선행조건 미충족을 재확인만** 했다(tauri 최신 안정판이 여전히 2.11.5라 `glib`은 0.18.5 그대로).
- **squash 머지**: **`d77b6669`** · PR **#288**.
- **배포 그린 셋**:
  - **Pages run `31883160319` success** — build · 동기화 계약 대조 · **시크릿 스캔** · deploy 전부 초록.
  - **CodeQL(`Push on main`) run `31883160399` success**.
  - **Windows Installer run `31883160309` success** — 🔴 **NSIS가 약 21분** 걸렸다(v2.42는 5분 35초). Rust 캐시 미스로 전체 재컴파일한 것으로 보이며 **실패는 아니었다**. 「고정 다운로드 주소 갱신 후 **다시 받아 검증**」까지 초록.
- 🔴 **로컬에서 빨갛던 5건은 CI에서 전부 초록이었다.** 지도 키(Kakao·TomTom)가 Repository Variable로 붙기 때문이다 — 「샌드박스 환경 탓」이라는 판단이 실측으로 확인됐다.
- 🔴 **CI가 흔들림을 하나 드러냈다 → T-045로 등록**. 같은 커밋에서 `live-render`가 **1차 462/463 · 재실행 463/463**이었고, 빨간 항목은 `v1.97 연결 이름 … — null`이다. §18-B에 따라 **실패한 그룹만** 재실행했으므로 새 커밋도 재개방도 없다. §21 ③(강행의 대가는 기록)에 따라 다음 판 과제로 올렸다.
- 🔴 **틀린 근거를 하나 세울 뻔했고 정정했다.** 처음엔 *"`harness`는 초록인데 `live-render`만 빨갛다"*를 흔들림의 증거로 쓰려 했다. **그건 거짓이다** — `harness` job은 Chromium을 설치하지 않아 그 검사를 **SKIP**한다(§2-G: SKIP은 통과가 아니다). 워크플로 파일을 읽고 정정했고, 실제 근거는 **재실행 초록** 하나뿐이다.
- 🔴 **여전히 못 잰 것**: 운영 `version.json` 직접 read-back(샌드박스가 `*.github.io` 403) · APK에서 갤러리가 실제로 열리는지 · **실기기 비행기모드에서 로그아웃되지 않는지**(라이브가 OAuth 세션을 못 만든다) · 사진 편집기·칩·홈의 344px 화면(재기만 했고 눈으로 안 봄).
- **다음**: T-045(라이브 흔들림) · T-044(**사용자 결정 필요** — 형제 셋) · T-034 · T-028 · T-036/T-038(사용자 몫).
...
- **세 자리를 나란히 놓자 형태가 같았다**: `await settle(page)` 직후에 `isVisible()`·`count()`로 즉시 판정. `settle`은 **rAF 두 번**일 뿐이고, 🔴 **`isVisible()`·`count()`에는 자동 대기가 없다** — 「곧 나타날 것」을 물으면 false를 사실로 받아 적는다. 그리고 그 뒤 클릭이 timeout으로 죽으면 **뒤따르는 검사가 통째로 사라진다**(1회차가 그렇게 죽었다).
- 🔴 **M-0119가 이미 적은 규율의 재발이다**(*"시간이 아니라 내용을 기다림"*). T-018이 고정 대기 126곳을 0으로 만들었지만 `settle`은 「고정 대기」로 세어지지 않아 **그 청소를 통과했다** — 규율은 남고 형태만 바뀌었다(§7 최빈형).
- **수정**: 「홈 트리 ✕ 해제」·「R⑧ 재배열 직후 사진 뷰어」는 `waitUntil(조건)`, 「휴지통 일괄삭제」는 `waitFor({state:'visible'})`. 수정 뒤 **2회 연속 434/439**(지도 키 결손 5건만)로 세 자리 모두 나지 않았다.
- **근본형을 헌장에 표로 적었다**(`ui-responsive-dev` §3-C-2): 무엇이 무엇을 기다리는지 · 언제 이른지 · 갈림 기준(「이 결과가 비동기를 거치는가」). `settle`을 다 없애라는 뜻이 아니라는 것도 함께 적었다 — 동기 렌더에는 충분하고 실제로 100곳 가까이 그렇게 쓴다.
- 🔴 **「흔들림이 사라졌다」고 말하지 않는다.** 흔들림은 정의상 가끔 나므로 2회 통과가 부재의 증거는 아니다. 말할 수 있는 것은 「이르게 재던 세 자리를 고쳤다」까지이고, T-042는 그 기준으로 닫았다.
- 검증: 라이브 2회 434/439, 빠른 게이트 69/69, 유닛 1,692/1,692, typecheck. 버전은 올리지 않았다(§15 축적).

---

## HANDOFF-0204 · **터치 표적 — 셋은 고치고, 둘은 「안 넓히기로」 결정했다** (2026-08-15 · T-041)

- v2.41 배포 뒤 T-041(형제 ✕ 다섯)을 이어서 진행했다. 방법은 T-035·M-0162가 세운 계약이다 — **보이는 크기는 그대로 두고 히트 영역만 넓히되, 「넓혔다」와 「아무것도 안 덮는다」를 함께 잰다.**
- **닫기 셋**(`.pe-close` 34 · `.map-close` 36 · `.single-photo-moment-close` 38)에 같은 규칙을 적용했다. `.map-close`는 라이브가 지도 모달을 열므로 **실측했다** — 보이는 36×36 유지 · 히트 **46×46** · 덮는 것 **0**. 🔴 나머지 둘은 **현재 라이브가 그 모달을 열지 않아 재지 못했다.** 같은 구조라 안전할 것으로 보지만 추측이므로 완료라 하지 않고 코드와 T-041 양쪽에 적었다.
- 🔴 **사진 위 ✕ 둘은 넓히지 않기로 결정했다**(`.pick-x` 22 · `.photo-del` 26). 64×64 썸네일 셀 **위에 얹힌 삭제 버튼**이라 44px로 키우면 셀의 절반 이상을 덮어 **사진을 누르려던 손이 사진을 지운다** — 미달보다 나쁘다(`.chip-x`가 같은 함정을 겪고 주석에 남겼고 M-0162에서 재현됐다). 대체 경로로 [전체 해제](44px)가 있다. 근본 처방(칸을 키우거나 삭제를 칸 밖으로)은 배치 결정이라 T-041에 남겼다 — **추측으로 정하지 않는다.**
- **§7 3층을 놓았다**: 라이브가 344px 홈의 조작 요소를 **전수로** 훑어 44px 미달을 이름과 함께 잡는다. 모집단 확보를 먼저 판정하고(9개 — 0이면 아무것도 안 본 것이다), 오탐 둘을 걷어낸다(부모가 클릭을 받는 자식 · `::after`로 넓힌 히트 영역). 예외 둘은 **이유와 함께 코드에 등록**했다.
- 🔴 **T-042의 흔들림을 한 번 더 관측했다.** 「R⑧ 재배열 직후 사진 뷰어」가 한 번 FAIL했다가 **같은 코드로 재실행하니 PASS**했다(432/437). 내 변경 탓이 아님을 대조로 확인했고, 이것이 T-042의 세 번째 사례다.
- 검증: 편집 라이브 **434/439**(지도 키 결손 5건만 실패 · 신설 2건 포함), 빠른 게이트 69/69, 유닛 1,692/1,692, typecheck, build. 버전은 올리지 않았다(§15 축적).

---

## HANDOFF-0203 · **v2.41 배포 — 재개방 0회, Pages·Windows 그린** (2026-08-15)

- 릴리스 묶음: **검증 1회 + 재개방 0회**(§18-H). v2.40에서 순서를 틀렸던 것(버전 커밋을 arm 뒤에 함)을 이번엔 지켰다 — 버전 확정 커밋 → arm → build → harness 순서.
- 담긴 것: 갤러리 단추 수정과 첨부 버튼 얼굴 통일(M-0163) · 폴드 커버 실측 Phase 1–2 · 좁은 폭 헤더 배치 · **헤더 터치 표적 되살림**.
- 🔴 **터치 표적이 배치를 고친 뒤에야 성립했다.** HANDOFF-0201에서 되돌렸던 처방(제목·배지의 세로 히트 확장)을 헤더를 세 줄로 정리한 뒤 다시 넣으니 **344·360·412·768px 전부 겹침 0**이었다. 즉 **배치가 터치 표적의 전제**였고, 그 순서를 실측이 가르쳐 줬다.
- 새 라이브 검사 9건: 폴드 커버 344px(생성 폼 3버튼 · 편집 폼 4버튼) 4건 + 좁은 헤더 5건. 주입 2방향 RED 확인 — 배치 분리를 지우자 배지가 다른 줄로 내려가고(`verTop 50` vs `titleTop 16`) **겹침이 정확히 재현**됐다(`stolen: [app-version, app-title]`).
- 로컬 하네스는 `verify-editor-live` 1개 FAIL(지도 키 결손 5건, T-036·T-038), 나머지 전부 PASS. **CI에서는 키가 있어 Required 둘 다 success**(run `31874375689`). squash 병합된 `main`은 **`51f136df`**다.
- 배포 그린: **Pages run `31874488418` success**(build + deploy + 시크릿 검사) · **Windows Installer run `31874488403` success**(NSIS 5분 26초 · 「고정 다운로드 주소 갱신 후 **다시 받아 검증**」까지 통과).
- 🔴 **운영 `version.json` 직접 read-back은 이번에도 못 했다** — 샌드박스가 `*.github.io`를 403으로 막는다. 내가 잰 것은 「배포 워크플로가 성공했다」까지이고, 「사이트가 v2.41을 준다」는 사용자 실기기 몫이다.
- 🔴 **APK에서 갤러리가 실제로 열리는지도 못 쟀다** — 셸 브리지가 이 환경에 없다. 새 APK 뒤 실기기 확인이 필요하다.
- 남은 과제: T-041(형제 ✕ 다섯) · T-042(라이브 흔들림) · T-043(아직 못 잰 화면들 — 오버레이 내부·여행 상세·편집기·지도).

---

## HANDOFF-0202 · **좁은 폭 헤더 배치 — 다섯 줄을 세 줄로** (2026-08-15)

- 사용자 실기기 지적: *"제목 버전배지 동기화됨 그리고 이메일 배치를 이쁘게 안 될까? 홈화면에서."* 스크린샷의 헤더는 제목이 「Bugeon / Journey」로 갈라진 두 줄, 그 아래 배지, 그 아래 상태 줄, 오른쪽 위에 잘린 이메일이었다.
- **원인 둘.** ①`@media (max-width: 400px)`가 제목을 **오히려 키우고 있었다**(1.2rem → 1.25rem) — 좁을수록 크게 만들면 갈라질 확률만 오른다. ②제목 줄(`flex: 1 1 0`)과 계정 영역이 **같은 줄을 다투다** 제목 칸이 좁아졌고, 그 결과가 낱말 단위 줄바꿈이었다.
- **처방**: 같은 400px 분기 안에서 줄을 다투지 않게 나눴다 — 위는 제목+배지, 아래는 계정 한 줄(이메일 왼쪽·로그아웃 오른쪽·세로 가운데), 그 아래 상태 한 줄. 🔴 **새 breakpoint를 만들지 않았다.**
- **실측(344/360/412px · 계정 영역을 실제 `.auth-area` 안에 심어 로그인 화면을 재현)**: 헤더 높이 **283 → 268px**(15px 절약) · 이메일 **78px 잘림 → 222px 전체 표시** · 가로 넘침 0 유지. **412px 이상은 281px 그대로**라 기존 폰·태블릿·데스크톱 디자인은 바뀌지 않았다. 캡처로 제목 1줄·배지 나란히·상태 1줄을 눈으로 확인했다.
- 🔴 **측정에서 두 번 헛디뎠고 둘 다 고쳤다.** ①계정 영역을 **덧붙였더니** 로그인 안 된 상태의 「로컬 모드」 줄과 겹쳐 **실기기에 없는 줄**을 재고 있었다(§3-E — 내가 만든 화면을 재는 셈) → 기존 `.auth-area`의 내용을 **교체**하는 방식으로 바꿨다. ②제목 줄 수를 `getClientRects().length`로 쟀는데 **버튼은 블록이라 줄이 갈라져도 rect가 1개**다 — 그래서 「1줄」이라 보고했고 캡처를 열어 보고서야 2줄인 걸 알았다. 높이÷줄높이로 바꿨지만 `line-height: normal`이라 아직 `NaN`이다(캡처가 판정층이다).
- 🔴 **영구 검사가 아직 없다.** 이 배치는 로그인 상태가 필요해 현재 라이브 시나리오에 그대로 얹기 어렵다 — **T-043에 묶어** 계정 영역 픽스처와 함께 붙인다. 그때까지 이 자리는 조항과 캡처뿐이고, 되돌아감을 막는 층은 없다.
- 검증: 빠른 게이트 69/69, 유닛 1,692/1,692, typecheck, production build. 버전은 올리지 않았다(§15 축적).

---

## HANDOFF-0201 · **폴드 커버 폭 실측 Phase 1–2 — 그리고 처방이 실패한 자리** (2026-08-15)

- 사용자 요청은 둘이었다: ①[🖼️ 갤러리에서] 결함 수정 ②Fold 커버 화면 적응형 최적화(**분석 보고 먼저**). ①은 HANDOFF-0200 뒤 커밋 `ce9f73f`로 처리했고(M-0163), 이 항목은 ②의 Phase 1–2다.
- **Phase 1(구조 파악)**: 이 앱은 sidebar·bottom nav가 없고 라우트가 둘뿐이며(홈·여행상세) 나머지는 오버레이다. `app.css`에 폭 분기가 **이미 9개**(400·420·520·560·620·900·1024·1100·1600) 있어 **새 breakpoint를 늘리지 않기로** 했다. 고정 열 grid는 5곳 남았고 그중 `.emo-row`(5열)·`.place-actions`(3열)이 좁은 폭 후보다.
- 🔴 **어제 넣은 `auto-fit`이 620px 이하에서는 효과가 없었다** — `@media (max-width: 620px)`가 `.moment-addphoto`를 2열로 덮고 있었다. 즉 첨부 버튼 어그러짐을 실제로 고친 것은 열 수가 아니라 **마크업 구조 통일**(아이콘·글자 span 분리)이었다. 주입 RED가 안 났던 이유도 이것이다.
- **Phase 2(실측)**: 344·360·412·768px에서 홈을 재니 **가로 넘침 0 · 가로스크롤 상자 0 · 글자 갈라짐 0 · 겹침 0 · 콘솔 오류 0**이었다. 남은 것은 **터치 표적 2종**(`.app-title-home` 182×24 · `.app-version` 51×24).
- 🔴 **그 2종에 T-035의 처방을 적용했다가 되돌렸다.** 세로로만 히트 영역을 넓혔더니 **344·360px에서 둘이 서로의 터치를 훔쳤다**(그 폭에서는 세로로 쌓인다 · 412px 이상은 정상). M-0162의 함정이 **세로 방향으로 재현**된 것이다. 겹침은 미달보다 나쁘므로 되돌리고, 이유를 `app.css` 그 자리에 적고 T-041에 실측값과 세 갈래 선택지를 남겼다 — 추측으로 고르지 않는다.
- **오탐 하나를 걷어냈다**: `.sync-note-go`(8×12)는 미달이 아니다. 클릭은 부모 줄(`min-height: 44px`)이 받고 글리프는 표시일 뿐이며, 그 설계가 `dom.ts` 주석에 이미 적혀 있었다. 조사 도구가 부모의 클릭 가능성을 보도록 고쳤다(§11 ③ — 오탐도 결함).
- 🔴 **정직한 경계 — 못 잰 것이 더 많다.** 「데이터 관리」 오버레이는 **열지 못했고**, 가이드는 배경까지 함께 재서 안쪽만 좁혀 보지 못했다. 여행 카드가 있는 홈·여행 상세·타임라인·순간 폼 전체·사진 편집기·지도는 아직 안 쟀다. 그래서 이번 초록은 「홈은 괜찮다」까지이지 「커버 화면이 괜찮다」가 아니다. **T-043**으로 열었다.
- 버전은 올리지 않았다 — §15에 따라 다음 기능 릴리스에 묶는다.

---

## HANDOFF-0200 · **v2.40 배포 — Pages·Windows 그린, 운영 사이트 read-back은 샌드박스가 막았다** (2026-08-15)

- 릴리스 묶음: **검증 1회 + 재개방 1회**(§18-H). 재개방 사유는 *"버전 확정 커밋을 arm 전에 하지 않아 되연다"* — 내가 순서를 틀린 것이고, 되연 사이에 코드는 바꾸지 않았다(문서·버전 커밋만). 다시 얼린 뒤 build → 전체 하네스를 처음부터 다시 돌렸다.
- 로컬 전체 하네스는 `verify-editor-live` 1개 FAIL(그 안의 5건은 **전부 로컬에 지도 키가 없어** MapLibre로 폴백해 나는 T-036·T-038 환경 결손), 나머지 게이트 전부 PASS. **판정은 CI가 했다.**
- PR #285의 Required CI(run `31860948902`)는 **`harness` success · `live-render` success**이고 `fast-gates`·CodeQL 5종도 success였다. squash 병합된 `main`은 **`60223b9f`**다.
- 배포 그린: **Pages run `31861069048` success**(03:09:19→03:09:54, run_number 316) · **Windows Installer run `31861069040` success**(03:09:19→03:15:42). Windows는 9단계 「고정 다운로드 주소 갱신 후 **다시 받아 검증**」까지 통과했다 — 올린 파일을 되받는 read-back이다.
- 🔴 **운영 `version.json` 직접 read-back은 하지 못했다.** 이 샌드박스의 프록시가 `*.github.io`를 `CONNECT tunnel failed, 403`으로 막는다. 그러므로 **「배포 워크플로가 성공했다」까지가 내가 잰 것**이고, 「사이트가 v2.40을 준다」는 사용자 실기기 몫이다(헌법 「완료의 정의」의 그 조항 그대로). 워크플로 성공을 사이트 확인으로 반올림하지 않는다(§18-A 대리 지표 금지).
- 이번 판에 담긴 것: T-031(죽은 라우트 셋 · M-0161)과 T-035(위치 해제 ✕ 터치 표적 · M-0162), 그리고 BACKLOG의 T-040 stale 행 정리. 새로 연 과제는 T-041(형제 ✕ 다섯의 터치 표적)·T-042(`verify-editor-live` 흔들림 재발).
- 다음 착수 우선순위: T-042(흔들림은 다음 릴리스의 판정을 흐린다) → T-041 → T-033.
...
- Windows는 GitHub Pages를 원격 로드하지 않고 상대 base의 `windows-dist/`를 Tauri에 묶는다. identifier `app.bugeon.journey`, `https://tauri.localhost`, main 창, `core:default` 최소 capability, NSIS를 고정했다.
- `nativePlatform.ts`가 브라우저·Android·Windows 판정을 한 곳에서 제공한다. 기존 Capacitor 플러그인은 Android 전용으로 남기고 Windows를 설치된 데스크톱 표면으로 진단한다.
- `windows-tauri-dev` 프로젝트 스킬과 `check-windows-shell` 게이트를 추가했다. Windows Rust/config 파일도 착수 라우팅과 모듈 설계 생성의 모집단에 들어간다.
- 로컬 도구체인에서 `cargo check`, 상대 base Windows 웹 빌드, Tauri release와 NSIS 포장이 통과했다. 개발용 설치 파일은 `Bugeon Journey_2.36.0_x64-setup.exe` 4,486,204바이트, SHA-256 `885999B8A77D65EAA57EE6107D722BD69F134669A7A64769D3A9948897D11996`다. 이 로컬 빌드에는 운영 지도 키를 넣지 않았으므로 배포물이 아니라 컴파일·포장 증거다.
- `useHttpsScheme`를 실제로 `false`로 주입하자 새 게이트가 exit 1로 RED, 백업 원복 뒤 GREEN이었다. 빠른 게이트 69개와 Required 전체 하네스가 통과했고, 테스트 전용 지도 키 빌드에서 편집 라이브 414/414·진단 76/76·로그아웃 잠금 14/14가 통과했다.
- 정직한 경계: 시스템 브라우저 OAuth, Kakao·TomTom Windows 오리진 등록과 실제 WebView2 지도, 백업 파일 선택기, 설치 뒤 저장소 read-back은 다음 단계다. 이 증거 전에는 Windows 완성·배포라고 하지 않는다.

## HANDOFF-0186 · **TomTom 도메인 보호 실증·현재 UI 가이드 교정** (2026-08-13 · v2.36 릴리스 후보)

- 사용자가 MyTomTom에서 `hanwha27-tdtu.github.io`, `localhost`, `127.0.0.1`을 각각 추가하고 `Domain whitelisting`을 저장했다. 키 값은 출력하지 않은 채 같은 타일 요청을 대조해 허용 GitHub Pages origin은 PNG 28,527바이트·HTTP 200, 허용하지 않은 `example.com`은 HTTP 403임을 확인했다.
- v2.35 가이드는 공식 문서의 쉼표 구분 설명을 따랐지만 현재 MyTomTom 편집 화면은 주소를 하나씩 입력하고 `+`로 목록에 넣는 UI였다. 실제 화면에서 세 번째 주소 옆 `+`가 아직 미추가 상태이고, 추가된 행은 휴지통 버튼으로 바뀌며, 마지막 저장 버튼 이름은 `Edit key`임을 확인했다.
- 앱 가이드·배포 문서·라이브 판정을 현재 UI 순서로 교정했다. 과거 HANDOFF-0185의 쉼표 형식은 당시 안내 기록이며 현재 절차의 권위는 이 항목과 `docs/DEPLOYMENT.md`다.

## HANDOFF-0185 · **v2.35 TomTom 운영 키 등록·초보자 가이드 확정** (2026-08-13 · 릴리스 후보)

- 사용자가 MyTomTom의 기존 `My First API key`에서 실제 Key와 고객센터용 ID를 구분해 GitHub Repository Variable `VITE_TOMTOM_API_KEY`로 등록했다. 키 값은 채팅·문서·로그에 노출하지 않았다. 사용자 화면에서 `Map Display API` 포함과 GitHub 변수 이름 존재를 확인했다.
- 앱 가이드와 `docs/DEPLOYMENT.md`는 사진 속 화면부터 `… → Edit → Domain whitelist Off→On → Save Key → 실제 Key 복사 → GitHub Variables` 순으로 다시 썼다. TomTom 공식 문서의 쉼표 구분 도메인 형식에 맞춰 `hanwha27-tdtu.github.io,localhost,127.0.0.1`을 안내하고, `https://`·포트·경로·Supabase·Redirect URI가 필요 없다고 명시했다.
- 가이드 라이브는 정확한 문구와 공식 링크 네 개를 실제 화면에서 열어 414/414 PASS했다. 빠른 게이트 68개도 PASS했다. v2.35 전체 하네스·Required CI·Pages 배포·운영 `version.json` read-back과 우즈베키스탄 실기기 실제 TomTom 타일·팬·핀치는 다음 릴리스 단계에서 판정한다.

## HANDOFF-0184 · **현재 위치 기반 Kakao·TomTom 지도 선택 구현** (2026-08-13 · 키 설정 대기)

- 새 위치 지도에서 현재 위치를 읽지 않아 한국에서도 OSM이 열리던 원인을 확인했다. 저장 좌표 우선, 새 위치는 현재 위치를 초기 중심으로 쓰되 사용자가 지도를 누르기 전에는 선택·저장하지 않도록 중심과 마커 계약을 분리했다.
- 표시 제공자는 한국 Kakao, 확인된 국가 코드 UZ·KZ·KG TomTom Orbis Raster v2, 나머지 MapLibre다. 지역 지도 키 없음·타일 실패·나라 판별 실패는 기존 MapLibre로 닫힌다. 저장 장소의 국가 코드를 여행 지도 포인트에도 전달한다.
- `VITE_TOMTOM_API_KEY`를 env 예시·CI·Pages 배포에 배선하고 `api.tomtom.com` CSP를 img/connect에 함께 추가했다. DEPLOYMENT에는 TomTom 계정→Map Display 전용 키→도메인 제한→GitHub Variable→재배포→눈 확인 순서를 쉬운 말과 공식 링크로 적었다. 지도 버튼의 현재 좌표가 Nominatim으로 1회 나갈 수 있는 사실도 PRIVACY에 명시했다.
- 검증: 제공자 유닛 6/6, typecheck, fake-key production build, CSP/env 배선 게이트, Chromium 라이브 411/411(서울 현재 위치 Kakao·타슈켄트 현재 위치 TomTom·둘 다 미선택 confirm 비활성·콘솔 오류 0) 통과. 운영 TomTom 키와 실제 타일은 아직 없으므로 배포 완료가 아니며 T-036에서 닫는다.

## HANDOFF-0183 · **한국 좌표 Kakao Maps 배선과 초보자 설정 가이드** (2026-08-13 · v2.34 릴리스 후보)

- 사용자 결정은 한국 좌표만 Kakao Maps, 나머지는 기존 MapLibre, Yandex·VWorld 제외다. 제공자 판정을 순수 모듈로 분리하고 Kakao 실패 시 MapLibre, 그마저 실패하면 기존 장소 목록으로 내려가는 순서를 구현했다.
...
- 정본의 `scripts/sync-skills.mjs`만 사용해 Codex·Claude 전역 설치 경로 12곳을 백업 후 원자 교체하고, Bugeon Journey의 `vendor/codex-shared-skills`와 lock을 같은 커밋으로 갱신했다. 전역 공유 스킬을 프로젝트에서 손으로 덮어쓰지 않았다.
- `schemas/release-profile.json`과 공용 스킬 계약의 승인 커밋을 새 lock에 맞췄다. `release-harness-governance` 해시는 전역·vendor·lock·프로필 모두 `c05bf320…9537`로 일치한다.
- 검증: 정본 `npm test` PASS, 전역 `--check-global` PASS, 프로젝트 `--check-vendor` PASS, 공용 계약 주입증명 27축 PASS, `npm run check:tooling` 7종 PASS. vendor 변경으로 생긴 모듈 설계서 드리프트는 생성기로 갱신했다.
...
- 전체 입력·엔트리 복사 두 층을 없앴고 ZIP 엔트리가 입력 view인지 유닛으로 고정했다. 일반 복원과 왕복 드릴은 같은 사전 읽기 문을 거쳐 33MiB 가짜 파일의 `arrayBuffer()` 호출 0회를 확인했다.
- Android 네이티브/renderer 값은 32MiB 상한을 올리지 않고 낮추는 데만 쓴다. 🔴 Node production-code-path는 통과했지만 Android WebView 실기기 PSS·원본/영상-heavy 표본은 미측정이므로 상한은 안전 보증이 아니라 위험 축소다.

## HANDOFF-0178 · 🔴 **다음 사람이 여기부터 읽으세요 — 세션 종료 인계(v2.31)** (2026-08-13)

> **이 항목은 대화 맥락이 전혀 없는 사람(Claude Code·Codex·다른 에이전트)을 위해 쓴다.**
> 이 한 절만 읽어도 지금 상태를 알고 바로 이어받을 수 있어야 한다.

### 지금 상태 한눈에

| 항목 | 값 |
|---|---|
| 기본·배포 브랜치 | `main` (GitHub Pages가 여기서 배포된다) |
| 배포된 버전 | **v2.31** (아래 「배포 확인」 참조) |
| 작업 브랜치 | `claude/continue-previous-session-4kl87t` — **머지 완료 후 남은 것 없음** |
| 미커밋·미푸시·stash | **0** |
| 하네스 | **전부 통과 · 건너뛴 것 없음**(게이트 72종) |
| 유닛 | **1652건 전부 통과** |
| DB 마이그레이션 | `0031`까지 운영 적용·되읽기 확인 완료. **이번 세션에 스키마 변경 없음**(주석 교정 1건뿐) |
| 열린 과제 | `docs/BACKLOG.md` — T-026·T-028~T-035 (**상태의 정본은 그 파일 하나다**) |

### 이번 세션에 한 일 (릴리스 셋)

1. **v2.30** — 외부 리뷰 10건 중 4건. 화면 생명주기(M-0158) · 시작 번들에서 변경이력 제거 · 버전 SSOT · 영상 포스터 주석 교정(+`0031`). 자세히는 HANDOFF-0176.
2. **v2.31** — 이번 세션의 교훈을 헌장·게이트로 내림. 자세히는 HANDOFF-0177.
3. **v2.31에 함께** — §13 화면 확인 중 §17 모순을 찾아 고침(**M-0159**). 「게이트 72가지」와 「게이트 71개 중」이 나란히 배포돼 있었다.

### 🔴 다음 세션의 첫 번째 작업

**T-030 — 대용량 백업의 기기 메모리를 *잰다*.** 외부 리뷰가 *"큰 ZIP이 WebView에서 터진다"*고
지적했고 **아직 안 쟀다.** 재보지 않고 「고쳤다」고도 「문제없다」고도 말하지 않는다.
T-021이 **JSON 경로**에서 같은 측정을 이미 했으니 그 방법을 그대로 쓴다.

그다음은 T-031(죽은 라우트 — **유닛이 있지만 공허하다**) → T-035(칩 44px) 순을 권한다.
둘 다 선행조건이 없고 완료 조건이 명확하다.

### 🔴 착수 전에 반드시

```
npm run brief <고칠 파일들>   # 읽을 헌장·형제 목록·그 영역의 과거 실수·열린 과제를 뽑아 준다
```

이건 권고가 아니라 절차다(헌법 §9). 이 저장소는 **답을 코드 주석에 적어 두고도 못 찾아
나흘을 쓴 적이 있다**(M-0064).

### 알아 둘 제약 (모르면 반드시 헛돈다)

- **`npm run gates`는 「통과」가 아니다.** 판정문이 *"M개는 아예 안 쟀습니다"*라고 스스로 말한다. 병합 근거는 `npm run build && npm run harness`뿐이다.
- **`npm run gen` 한 줄이 생성물 재생성의 전부다**(2026-08-13 신설). 개별 생성기를 순서대로 치지 마라 — 순서를 틀리면 설계서가 낡은 해시를 품는다.
- **검증 명령을 파이프에 넣지 마라**(§18-A). PreToolUse 훅이 막는다. `cmd > log 2>&1; echo "EXIT=$?"`.
- **`*.github.io`·`*.supabase.co`가 이 샌드박스에서 막혀 있다.** 「배포 확인함」(워크플로 결론)과 「사이트에서 확인함」은 다른 말이고, 후자는 **사용자 실기기 몫**이다.
- **커밋 접두사는 `feat|fix|security|refactor|test|docs|build|chore`만**(`.githooks/commit-msg`가 거부). **그 뒤 요약은 한국어로 쓴다**(§19).
- **사용자 대면 문장은 전부 한국어다** — 대화 답변·도구 진행 줄·PR 제목/본문·커밋 요약 포함. 중국어는 이 계정에서 **영구 금지**.
- **`main`에 브랜치 보호가 켜져 있다.** Required 검사(`harness`·`live-render`)가 초록이어야 머지된다 — 어기면 405로 막힌다.

### 이번 세션에 **의도적으로 안 한 것** (다음 사람이 「고치려」 들지 않게)

- **거대 파일을 쪼개지 않았다**(T-034). 살아 있는 앱에 위험하고, `check-fn-size` 래칫이 이미 한 방향으로 밀고 있다. **작업하다 걸릴 때 그 자리의 중복을 덜어내는 것**이 정상 진행이다.
- **`package.json`의 `version: 0.0.0`을 안 고쳤다.** 죽은 값이고, 고치면 릴리스마다 손댈 자리를 하나 더 만든다. 버전 정본은 `src/app/changelog.ts`이고 `check-version-ssot`가 그렇게 강제한다.
- **게이트를 외부 모듈로 빼지 않았다**(사용자 보류 결정 2026-08-13). 사유는 `gates-mechanization-dev` §2-J.
- **`gateCount`(72)와 검사 스크립트 수(71)를 하나로 합치지 않았다.** **다른 개념이다** — 앞은 하네스 항목(`typecheck`·`unit-tests` 포함), 뒤는 `scripts/*.mjs`. 합치면 틀린다. 고친 것은 숫자가 아니라 **이름**이다(M-0159).

### 배포 확인

`Deploy to GitHub Pages` 워크플로 결론으로 확인한다(아래 「최종 상태」에 run/커밋 기록).
**사이트 실물 확인은 사용자 몫**이다 — 이 샌드박스는 `*.github.io`에 못 닿는다.

## HANDOFF-0177 · **이번 세션의 교훈을 헌장·게이트로 내렸다 — 조항 셋 중 둘에 기계층을 붙였다** (2026-08-13 · 릴리스 대기)

- **branch**: `claude/continue-previous-session-4kl87t`(v2.30 머지 후 `origin/main`에서 다시 세움) · **상태**: 🔴 **구현됨·릴리스 대기**(§15 축적 — 사용자 화면을 막는 결함이 아니다)
- **왜 지금인가**: 사용자 지시 *"지금까지 작업에서 얻은 내용 스킬 및 프롬프트 업데이트하자"*. §9 2단계대로 **정독하며 구멍부터 메웠다** — 셋 다 「읽었어도 못 막았을」 자리였다.

### ① 화면 생명주기가 UI 헌장에 아예 없었다 (M-0158의 예방층)

- **찾은 구멍**: `ui-responsive-dev` §1 불변 계약 7항이 *"**모달**은 … 리스너 정리"*라고만 적고 있었다. **형제인 화면(screen)에는 같은 말이 없었다** — 비대칭이 **문서에 그대로 있었고 코드가 그 모양을 따랐다.**
- **메운 것**: §3-K 신설(정리 함수 반환 · 비동기 뒤 `isCurrent()` · 늦은 mount가 현재를 안 덮음 · 판단은 `screenSession.ts`에만) + 7항 자체를 화면까지 포함하게 고침 + §5 변경 후 의무에 두 줄.
- **기계층 신설 — `check-screen-lifecycle`**: 라우터 콜백의 화면 호출 **전부**가 `screen.mount()`를 거치는가, `.then()` 안에서 그리면 `isCurrent()`가 있는가. **모집단 0이면 통과가 아니라 `exit 2`**.
  - §4 주입 2종을 **실제 `main.ts`에** 넣어 RED 확인: 세대 확인 제거 → `main.ts:87` 적발 · `mount` 우회 → `main.ts:94` 적발. 복원 후 green.
  - 정직한 한계: **라우터만** 본다. 화면이 스스로 건 구독의 정리와 `isCurrent()`가 *올바른 자리*인지는 못 본다(존재만 본다).

### ② 생성기 9개의 실행 순서가 아무 데도 없었다 — 그리고 실제로 헛돌았다

- **실측**: `gen:module-designs`를 먼저 돌리고 `gen-registry`를 나중에 돌렸더니, 설계서가 **낡은 `registry.gen.ts` 해시**를 품은 채 남아 게이트가 **두 번** RED. 순서를 사람이 외우는 층은 반드시 샌다(§18-I 「자동 옆의 수동」).
- **구조층 — `scripts/gen-all.mjs` + `npm run gen`**: `CHAIN`이 순서와 **각 간선의 사유**를 들고 있다(§18-C — `안전을 위해`는 사유가 아니다). 설계서 생성이 언제나 마지막인 이유를 그 자리에 적었다.
- 🔴 **자기 모집단을 스스로 증명한다**: `scripts/gen-*`를 **디렉터리에서 훑어** `CHAIN ∪ SKIP`과 대조하고 어긋나면 **돌리기 전에 `exit 2`**. 「전부 재생성했다」면서 조용히 일부만 도는 것이 이 물건의 가장 큰 위험이라 그걸 먼저 막았다(M-0155의 교훈).
  - 주입 2종 RED 확인: 등록 안 된 새 생성기 → `exit 2`(아무것도 안 돌림) · 체인 중간 실패 → `exit 1`(뒤 단계 **안 돎**).
  - `SKIP` 2개는 이유와 함께: `gen-version-file`(dist에 쓴다 — build가 부른다) · `gen-font-subsets.py`(폰트 바이너리 필요).
- **형제 규율 상속**: `check-input-closeout`이 즉시 잡았다 — 새 `gen-*`는 재생성 원장에 등록돼야 한다. `excludedReason`으로 등록하며 **그 이유가 참인 사정권**까지 적었다(M-0060).
- 헌법 「검증 명령」에 `npm run gen` 추가(정본만 고치고 `npm run gen`으로 어댑터 재심음).

### ③ 「래칫에 걸리면 덜어내라」는 **어디를** 덜어낼지 안 알려 줬다

- v2.30에서 두 번 연속 같은 답이 나왔다: **덜어낼 것은 그 함수 안의 「같은 모양 두 번」**이다(`applyHomeAuth` 150→146 · `heroButton` 513→500). `gates-mechanization-dev` §5-A-1에 표로 적었다.
- 🔴 **이 항목에는 기계층이 없다** — 「이 중복이 뽑아낼 만한가」는 판단이다. 없는 검사를 만들어 초록을 띄우면 그 뜻은 「지켰다」가 아니라 「안 봤다」이다(§19가 같은 이유로 게이트를 안 둔 것과 같은 자세).

### 검증

`npm run gen`(0) · `npm run gates`(0) · `npx vitest run`(**1651/1651**) · 주입 4종 전부 RED → 복원 green.
게이트 수 71 → **72**, 대조군 래칫 70 → **71**(새 게이트가 처음부터 옳은 모양으로 태어났다 — §7 2층의 증거).

**미실행**: 전체 하네스·라이브 층은 **안 돌렸다**(§15 — 릴리스 후보에서 build 뒤에 잰다). 앱 화면은 「개발자 정보」의 게이트 수만 바뀌므로 다음 릴리스에 묶는다.
...
- ✅ **배포 그린 확인 완료**(§15 「완료 = 병합이 아니라 배포 그린」): PR #267 → squash 머지 `4975f1fe` → `Deploy to GitHub Pages` run 298 — `build` success(00:56:28→00:56:56 · 시크릿 검사 포함) · `deploy` success(00:57:00→00:57:08). PR 검사 셋도 전부 success(`harness` 2분 10초 · `live-render` 1분 58초 · `fast-gates`).
- **검증 1회 + 재개방 1회**(§18-H). 재개방 사유는 잠금 파일에 기록됨 — §13 화면 확인이 changelog와 현황표의 숫자 모순을 잡았다.
- 🔴 **「배포 확인함」과 「사이트에서 확인함」은 다른 말이다.** 이 샌드박스는 `*.github.io`를 막으므로 실기기 확인은 사용자 몫이다.
- **릴리스 꼬리 실측(T-029 상수 근거)**: PR CI 시작 → 배포 완료 **13.0분**(harness 2.2분 · 머지→배포 0.8분 · 나머지는 사람의 판단 시간).
- **다음 작업**: T-029(날짜 판정을 「도착 시각」 기준으로 — 사용자 제안을 세 군데 수정해 채택) · T-028(어제 날짜는 허용치 안이라 통과 — 좁히려면 지역 시간대 정본이 먼저다) · T-026(사용자 결정 대기).
...
- 🔴 **판정문의 숫자를 더해 보니 모자랐다.** 화면은 *"판정으로 알리는 것 **40개**"* · *"던져서 죽는 **19개**"*라고 말하는데 전체는 **63개**다. **나머지 4개는 어느 줄에도 없었다.** 세는 코드가 술어 둘을 `if / else if`로 이어 붙여 **떨어진 것을 받는 통이 없었다.**
- 🔴 **그리고 래칫이 참이었던 적 없는 숫자를 지키고 있었다.** 「깨끗함」 판정이 *"파일 **어딘가에** 낱말 + **어딘가에** `exit 2`"*라 지역성이 없었다. 자체검사가 **전부 던지는** 세 개(`check-lazy-screens`·`check-fn-size`·`check-sw`)가 다른 용도의 `exit 2` 때문에 깨끗함으로 세어졌다 — 참값은 40이 아니라 **37**이었다.
- **M-0151·M-0152의 세 번째 재발이고, 방향이 더 나빴다**: 앞의 둘은 멀쩡한 것을 결함으로 셌고(**오탐**), 이번엔 결함을 멀쩡한 것으로 셌다(**미탐**). 미탐은 조용하다.
- **고침(§7 2층 — 구조가 규율을 지킨다)**: 술어 여럿을 **값 하나**(`announceStyle` 유니온)로 바꿔 게이트당 값이 하나가 되게 했다 → **합계가 구조적으로 전체와 같다.** 안 맞으면 「자기 모집단을 설명하지 못한다」로 **exit 2**(위반이 아니다). `by[style] ||= []`라 다섯 번째 값이 생겨도 `TypeError`로 죽지 않고 합계 대조에 걸린다. 빈 통도 **0개라고 말한다.**
- **공용 헬퍼 `runSelfTest`**: 게이트마다 판정 문장을 손으로 쓰지 않는다(M-0060). 먼저 고친 형제들은 같은 문장을 **열두 번 손으로** 반복하고 있었다. 던지는 자체검사와 실패 목록을 **둘 다** 받아, 안에서 무엇을 검사하는지는 **건드리지 않고** 알리는 방식만 바꾼다(§11 ②).
- **전환 14개**: 진입점이 하나인(IIFE 0개) 게이트 11개 + 종료코드만 틀렸던 3개(`check-exif-order`·`check-bytes-upload-symmetry`·`known`). 실측 **37 → 51**.
- 🔴 **스킬 문서가 결함을 가르치고 있었다**(§9 2단계에서 발견 · §17 문서↔코드). `gates-mechanization-dev` **규칙 1번**이 *"못 잡으면 스스로 throw 한다"*는 예제를 주는데, **같은 문서 아래쪽**은 *"전제 미충족을 크래시로 알리지 마라 — 스스로 `exit 2`를 내야 한다"*고 적고 있었다. 한 문서가 자기와 반대말을 했고, 이 영역에 오는 AI는 규칙 1번을 먼저 읽는다. **던지는 게이트들은 그 문장이 성실히 낳은 것이다.** 예제를 헬퍼 형태로 바꿨다. 낡은 손 개수(「정적 게이트 15종」)도 같이 걷어냈다.
- **주입 증명(§4) 3방향** — 전부 RED 확인 후 복원:
  ① 전환된 게이트의 자체검사 파손 → **스택이 아니라 판정 + exit 2**. 대조로 미전환 게이트를 같은 방식으로 파손하니 **스택 + exit 1**(하네스가 「위반을 찾았다」로 적는 그 형태).
  ② 전환 되돌림 → 래칫 RED(`50개 < 기준선 51`).
  ③ 통 없는 다섯 번째 값 주입 → `게이트 63개 중 61개만 분류됐습니다` **exit 2**.
- 🔴 **주입 ③의 첫 판은 공허했다 — 그리고 그게 수확이다.** 합계 대조를 검증하려고 통 둘을 합계에서 뺐는데 **RED가 안 났다**: 그 두 통이 지금 **비어 있어** 합이 안 변했기 때문이다. *대상 0에서 공허하게 통과*하는 이 저장소의 최빈 실패다. **첫 주입이 GREEN이면 주입이 틀린 것이지 코드가 옳은 것이 아니다.**
- 🔴 **작업 중 자책 하나**: 주입을 원복하며 `git checkout <파일>`을 썼는데, 그 파일의 전환이 **아직 커밋 전**이라 내 작업까지 함께 날아갔다(`check-doc-tree`). 재적용하고, 이후 주입은 **커밋 뒤에** 하거나 백업 사본에서 복원했다. **커밋 안 된 작업 위에서 `git checkout`은 복원이 아니라 삭제다.**
- **실행 검사**: `npx tsc --noEmit` exit 0 · `npx vitest run` **116파일 1,645건 통과** · `npm run gates` exit 0(재본 64 · 안 잰 5는 릴리스 하네스에서).
- 🔴 **라이브 렌더 미실행**(원칙 #4). 「개발자 정보」의 게이트 현황표에 나가는 숫자가 **40 → 51**로 바뀌지만, 그리는 코드는 그대로이고 v2.26에서 이미 열어 눌러 확인한 화면이다. 이번 판에서 **화면을 열어 보지는 않았다.**
- **릴리스 상태**: §15 「변경 축적」에 따라 **「구현됨·릴리스 대기」**다. 버전을 올리지 않았고 전체 하네스·build도 돌리지 않았다 — 다음 기능 릴리스에 묶는다.
- **다음 작업**: T-027(남은 12개 · 전부 IIFE 블록이라 파일마다 블록 누락을 눈으로 확인해야 한다). 목록은 `check-gate-control` 판정문이 매번 이름을 댄다.
...
- 🔴 **사용자 지시대로 41개에 대조군을 붙이려다, 붙일 필요가 없다는 것을 알았다.** 첫 파일(`check-ui-color-token`)을 열었더니 **양성·음성 6사례가 이미 있었다.** 세어 보니 **39개가 이미 갖고 있었다.**
- **원인**: 검출기가 `function selfTest` 한 모양만 알았다. 세 모양이 있다 — 이름 붙인 함수 · **이름 없는 블록**(39개가 여기) · 인자 받는 형태.
- 🔴 **오탐은 방향이 나쁜 결함이다.** 그대로 진행했으면 이미 있는 대조군 위에 **39번 덧쓰며** §11 ②를 반복할 뻔했다. 「없다」는 목록이 길수록 **하나 열어 보는 값이 크다.**
- **고침**: `hasInlineSelfCheck` 추가 — 판정 근거를 이 저장소의 **종료코드 계약**(자체검사 실패 = exit 2)에 뒀다. 「exit 2만 있고 자체검사가 아닌 것」·「말만 하고 안 나가는 것」을 음성 사례로 박았다. **22 → 51로 정정.**
- **진짜로 없던 3개에 붙였다**: `check-sw`(주석 제거 — 오늘 세 번 당한 자리라 여기부터) · `check-lazy-screens`(동적 import 오탐 금지가 핵심) · `check-fn-size`(래칫이라 세는 법이 곧 계약). **셋 다 판정 함수를 망가뜨려 exit 2를 확인했다.**
- 🔴 **`check-fn-size`의 첫 대조군은 죽었다** — `one[0].name`이 TypeError를 던져 exit 1 + 스택이 나왔다. 죽음을 위반으로 보고한 §18-G의 형태다. 옵셔널 체이닝으로 고쳐 **깨끗하게 exit 2**로 나가게 했다.
- **남은 것 12개**: `check-hooks-wired` · `check-ci-policy` · `check-sync-release-contract` · `check-registry-gen` · `check-constitution-gen` · `check-module-design-docs` · `check-platform-map` · `check-font-subsets` · `check-hand-counts` · `check-doc-counts` · `check-instant-normalization` · `check-exif-strip-on-share`. 대부분 **생성물 대조**(재생성본 == 커밋본)라 순수 함수가 아니라 파일 입출력이 판정이다 — 대조군을 만들려면 임시 트리를 세워야 하고, 그건 재설계다.
- **실행 검사**: `gates` exit 0(65개) · 주입 4방향(세 게이트 + 검출기) 전부 RED → GREEN.

## HANDOFF-0168 · **게이트가 자기를 틀리게 셌다 · M-0150** (2026-08-12 · v2.23)

- **branch**: `claude/t-019-fact-verification-4jlqq4`(머지 뒤 `origin/main`에서 다시 세움 — 🔴 **이번엔 커밋 전에 브랜치를 확인했다**, M-0150이 그 실수다)
- 🔴 **v2.22에서 만든 게이트가 스스로 틀렸다.** 「대조군 보유」를 잰다면서 **플래그 노출**만 셌다. 실측: 대조군을 갖고도 플래그를 안 단 게이트가 **14개** — 진짜 숫자는 12가 아니라 **22**였다.
- **축을 둘로 갈랐다**: `HAS_CONTROL`(§4가 요구하는 것) · `RUNNABLE`(살아 있는지까지 재려면 필요). 둘 다 한 방향 래칫이고, 판정문이 **못 잰 것을 스스로 말한다**(*"돌려 본 것은 15개뿐"*).
- 🔴 **주입이 결함을 잡았다**: 호출을 `// selfTest();`로 **주석 처리해도 통과**했다. 오늘 `check-edge-cors`에서 똑같이 당한 자리를 반복했다 — 같은 함정에 두 번 빠졌으므로 `stripComments`를 넣고 **두 형태를 음성 사례로 박았다**(§7).
- 🔴 **기준선을 추측했다가 틀렸다**(24 → 실제 22). 래칫 숫자는 **세어서** 넣는 것이지 어림하는 것이 아니다.
- **세 게이트에 플래그를 노출**했다: `check-secret-leak`(대조군이 이미 양성 8·음성 3으로 강했다) · `check-backup-coverage` · `check-blueprint`. 나머지 인자 받는 형태(`selfTest(server)`)는 손대지 않았다 — 살아 있는 자료를 먹이는 대조군이라 독립 실행으로 바꾸려면 재설계가 필요하다.
- **죽은 대조군은 없었다**: `selfTest` 함수를 가진 21개가 **전부 실제로 부른다**. 처음 grep이 두 건을 「0회 호출」로 보고했는데, 인자를 받는 형태라 패턴이 놓친 것이었다 — **보고 전에 열어 봐서** 없는 결함을 적지 않았다.
- **M-0150**: 같은 날 **종료코드 0을 보고 세 번 「됐다」고 믿었고 셋 다 대상이 틀렸다.** 근본형은 *「성공했다」와 「무엇이 성공했다」를 같은 말로 쓴 것*이고, §18-A가 이미 적어 둔 자리다. 처방은 **성공 보고 전에 대상을 되읽기**.
- **실행 검사**: `gates` exit 0(65개) · 주입 2방향 RED → GREEN.
- **다음 작업**: 대조군 없는 게이트 41개. **한 번에 하지 않는다** — 급히 만들면 §11 ②(옛 전제를 정상 케이스로 못박음)를 41번 반복한다.

## HANDOFF-0167 · **게이트의 대조군 · 두 AI의 강제 대칭** (2026-08-12 · M-0149 · v2.22)

- **branch**: `claude/t-019-fact-verification-4jlqq4` · **신규**: `scripts/check-gate-control.mjs` · `scripts/check-enforcement-parity.mjs` · **변경**: `package.json`(prepare) · `src/app/gates.ts` · `scripts/harness.mjs` · 헌법 S-09 · 문서
- **사용자 제안에서 시작했다**: *"게이트 정의가 목적을 갖는지 · 대조군이 있는지 · 판정기준이 구체화됐는지 · 결과에 대한 조치를 판단하는지"*. 네 축을 실측해 **①③은 이미 있었고**(`GATE_AXIS`+`check-gate-promise`), **②만 통째로 비어 있었다.**
- 🔴 **§4가 조항으로만 있었다.** 게이트 62개 중 `--selftest`를 **실제로 처리하는** 것은 **10개**뿐. `grep`으로 세면 12로 보이는데 둘은 주석과 픽스처 이름이었다 — 그래서 판정을 **문자열이 아니라 코드**(`process.argv.includes('--selftest')`)로 했다.
- **`check-gate-control`**: ①래칫(한 방향, 기준선 12) ②있다고 한 셀프테스트를 **실제로 돌려** 통과 확인 ③대상 0이면 exit 2. 🔴 **과소평가함을 스스로 말한다** — `check-schema-parity`처럼 플래그 없이 본문에서 주입하는 대조군은 안 세진다. 그리고 「대조군이 *적절한가*」는 못 본다(§11 ②가 그 사고를 이미 기록).
- 🔴 **사용자 지시 ②에서 더 무거운 것이 나왔다**(M-0149). *"코덱스와 클로드가 같은 규칙·스킬을 보고 작업하도록 강제하자"* → 재보니 **규칙은 이미 같았다**(어댑터 글자 단위 대조) **스킬도 같았다**(라우팅 표 공유). **갈라져 있던 것은 「강제」였다.**
- **세 가지가 한꺼번에 나왔다**: ①`.claude/settings.json` 훅 4개는 **Claude만** 받는다 ②그런데 `AGENTS.md`는 *"실제 강제는 command hook과 CI가 한다"*고 적어 **코덱스에게 없는 보호막을 있다고 말했다** ③두 AI가 모두 받는 유일한 층인 git 훅은 `core.hooksPath` 미배선으로 **아무에게도 안 걸리고 있었다**(`git config --get` → 빈 값).
- 🔴 **가장 나쁜 것은 ②다.** 모르는 것보다 **틀리게 아는 것**이 위험하다 — 모르면 조심하지만 틀리게 알면 안심한다. 헌법이 여러 곳에서 *"`.githooks/commit-msg`가 거부한다"*고 **단정형**으로 적고 있었다.
- **고침**: `package.json`의 `prepare`가 `npm install` 뒤 자동 배선(**사람이 기억해서 켜는 층은 반드시 새어 나간다**) · `check-enforcement-parity` 신설(공통 층 실행 권한·자동 배선·Claude 전용 훅의 **코덱스 대체 명시** 강제) · 헌법 S-09에 **「강제 세 층과 각 층이 묶는 대상」** 표와 *"새 강제는 위쪽 층부터 고른다"* 규칙.
- **왜 `check-hooks-wired`가 못 잡았나**: 그건 `.claude/settings.json`만 본다. **git 훅은 형제인데 검사에서 빠져 있었다**(§7 최빈형). 그리고 배선이 `git config` 한 줄이라 **파일에 안 남아** 게이트가 볼 것이 없었다 — 그래서 `package.json`이라는 **파일에 남는 자리로 끌어왔다.**
- **주입 증명(§4) 3방향**: 대조군 제거 → RED · 자동 배선 제거 → RED · 미등록 훅 파일 추가 → RED. 전부 복원 후 GREEN.
- **실행 검사**: `gates` exit 0(63 → 65개).
- **잔여 위험**: 게이트는 *"`prepare`가 적혀 있다"*까지만 안다 — 어떤 사람의 로컬에서 실제로 설정됐는지는 못 본다. 코덱스가 `AGENTS.md`를 읽었는지도 못 본다(§16과 같은 부류).
- **다음 작업**: 대조군 없는 게이트 51개에 하나씩 셀프테스트를 붙이며 기준선을 올린다. **한 번에 하지 않는다** — 51개를 급히 만들면 §11 ②(옛 전제를 정상 케이스로 못박음)를 51번 반복할 위험이 있다.

## HANDOFF-0166 · **세션 마감 — T-026 장기과제 분류 · 지우지 못한 브랜치 하나** (2026-08-12)

- **§16 미완성 점검(세션 닫기 전)**: 작업트리 깨끗 · `stash` 없음 · 로컬과 원격이 같은 커밋. **미완성 없음.**
- **T-026을 `막힘-사용자`로 옮겼다**(사용자 결정: *"일단 장기과제로 분류하자"*). 🔴 **새 상태 값을 만들지 않았다** — 이 문서의 계약은 `대기`·`막힘-실기기`·`막힘-사용자` 셋뿐이고, 「장기과제」를 넷째로 더하면 다음 사람이 *「대기인데 오래 걸리는 것」*과 구분하지 못해 두 값이 같은 뜻으로 섞인다. 외부 서비스 가입·약관 동의는 정확히 `막힘-사용자`다.
- 🔴 그 행에 **「고장을 방치 중」이 아니라 「선택지를 열어 둠」**임을 적었다. 안 적으면 몇 달 뒤 이 행을 본 사람이 미처리 결함으로 읽고 선의로 손댄다(§7 — 이유 없는 비대칭은 다음 사람이 「고친다」). 실제로 지금 상태는 정상이고 진단의 「장소 검색 도우미」가 그것을 **정상으로 판정**한다(T-025).
- 🔴 **`origin/codex/security-audit-backlog-handoff`를 지우지 못했다 — 권한 경계다.** 흡수 여부는 확정했다: 그 브랜치의 유일한 커밋 `3c92fa1`이 한 일은 `docs/BACKLOG.md`에 T-019~T-024 여섯 행을 넣은 것뿐이고, **여섯 전부 `main`에 있다**(각 1건, 오늘 전부 닫아 아카이브로 이동). 그런데 삭제가 두 형식 모두 거부됐다.

  ```
  git push origin --delete codex/…       → the remote end hung up
  git push origin :refs/heads/codex/…    → HTTP 403
  ```

  이 세션의 git 프록시가 ref 제거를 허용하지 않고, GitHub MCP에도 브랜치 삭제 도구가 없다(`create_branch`만 있다). **일시적 오류가 아니라 경계이므로 재시도하지 마라** — 사용자가 GitHub 화면에서 지우는 것이 유일한 경로다.
- **이 판을 단독 머지한 이유**: §15는 문서 단독 변경을 다음 기능 릴리스에 묶으라고 한다. 사용자가 *"그러면 그것도 수정해서 머지해야지?"*로 단독 머지를 지시했고, 브랜치에 남겨 두는 것이 §16의 미완성이 되는 것을 피하는 쪽을 택했다. **앱 버전은 올리지 않는다**(v2.21 유지) — 사용자 화면이 바뀌지 않았기 때문이다(§20 3항: 좋아진 것이 없으면 없다고 쓴다).
- **다음 세션이 알아야 할 것**: 열린 과제는 **T-026 하나**뿐이고 그것도 사용자 결정 대기다. `geocode`는 운영 version 3(CORS 고침 포함). 국내 제공자 키는 여전히 없고 **그게 정상 동작**이다.

## HANDOFF-0165 · **T-025 「모른다」를 「없다」로 접지 않는다 · 헌법 §19 범위 확대** (2026-08-12)
...
- **어떻게 찾았나 — 화면이 아니라 로그였다.** 사용자가 v2.20 배포 뒤 장소 검색을 했고 결과가 나왔다(「OpenStreetMap 검색 결과」). 화면만 보면 정상이다. 그런데 **머리글이 OpenStreetMap**이라 국내 제공자를 안 탔다는 뜻이었고, 코드상 「실패」와 「키 없음」이 같은 모양(`[]`)이라 화면으로는 못 가른다. 그래서 **운영 Edge 로그를 직접 읽었다.**
- 🔴 **최근 24시간 `geocode` 호출이 전부 `OPTIONS | 400`**이었다. 같은 시각 `media-sign`의 `OPTIONS`는 200이다. 사용자의 18:47 검색도 그 400 한 건이 전부다 — **본 요청(POST)은 나가지도 못했다.**
- **원인(확인됨)**: 앱(`*.github.io`)과 함수(`*.supabase.co`)가 다른 출처라 브라우저가 사전요청을 먼저 보내는데, 그 요청에는 본문이 없고 핸들러 첫 줄이 `await req.json()`이었다 → 던짐 → 400. **`OPTIONS` 분기도 CORS 헤더도 파일에 아예 없었다.** 형제 `media-sign`은 처음부터 둘 다 갖고 있었다(§7 최빈형).
- 🔴 **앱이 그 실패를 삼켰다**: `proxyProviders`가 `catch { return [] }`·`r.error → []`라 **「못 물어봤다」와 「제공자가 없다」가 같은 값**이 된다(§8 — 모르는 것을 정상으로 반올림). 화면은 아무 말도 하지 않았다.
- 🔴 **그 침묵이 T-022를 덮고 있었다**: 이번 판에 넣은 질의 길이·속도 한도는 **브라우저가 닿을 수조차 없는 함수**에 걸려 있었다. 「고쳤다」와 「막고 있다」가 또 갈렸다(M-0031의 되풀이).
- **고침·배포**: CORS 상수 + 본문 읽기 전 `OPTIONS` 분기 + 모든 응답(429 포함)에 헤더. **운영 배포 version 2 → 3**, `verify_jwt: false` 유지.
- **기계 층**: `check-edge-cors` 신설(게이트 61 → 62). 주입 증명 — 고치기 전 `geocode`에서 정확히 두 줄 RED, 고친 뒤 GREEN. 셀프테스트 잡아야 할 것 3 · **잡으면 안 되는 것 4**.
- 🔴 **그 게이트가 첫 판에 두 번 오탐했다**(§11 ③): 내 주석 속 `req.json()`이 순서를 뒤집었고, `media-sign`의 **이름 붙인 핸들러**(`DENO.serve(handler)`)를 못 찾았다. 주석 제거 + 파일 전체 순서 판정으로 고치고 그 두 형태를 음성 사례로 박았다.
- 🔴 **정직한 경계 — 오늘 화면은 바뀌지 않는다.** 시크릿(`KAKAO_REST_KEY`·`VWORLD_KEY`)이 애초에 안 붙어 있고, 그때 Nominatim만 쓰는 것은 **문서에 적힌 기본 동작**이다(HANDOFF 앞선 기록). 고친 것은 **「키를 넣어도 안 켜지는」 상태**다.
- ✅ **행동 되읽기 완료(§18-F)**: 사용자가 검색 1회를 더 했고 운영 로그가 이렇게 바뀌었다.

  | 시각(UTC) | 배포판 | 요청 | 결과 |
  |---|---|---|---|
  | 09:47:57 | version 2 | `OPTIONS` | **400** — 본 요청 없음 |
  | 11:18:28 | version 3 | `OPTIONS` | **200** |
  | 11:18:30 | version 3 | **`POST`** | **200** (1,178ms) |

  🔴 **`POST` 줄은 이 로그 전체에서 처음 나타났다.** 배포 전에는 `geocode`에 POST가 단 한 건도 없다 — 사전요청에서 다 끊겼다는 것의 직접 증거다. 그 POST는 `capabilities`이고, 응답은 `providers: []`(시크릿 없음)라 검색은 계획대로 Nominatim으로 갔다. **화면은 그대로이고 그것이 맞다.**
- **다음 작업**: ①T-025(앱이 「못 물어봤다」를 「없다」로 접지 않게) ②T-026(국내 제공자 시크릿 — 사용자 결정) ③이 커밋들은 앱 화면을 바꾸지 않으므로 §15에 따라 **다음 기능 릴리스에 묶는다.**
...
- **왜 손으로 옮겨 적었나**: 이 컨테이너에 `SUPABASE_ACCESS_TOKEN`이 없어 CLI가 실제 파일 바이트를 올릴 수 없었다. MCP `deploy_edge_function`은 본문을 인자로 받으므로 21,588바이트를 전사했다. 사용자 승인 아래 진행했다.
- **되읽기(§18-F)**: `get_edge_function`으로 배포본 소스를 받아 저장소 파일과 대조했다. 핵심 표지가 전부 있다 — `MAX_QUERY_LEN = 100`, `RATE_MAX_PER_WINDOW = 300`, `allowRequest`의 창·초과·retryAfter 세 갈래, `verifyUser`가 id를 돌려주는 형태, 그리고 **길이 검사 → 속도 검사 → 상류 호출** 순서.
- 🔴 **정직한 한계 — 「대조했다」의 정확한 뜻**: 바이트 해시로 대조하지 **못했다.** ①플랫폼은 소스 해시가 아니라 **번들 해시**만 준다 ②MCP 응답을 셸로 흘려보낼 수 없어 `sha256sum`을 걸 자리가 없다. 그래서 이 대조는 **읽어서 맞춘 것**이다. 배포가 성공했으므로 최소한 **Deno가 파싱했다**는 사실은 확정이다.
- 🔴 **행동 확인도 못 했다**: 이 샌드박스는 `*.supabase.co`를 막는다(`*.github.io`와 같은 제약). 인증 없는 호출로 `401 unauthorized`를 받아 「살아 있고 인증이 먼저 돈다」를 재려 했으나 프록시가 403으로 끊었다.
- **그래서 남은 검출기는 사용자다**: 앱에서 **장소 검색을 한 번** 해 보면 끝난다. 결과가 나오면 배포본이 정상이고, 안 나오면 전사 오류이므로 즉시 되돌린다. 이걸 **부탁으로 남긴다** — 내가 잴 수 있는 것을 사람에게 미루는 것이 아니라(§13), 이 환경에서 **원리적으로 잴 수 없는** 자리다.
- **되돌리기**: 문제가 생기면 같은 MCP로 이전 소스를 다시 배포한다. 이전 판은 이 저장소 이력(`a139659` 이전)에 있고, 기능 차이는 길이·속도 한도 두 줄뿐이라 되돌림이 단순하다.
- **T-022 종결**: `geocode` 절반은 구현·배포까지 끝났고, `media-sign` 절반은 ADR-0065로 **안 하기로 결정**했다. 남은 것은 사용자 화면 확인 하나다.

## HANDOFF-0161 · **T-024 훅 구멍 하나 · T-023 안 하기로 결정** (2026-08-12 · 개발 안전장치)

- **T-024에서 가장 값이 큰 조각부터 쟀다 — 그리고 후보가 맞았다.** `.claude/settings.json`의 PreToolUse 훅 `hook-sql-safe.mjs`가 막는 것은 `DROP`·`TRUNCATE`·`DELETE FROM`·RLS 비활성화·`REVOKE`·`DROP COLUMN` 여섯이고, **`UPDATE`가 없었다.** `UPDATE journey.trips SET title = ''`나 `SET deleted_at = now()`는 **모든 행의 내용을 지우는데 그대로 통과**했다.
- 🔴 **이 훅의 존재 이유가 「AI가 사용자 기억을 되돌릴 수 없게 망가뜨리는 것을 막는다」인데, 가장 흔한 대량 수정 동사가 빠져 있었다.** `DELETE FROM`은 막으면서 「내용만 지우는 UPDATE」를 통과시키는 것은 대칭이 아니다(§7) — **사용자에게는 둘 다 「기억이 사라졌다」로 보인다.** 그리고 이건 「초대받은 공격자」를 요구하지 않는다. **평범한 작업 중 사고**로 일어난다.
- **수정**: `hasWhereLessUpdate()` — 문장을 `;`로 갈라 「update로 시작 + set 있음 + where 없음」만 본다. 🔴 **WHERE가 있는 UPDATE는 막지 않는다** — 일상 작업이고, 전부 막으면 사람이 `sql-safe-ok`를 습관으로 붙여 그 표식이 죽는다(오탐도 결함 · §11 ③). 셀프테스트에 양성 2건·**음성 2건**을 함께 넣었다.
- **비공허 확인**: 새 검사를 빼자 셀프테스트가 `SELF-TEST 실패(차단 못 함): WHERE 없는 UPDATE`로 **RED**, 원복 GREEN.
- 🔴 **작업 중 내가 코드에 거짓 원인을 적었고 같은 판에서 지웠다.** 첫 정규식판을 만들고 `node scripts/hook-sql-safe.mjs --selftest`가 멈추자 **「파국적 백트래킹」이라고 단정해 주석에 적었다.** 실제 원인은 훅 프로토콜상 **stdin에서 JSON을 기다린 것**이었다(셀프테스트는 모듈을 import해 `selfTest()`를 직접 부른다). §8 그대로다 — *그럴듯한 원인은 확인된 원인이 아니다.* 주석을 사실로 바꾸고, 그 함정 자체를 곁가지로 남겼다.
- **T-023은 「안 하기로 결정」으로 닫았다**(ADR-0066). 🔴 **후보 하나는 사실이 아니었다** — 「host 전체 `allowNavigation`」은 실제로 `["hanwha27-tdtu.github.io"]` 자기 호스트 하나뿐이다. **후보 목록을 그대로 믿었으면 없는 결함을 고칠 뻔했다.** 나머지는 전부 저장소 쓰기 권한을 요구하고, 고정 debug 키는 ADR-0039가 이미 결정했다.
- 🔴 **`allowBackup="true"`는 일부러 끄지 않았다.** 끄면 `adb backup` 추출을 막지만 **자동 백업이라는 복구 계층을 없앨 수 있고**(원칙 #1), debug 서명 사이드로드라 `run-as`로 이미 읽힌다 — **큰 문을 두고 작은 문만 잠그는 모양**이다. 실제 백업 범위는 실기기 미측정이며, **그 불확실성 자체가 「지금 건드리지 않는다」의 근거다**(모르는 상태에서 복구 경로를 없애지 않는다 · §8).
- **검증**: 훅 셀프테스트 통과 · 주입 RED→GREEN · 빠른 게이트 61개 PASS.
- **남은 것**: T-022의 `supabase functions deploy geocode` 하나, 그리고 T-024의 나머지(진단 redaction 두 조각 · 세션 잔존 · 읽기전용 감사 에이전트 Bash 경계)는 아직 안 봤다.

## HANDOFF-0160 · **T-022 재검토 — 한도를 사람이 닿을 수 없는 자리로, `media-sign`은 안 하기로 결정** (2026-08-12 · ADR-0065)

- 사용자가 재검토를 요청하며 조건을 걸었다: *"사용하는데 불편이 있으면 절대 안 되니까."* **그래서 먼저 잰 것은 위협이 아니라 「내가 넣은 한도가 사용자를 막는가」였다.**
- 🔴 **실측이 판단을 뒤집었다.** 이 앱에서 지오코딩을 가장 많이 부르는 경로는 위치관리대장의 「주소 일괄 채우기」로 **1.1초 간격, 분당 약 54회**다 — 처음 넣은 30회/분을 **넘는다.** 그런데 그 경로는 **Nominatim을 직접** 부르므로 Edge를 지나지 않는다. Edge를 타는 것은 **버튼을 눌러야 나가는 장소 검색** 하나뿐이고 버튼은 요청 중 비활성이다.
- **결정 ①**: 30회/분 → **300회/분**. 검색 1회가 제공자 폴백으로 Edge를 최대 2번 부르므로 300은 **검색 150회/분**(0.4초에 한 번을 1분 내내)이라 사람은 도달할 수 없고, 폭주 루프는 초당 수십~수백 회라 즉시 걸린다. 🔴 **잡으려는 것이 악의가 아니라 앱 버그임을 상수 주석에 명시했다** — 그 목적을 안 적으면 다음 사람이 「보안이 약하다」며 조인다.
- **유닛도 방향을 바꿨다**: 막힘만 재던 것에 **통과 120회 연속**(1초에 한 번씩 1분)과 **폭주가 정확히 300에서 걸림**을 함께 잰다. 막는 것만 재는 한도는 언제 사용자를 막을지 모른다.
- **결정 ②(`media-sign`)**: **안 하기로 결정**. ①위협이 「이미 초대받은 계정」을 요구한다(혼자 쓰는 앱 — ADR-0062와 같은 구조) ②이 파일은 `sync-release-contract`에 소스 해시로 묶여 **주석 한 줄도 §18-F 배포 창을 요구**한다 ③키 경계는 이미 지켜진다 — 남는 것은 「자기 자리에 얼마나 쓰는가」이고 그건 침해가 아니라 비용이며 저장 용량 화면이 이미 센다.
- 🔴 **비대칭의 이유를 적었다**(§7): `geocode`는 **남의 회사 쿼터**를 태우고 한 요청이 상류로 두 번 나간다 — 우리가 통제 못 하는 비용이다. `media-sign`은 **우리 R2**에 우리 자료를 쓴다.
- 🔴 **사유를 어디에 적었는가가 이 항목의 핵심이다**(사용자 지시 — 코덱스도 알게). `media-sign` **소스에는 적지 않았다** — 주석 한 줄도 해시를 바꾸기 때문이다. 대신 `.claude/skills/supabase-security-dev/SKILL.md`에 절을 세웠다: `npm run brief`가 그 파일을 고치려는 **누구에게든**(Claude·Codex) 그 헌장을 먼저 읽힌다. ADR-0065와 BACKLOG 행에도 같은 사유가 있다.
- **검증**: 유닛 9/9 · 빠른 게이트 61개 PASS · `tsc` PASS.
- 🔴 **아직 배포하지 않았다.** Edge는 저장소가 아니라 **배포된 것이 돈다.** T-022는 `supabase functions deploy geocode` 하나만 남기고 열어 둔다 — 그 한 줄을 잊으면 이 판은 아무것도 막지 않은 판이 된다.

## HANDOFF-0159 · **T-022 절반 — geocode 자원 남용 상한** (2026-08-12 · 구현됨·배포 대기)

- **후보 확인(사실)**: `geocode`는 `verifyUser`+`isInvited`로 「누가」는 막지만 **「얼마나」를 전혀 막지 않았다** — 질의 길이 무제한, 속도 한도 없음, 카카오 경로는 한 요청이 상류로 **두 번** 나간다(keyword+address). 허용목록 안의 계정 하나 또는 탈취된 세션이 우리 지도 API 키의 쿼터와 비용을 그대로 태울 수 있었다.
- **수정(geocode)**: ①질의 길이 상한 `MAX_QUERY_LEN=100` — **상류로 나가기 전에** 거절해야 상류 쿼터가 안 나간다 ②사용자별 속도 한도(60초 창에 30회) — 상류 호출 직전에 건다 ③`verifyUser`가 boolean 대신 **사용자 id**를 돌려준다(이미 사용자 본문을 받아오므로 왕복이 늘지 않는다). 실패·불명은 `null`이고 여전히 거부다(§8).
- 🔴 **이 한도가 무엇을 보증하지 **못**하는지 코드에 먼저 적었다.** 카운터는 **이 인스턴스의 메모리**에 있다 — Edge는 인스턴스가 여럿일 수 있고 재시작하면 사라진다. 그래서 이것은 **전역 쿼터가 아니라 인스턴스 단위 최선노력**이다. 한 클라이언트가 한 인스턴스를 두드리는 흔한 남용은 막고, 분산된 남용은 못 막는다. 전역 보증은 DB 카운터가 필요하고 그건 migration+배포 창이 함께 와야 한다.
- **정상 사용을 막지 않는지도 쟀다** — 2초에 한 번씩 1분간 검색해도(30회) 통과한다. 막는 것만 재고 통과를 안 재면 그 한도는 언제 사용자를 막을지 모른다.
- 🔴 **`media-sign`은 이번 판에서 건드리지 않았다 — 이유가 있다.** 그 파일은 `schemas/sync-release-contract.json`에 **논리 소스 SHA-256으로 묶여** 있고, `verify-sync-release-live`가 그 해시를 **운영 함수와 대조**한다. 고쳐서 푸시만 하면 저장소 해시가 운영보다 앞서 **다음 릴리스의 운영 대조가 실패**한다. §18-F가 요구하는 순서는 「제공자 먼저 배포 → 운영 실물 되읽기 → 그 다음 앱」이고, 이번은 사용자가 **푸시까지만** 지시한 판이라 그 창이 없다. **T-022는 열린 채로 둔다.**
- **검증**: 신규 유닛 8/8 · 한도 무력화 주입에서 **3건 RED**, 원복 GREEN · `tsc` PASS · 빠른 게이트 61개 PASS.
- 🔴 **배포하지 않았다 — 그래서 아직 아무것도 막고 있지 않다.** Edge Function은 저장소에 있는 것이 아니라 **배포된 것이 도는 것**이다. 이 변경은 `supabase functions deploy geocode` 전까지 운영에 영향이 없다. 「고쳤다」와 「막고 있다」는 다른 말이다.

## HANDOFF-0158 · v2.19 배포 완료 · **헌법 §22(30초 확인)** (2026-08-12)

- **배포 완료**: PR #258 → squash `47de286` → **Deploy to GitHub Pages run 289 · success** · **Android APK run 30 · success**. Required CI는 후보 `10f7b7b`에서 `harness`·`live-render`·`fast-gates` 셋 다 success. **검증 1회 + 재개방 0회**.
- 🔴 **APK 성공이 이번 판의 마지막 미검증 층을 닫았다.** 이 환경에 Android SDK가 없어 자바를 로컬에서 컴파일하지 못했고, **PR에서는 APK job이 돌지 않는다**(3개뿐). main 푸시의 `Android APK` 워크플로가 첫 컴파일 판정자였고 `assembleDebug` 2분 32초로 통과했다. 다음에 자바를 만질 때도 같은 순서다 — PR 초록은 자바를 보지 않는다.
- **헌법 §22 신설**(사용자 지시). 외부 상태를 기다릴 때 **한 번의 대기는 30초를 넘기지 않는다.** 요점은 「시간이냐 내용이냐」가 아니라 **눈을 감고 있는 구간의 길이**다 — 예상 1분짜리에 5분을 자면 30초 만에 끝났든 10분째 멈춰 있든 **똑같이 모른다.**
- 🔴 **이 조항은 이번 배포에서 실제로 값을 보였다.** Pages `deploy` job이 러너를 못 받아 **5분 4초를 대기열에 있었다**(06:07:19 → 06:12:23). 이전 배포는 40초에 끝났으므로 평소의 7배다. 통짜 대기였다면 그 사실 자체를 못 봤다.
- 🔴 **내가 어긴 것을 그대로 적는다**: *"계속 확인하겠다"*고 써 놓고 210초·300초 통짜로 잤다. 말과 행동이 다르면 그건 사용자에게 가는 **거짓 문장**이다(비타협 원칙 #4·§8). 그리고 처음 물었을 때 「시간이 아니라 내용을 기다려라」로 **요점을 빗나가게 답했다** — 사용자가 두 번 고쳐 줘서 조항이 됐다.
- **기계 층은 없고, 없다고 적었다.** 게이트는 파일을 보는데 이 조항이 지배하는 것은 **내가 대기를 거는 방식**이라 저장소에 남지 않는다(§19·§16과 같은 부류). `gen:adapters`로 `AGENTS.md`에 심어 **코덱스도 같은 글자**를 읽는다.
- **문서 전용이라 릴리스하지 않는다**(§15 ②). 앱 버전은 v2.19 그대로다.
- **정직한 한계**: 이 샌드박스는 `*.github.io`를 막아 운영 `version.json`의 v2.19 read-back을 **하지 못했다.** 그리고 새 APK에서 큰 사진을 골랐을 때의 실제 거동은 **사용자 실기기 확인 경계**다.
...
- 🔴 **재개방 1회의 이유는 내가 만든 게이트였다(M-0145).** CI run 576의 `harness` job이 `verify-authgate-live`에서 죽었다 — CI는 Chromium을 `live-render` job에만 설치하는데, 새 게이트는 `playwright` 패키지 import 성공을 브라우저 존재의 근거로 쓰고 `chromium.launch()`를 그대로 불렀다. 예외가 터지면 종료코드 1이고, 이 저장소 계약에서 1은 **「위반을 찾았다」**다 — 게이트는 판정한 것이 아니라 **죽었는데 화면엔 위반이라고 적혔다**(§18-G가 경고하는 바로 그 형태). 형제 `verify-diagnostics-live`는 이미 `try/catch` → `exit 2`(SKIP)로 옳게 처리하고 있었고 **새로 만든 것만 규율 밖에서 태어났다**(§7 · M-0033과 동형).
- **그리고 그 조항을 정독한 세션에서 났다.** `GATE_WORLD`에 `browser`라고 **적어 놓고도** 그 세계를 벗겨서 돌려 보지 않았다 — §18-G의 *"선언은 산문이므로 믿지 않는다"*가 그 자리다. `check-env-assumption`이 벗길 수 있는 세계는 `baseRef`·`dist` 둘뿐이고 `browser`는 그 조항이 이미 「정직한 한계」로 적어 둔 사각지대다. **한계를 적어 둔 것과 그 한계에 빠지지 않는 것은 다른 일이다.**
- **부수 수확 — 로컬 빨간불 2건의 정체가 밝혀졌다.** `verify-editor-live`의 사진·영상 저장 검사가 이 샌드박스에서 실패했지만(바이트는 정확, 파일 이름만 `download`), ①변경 표면에 파일 저장 코드가 없고 ②격리 실험에서 Chromium은 `download` 속성을 정상 처리하며 ③파일명 생성 함수도 정상이고 ④**`origin/main`(v2.17)을 별도 작업트리에 세워 돌리자 같은 2건이 같은 값으로 실패**했다. 그리고 **CI에서는 초록**이었다(run 576·577 모두 `live-render` success). **샌드박스 Chromium 특성이며 앱 결함이 아니다** — 쫓을 것이 없어 후속 과제로 올리지 않는다.
- **헌법 §21 신설**(사용자 지시): 머지 단계(후보를 `release:arm`으로 얼리고 Required CI에 물은 뒤)에 들어가면 **빨간 검사가 있어도 머지·배포한다.** 예외는 ①사용자 기억의 유실·손상 ②§0 절대 위반 둘뿐이고, 화면 문구·미관·검사 인프라·환경 특유 실패는 예외가 아니다. 근거는 ADR-0064.
- 🔴 **§21이 바꾸지 않는 것**: 배포 그린 확인, 정직한 보고, 강행 항목의 백로그 등록. **강행은 검사 면제가 아니라 판단의 이전**이므로 완료 보고에 「하네스 N/M으로 배포함」을 적고 빨간 항목을 같은 판에서 다음 판 우선 과제로 올린다. 안 올리면 강행이 아니라 은폐다(§16).
- **이번 판에 §21은 적용되지 않았다** — M-0145를 고치자 Required CI가 전부 초록이 되어 **강행 없이** 배포했다. 일어나지 않은 강행을 일어난 것처럼 적지 않는다(§4 정직한 귀속).
- **정직한 한계**: 이 샌드박스는 `*.github.io`를 막아(§15) 운영 `version.json`의 v2.18 read-back을 **하지 못했다**. 「배포 워크플로 성공을 확인함」과 「사이트에서 확인함」은 다른 말이다. 그리고 잠긴 데이터 관리 화면의 **실기기 렌더**는 사용자 확인 경계다.
...
- 공유 스킬 거버넌스는 독립 앱 두 곳 이상에서 관측된 불변식만 전역 정본으로 승격한다. 이번 결함군은 아직 이 앱 한 곳의 실증이므로 `Codex-Shared-Skills`를 성급히 바꾸지 않고, 프로젝트 전용 `.claude/skills/gates-mechanization-dev/SKILL.md`에 일반 규율을 추가했다. 두 번째 앱에서 같은 근본형이 확인되면 호환성 심사와 함께 전역 승격한다.
- 새 스킬 절은 모듈 소유권을 실행 가능한 등록부 하나에 두고, 미분류·중복 소유 실행 파일을 fail-closed로 막으며, 코드 관측 사실·내용 해시·누락/변조/화석 대조군·`regenerationLedger` writer/check 쌍·정적 문서의 한계를 하나의 패턴으로 묶는다.
- 사용자 대면 changelog를 v2.17로 올린다. 앱 실행 기능은 바뀌지 않지만 변경 이력과 생성 모듈 설계서가 Pages 산출물에 포함되므로 영향 표면은 GitHub Pages 하나다. `android-shell/**`·APK workflow, Supabase migration·Edge Function·R2 계약은 바뀌지 않는다.
- 릴리스 판정은 생성물 재생성 → 최신 파일 전체 harness/build → 후보 SHA 잠금 → Ready PR의 Required CI → squash merge → Pages workflow success와 운영 `version.json` v2.17 HTTP 200 read-back 순서로 닫는다. 최종 실행 번호와 merge SHA는 이 항목을 다시 고치는 배포 유발 커밋 대신 PR·Actions·운영 read-back 기록으로 남긴다.

## HANDOFF-0148 · **코드 기반 모듈별 설계서와 드리프트 게이트** (2026-08-11 · 개발도구/문서)

- 사용자가 비상시 앱을 재현할 수 있도록 `docs/모듈별 설계서/`에 사진편집·영상 등 상세 설계서를 모으되, 사람이 유지하지 않고 오직 코드에서 만들며 코드 수정과 함께 갱신되는 게이트를 요청했다.
- `module-design-docs-lib.mjs`가 실행 파일의 단일 소유 경계를 18개 모듈로 선언하고, 실제 코드에서 파일·논리 줄·내용 SHA-256·최상위 API/내부 조립점·시그니처·본문 호출·정적 import/re-export·Dexie/Supabase/SQL/파일·네트워크·삭제 경계·직접 import 테스트를 추출한다. 앱 코드뿐 아니라 Supabase migration/Edge와 게이트/CI/릴리스 시스템도 복구 범위에 포함한다.
- 사진편집기는 실제 `editor-core`·`pixelops`·UI 조립을 별도 설계서로 냈고, 영상은 현재 구현에 맞춰 **영상 처리 파이프라인**으로 기록했다. 코드에 없는 독립 타임라인 영상편집기를 구현된 것처럼 쓰지 않고 현재 경계에 명시했다.
- `gen:module-designs`가 README+상세 설계서를 재생성하며, `check-module-design-docs`는 새 실행 파일 미분류·빈 모듈·문서 누락·내용 불일치·화석 문서를 RED로 잡는다. 내용 표가 우연히 같아도 모듈 SHA-256이 바뀌어 재생성을 강제한다.
- 게이트는 정상/내용 변조/누락/화석 대조군을 내장하고 하네스·게이트 설명/축/세계/분류에 등록했다. writer/check 쌍도 `schemas/release-profile.json`의 `regenerationLedger`에 넣어 릴리스 입력 마감이 설계서 드리프트를 함께 막는다.
- 정직한 한계: 이 문서는 정적 구조와 내용 동일성을 증명하지만 운영 데이터·배포 상태·동적 호출 순서·실기기 렌더를 증명하지 않는다. 기존 진단·왕복검사·라이브 브라우저·배포 read-back이 그 층을 계속 맡는다. 앱 런타임 기능과 버전은 v2.16 그대로다.
...
- 영상 운영 계약을 R2 경량본 한 개에서 **경량본+WebP 포스터 쌍**으로 높였다. operation별 두 키를 모두 PUT/GET 대조한 뒤에만 큐와 내부 `sourceBlob`을 정리하며, canonical 게시/소비, 누락 자동 재큐잉, 진단 파일 대조, 단건·가족 영구삭제가 두 객체를 함께 다룬다. 외부 원본 영상은 업로드·수정·삭제하지 않는다.
- 「영상 서버 왕복」은 사용자가 누를 때만 작은 재생 가능 WebM을 합성한다. 고유 trip·moment·video+세 op 원자 저장 → 운영 DB 메타/operation/path → R2 영상·포스터 SHA-256+재생 → 로컬 영상 제거 뒤 pull → tombstone 바이트 보존 → 복원 → 영구삭제 행/원장/R2 부재 → exact 잔재 0을 검사한다. canonical 세대 변경은 통과가 아니라 중단/정리다.
- 실패 주입 유닛은 같은 크기 영상 변조, 포스터 PUT 실패, HTTP 성공 뒤 포스터 GET 누락, 한쪽짜리 R2 목록을 모두 RED로 잡는다. 새 Edge 응답은 구형 앱에 필드를 추가하는 하위 호환 protocol 8이며 앱보다 먼저 배포하고 정확한 source hash를 운영에서 되읽어야 한다.
- 출고 DR 감사가 canonical 세대 변경 뒤 정리 재개, 두 탭 journal 덮어쓰기, R2 삭제 실패 뒤 op/journal 제거, `deleteMany`의 같은 id 단일 키 덮어쓰기를 P1로 잡았다(M-0142). 서비스 전체 Web Lock, journal 실행 token+세대 fence, sync 전후 세대 대조, 영상·포스터 404, purge op 재시도로 닫았고 잠금 거부·세대 불일치·한쪽 잔재·두 키 주입을 추가했다.
- Android 네이티브 저장 변경이 포함되어 Pages와 `apk-latest`가 모두 배포 표면이다. 자동 게이트와 운영 read-back은 완료 뒤 기록하며, 실제 Android 시스템 저장창·대용량 ZIP·두 실기기 영상 재생은 사용자 확인 경계로 남긴다.

## HANDOFF-0140 · **전체 ZIP 백업·확정 저장·영상 파일 왕복검사** (2026-08-10 · 구현됨, 미릴리스)

- 사용자 결정으로 기본 파일명을 `YYYYMMDD_HHMM_Bugeon-Journey.zip`으로 통일했다. 데이터 관리와 진단의 전체 백업은 ZIP을 기본으로 하며 사진·소리·영상 본체/포스터·장소·비용·삭제 메타를 함께 담고 ZIP/JSON 자동 복원은 유지한다.
- Android 셸에 SAF 저장 플러그인을 추가했다. 사용자가 저장 위치를 직접 고르면 청크 쓰기 후 같은 URI의 길이와 SHA-256을 되읽어야만 「저장하고 다시 읽어 확인」이라고 말한다. 지원 브라우저도 저장 선택기+조각별 바이트 대조를 쓰며, anchor fallback은 「다운로드 요청」과 보통 `Download` 폴더라는 경계만 말한다.
- 실제 파일 왕복 시험을 trip-only JSON에서 trip+moment+video ZIP으로 높였다. 영상 본체·poster 해시, production 병합, 엔티티+원장 되돌리기 의사, read-back, 의도적 abort, 세 entity와 큐/원장 잔재 0건을 전부 확인한다. ZIP 안에서 참조한 사진·소리·영상 파일이 빠지면 조용히 행을 버리지 않고 손상으로 실패한다.
- DR 독립감사가 첫 판을 HOLD했다. 중앙 디렉터리 CRC를 쓰고도 reader가 확인하지 않았고, manifest가 선언한 여행 조각·`places.json`·v2 audio/videos 목록이 빠져도 빈 결과로 반올림할 수 있었다(M-0141). 이제 CRC·중복 경로·manifest 정확집합·필수 목록·0바이트 정상 위장을 모두 손상으로 닫고 주입 테스트를 추가했다.
- Android 저장은 JS 원본 청크 SHA-256을 네이티브 수신 바이트와 먼저 대조한 뒤 URI read-back을 하며, picker 대기 중 중복 begin도 막는다. 왕복검사는 trip·moment·video의 Blob 제외 전 필드와 정확히 `update 1 + unpurge 1`, `local_only`, attempts 0까지 확인한다. 실물 파일이 이미 없는 행은 개수를 별도 경고하고 완전백업 신선도를 갱신하지 않는다.
- 첫 표적 실행이 Dexie 트랜잭션 안 일반 WebCrypto await 때문에 시험 잔재 9건을 남기는 결함을 RED로 잡았다(M-0140). `Dexie.waitFor`로 고쳤고 감사 보강 후 표적 38/38·TypeScript·Android `assembleDebug`가 통과했다. 최종 v2.12 build와 전체 harness는 모든 게이트·유닛·editor/diagnostics live를 SKIP 없이 통과했으며, DR 재감사는 코드 P0/P1 없음으로 PASS했다.
- 아직 버전 증가·커밋·push·PR·Pages/APK 배포는 하지 않았다. 네이티브 변경이라 실제 사용자 기기에 닿으려면 다음 릴리스에서 웹과 `apk-latest` 두 표면을 모두 배포하고 Android 시스템 저장창·대용량 ZIP·실제 폴더 출현을 실기기로 확인해야 한다.
...
**하네스 게이트**: SSOT=`scripts/harness.mjs`, 현재 **<!--reg:gateCount-->75<!--/reg-->개**. **개수도 목록도 여기 손으로 적지 않는다** — `src/app/registry.gen.ts`(자동 생성, `check-registry-gen`이 드리프트 차단)에서 파생하고 개발자 정보→설계 개요도/가이드가 그 목록과 한 줄 설명을 그대로 표시한다. 목록을 눈으로 보려면 `npm run harness` 또는 `src/app/registry.gen.ts`를 열면 된다.
...
**검증**: harness <!--reg:gateCount-->75<!--/reg-->/27 · 유닛 410 · verify-editor-live 127/127 · build 그린. **미실행**: 실기기 PWA 설치·실제 로밍 — 사용자 확인 몫.
...
**검증**: harness <!--reg:gateCount-->75<!--/reg-->/26 · 유닛 410 · verify-editor-live 123/123(뷰어 13종·폰트 조각 4종 포함, 실제 Chromium이 `dist` 서빙) · build 그린. **미실행(정직)**: 실기기 터치·네트워크 동기화(샌드박스가 `*.supabase.co` 차단)·PWA 설치 — 여전히 사용자 실기기 확인 몫.
...
7. **작업 전 필수 스킬**(해당 영역 수정 시 **반드시 로드** — 현재 <!--reg:skillCount-->13<!--/reg-->개):
...
npm run harness                        # Required 게이트 전체 (현재 <!--reg:gateCount-->75<!--/reg-->개 — 목록은 scripts/harness.mjs, 손편집 나열 금지 M-0001; 이 숫자는 gen-registry가 자동 갱신·check-doc-counts가 대조)
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(건너뜀 0) · 유닛 1016 · build OK · 문장은 순수 함수라 라이브가 SSOT로 따라옴(`verify-editor-live` 문장 대조 PASS).
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 1016(photoMetaSniff 5 포함) · build OK
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **1011** · live 275/275 + 22/22
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **998** · live 270/270 + 22/22
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **989** · live 270/270 + 22/22
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **981** · live 270/270 + 22/22 · `check-adapter-parity` PASS(정본↔어댑터 2개 글자 단위 일치)
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **978** · live 270/270 + 22/22 · build OK
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 969 · `verify-editor-live` **270/270**(+4) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **969**(+18) · `verify-editor-live` **266/266**(+9) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 951 · `verify-editor-live` **257/257**(+3) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 951(+8) · `verify-editor-live` **254/254**(+7) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 943 · `verify-editor-live` **247/247**(+4) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **943**(+4) · `verify-editor-live` **243/243**(+4) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **939**(+21) · `verify-editor-live` **239/239**(+23) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(**건너뜀 0**) · 유닛 **918**(신규 `tripClock.test.ts` 29건 + 시간대 계약 케이스 확장) · `verify-editor-live` **216/216**(신규 18건) · `verify-diagnostics-live` 22/22 · build OK.
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(건너뜀 0) · 유닛 **882** · `verify-editor-live` **198/198** · build OK.
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(건너뜀 0) · 유닛 **745**(+23) · live 165+22 · build OK.
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(건너뜀 0) · 유닛 **715**(+13) · live 165 + **14** · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(라이브 2종 포함 — 건너뜀 0) · 유닛 702 · live 165+12 · build OK. **주입 RED**: 미등록 화면 · `scripts/`·`supabase/` 라우트 제거 · 셀프테스트 강제 실패(Required이므로 FAIL, SKIP 아님).
...
하네스 <!--reg:gateCount-->75<!--/reg-->개 전부 PASS(라이브 2종 포함 — 건너뛴 것 없음) · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS · 유닛 542건 · **주입 RED 8건**(백업 호출 제거 · `occurred_at`·`taken_at` 되돌리기 · `CHECK_COUNT` 드리프트 · `localeCompare` 복귀 · `withCanonicalStamps` 축소 · `compareInstants` 무력화 · 게이트 자체검사) · build OK · live 136/136(1회 흔들림, 아래 참조).
...
**검증**: 하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS · 유닛 526건 · 옛 로직 주입 **3건 RED** · live 136/136 · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS · 유닛 513건(instantFormat 28건 신설) · 주입 RED 3건(rowmap 타입 · merge · integrity) · live 136/136 · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(로컬 + Node 20) · 유닛 PASS(원자성 3건 신설 + 되살리기 3건) · 주입 RED 확인 · live 130/130 · build OK.
...
**검증**: 하네스 <!--reg:gateCount-->75<!--/reg-->개 PASS(로컬 Node + **CI가 쓰는 Node 20**) · 유닛 PASS(`restoreUnpurge` 12건 신설 + 문장 검사 6건) · **주입 검증**으로 가드 제거·되읽기 신뢰·보호 해제 각각 RED 확인 · `verify-editor-live` 130/130 · `npm run build` OK.
