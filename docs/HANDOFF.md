...
- **되읽기(§18-F)의 한계**: 배포는 성공했으므로 **Deno가 파싱했다**는 사실은 확정이다. 행동 확인(사전요청 200)은 이 샌드박스가 `*.supabase.co`를 막아 못 했다 — **다음 검색 한 번의 운영 로그**가 그 판정자다.
- **다음 작업**: ①사용자가 검색 1회 → 로그에서 `OPTIONS 400`이 사라졌는지 확인 ②T-025(앱이 「못 물어봤다」를 「없다」로 접지 않게) ③국내 제공자를 쓰려면 시크릿 등록.
