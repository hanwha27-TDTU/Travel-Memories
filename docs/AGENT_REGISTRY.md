...
> **운영 원칙 (§18)**: 139개를 동시에 실행하지 않는다. Orchestrator가 작업 성격에 따라 필요한 역할만 호출한다. 실제 `.claude/agents/`에는 이 역할들을 묶은 **통합 에이전트 10개 + 디자인 에이전트 16개 = 물리 에이전트 26개**를 생성한다. 각 통합 에이전트는 아래 세부 역할을 내부 책임으로 포함한다. (즉 **139 논리 역할 → 26 물리 에이전트.** v0.2 기준서의 123→10을 상위 집합으로 확장한 것이다.)
